...
class ArxivSpider(scrapy.Spider):
    def __init__(self, *args, **kwargs):
        ...
        # 测试模式：设置 MAX_PAPERS 环境变量限制爬取论文数
        max_papers = os.environ.get("MAX_PAPERS", "")
        self.max_papers = int(max_papers) if max_papers.isdigit() else None
        self.paper_count = 0
    ...
    def parse(self, response):
        ...
        for paper in response.css("dl dt"):
            if self.max_papers and self.paper_count >= self.max_papers:
                self.logger.info(f"Reached MAX_PAPERS limit ({self.max_papers}), stopping crawl")
                return
            ...
            if subjects_text:
                ...
                if paper_categories.intersection(self.target_categories):
                    ...
                    self.logger.info(f"Found paper {arxiv_id} with categories {paper_categories} ({self.paper_count}/{self.max_papers or '∞'})")
                else:
