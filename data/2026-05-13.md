...
TL;DR: 本文提出EpiKG，一种带有断言和时序标签的患者知识图谱，通过问题意图路由检索，在真实EHR笔记上显著提升临床问答准确性。在ClinicalBench基准上，意图感知的KG-RAG相比稠密检索基线提升8.84个百分点，主要结果提升22个百分点。医生审核发现56%的自动生成参考答案有缺陷，强调临床QA基准需要医生人工审核。


<details>
  <summary>Details</summary>
Motivation: 现有临床推理基准仅评估干净输入，忽略了真实EHR笔记中否定、时序和患者归属关系等断言敏感问题，导致检索阶段就可能出错。因此需要关注检索步骤，构建能应对断言变化的检索系统，并通过医生审核确保基准答案的可靠性。

Method: 构建EpiKG患者知识图谱，为每个事实附加断言标签和时序标签，根据问题意图进行检索路由。设计ClinicalBench测试集，包含43名MIMIC-IV患者、400个问题，覆盖9个断言敏感类别。在6个LLM上进行7条件消融实验，主要终点采用双外部医生盲评的50项严格一致条目的配对McNemar检验，对比意图感知KG-RAG与Contriever稠密检索基线。

Result: 主要终点：与基线相比正确率提升+22.0个百分点（95%置信区间[+5.1, +31.5]，p=0.0192）。架构新颖性（意图感知KG-RAG vs. 稠密检索）在排除变化后的362题子集上提升+8.84个百分点（p=1.79e-3），在已知意图下提升+12.43个百分点。敏感性分析：三医生多数票提升+24.0个百分点，关键词代理提升+39.5个百分点。增益随LLM自身基线升高而缩小（beta=-1.123，r=-0.921，p=0.009）。医生审核发现56%的自动生成参考答案存在缺陷。

Conclusion: 意图感知、断言和时序敏感的KG-RAG方法显著改善了真实EHR笔记上的检索准确性。临床问答基准必须经过医生审核才能保证答案质量，否则自动生成的结果缺陷率过高。文章公开了ClinicalBench、冻结评估器、三方审核数据及EpiKG输出栈，供后续研究使用。
...
TL;DR: The Bicameral Model enables two frozen pretrained language models to coordinate through a continuous, concurrent channel using a small trainable neural interface, learning a selective communication protocol purely from task loss.


<details>
  <summary>Details</summary>
Motivation: Existing multi-model systems rely on text-based serialization, limiting efficiency and expressiveness; the paper explores whether language models can instead communicate via continuous hidden states to improve tool use and collaborative reasoning.

Method: Two frozen LMs are coupled through a trainable translation network and a learned suppression gate (≈1% of parameters). At each generation step, both models run in lockstep: a primary model drives the task, an auxiliary model executes tools or solves constraints, conditioning on each other’s activations via the interface. The gate learns a communication protocol without any prescribed format.

Result: On arithmetic with a calculator, accuracy improved from 36% to 96% (two 0.5B models). On logic grid puzzles with a Z3 solver, the system achieved 1.7× the unaugmented baseline (0.6B models). On mathematical reasoning with a Python sandbox, the auxiliary model generated problem-specific code from hidden-state signals without seeing the problem text.

Conclusion: Selective continuous communication can be learned end-to-end from task loss, enabling frozen LMs to coordinate with tools efficiently without explicit text exchanges, showing strong gains across multiple tool-augmented reasoning tasks.
...
TL;DR: ReVision reduces visual token redundancy in computer-use agent interaction histories via a learned patch selector, cutting token usage approximately 46% while improving success rate by 3%, enabling longer histories to boost performance.


<details>
  <summary>Details</summary>
Motivation: Computer-use agents encode each GUI screenshot into many visual tokens, causing token cost to grow rapidly with interaction history. Under fixed context budgets, this limits the amount of history that can be used, resulting in no or little performance gain from history. The inefficiency stems from redundant visual information across consecutive screenshots.

Method: ReVision trains multimodal language models on trajectories with a learned patch selector that compares patch representations across consecutive screenshots, removing redundant patches while preserving the spatial structure required by the model.

Result: On OSWorld, WebTailBench, and AgentNetBench using Qwen2.5-VL-7B with 5 history screenshots, ReVision reduces token usage by ~46% on average and improves success rate by 3% over a no-drop baseline. Longer trajectories can be processed with fewer tokens, and performance continues to improve as more past observations are added once redundancy is removed, showing that saturation was due to inefficient representations, not uselessness of past information.

Conclusion: The perceived saturation of visual history in CUAs arises from inefficient token representations, not from limited utility of past observations. By removing redundant visual patches, ReVision allows agents to effectively use longer histories, achieving better performance with fewer tokens.
...
TL;DR: Hebatron是一个基于NVIDIA Nemotron-3稀疏混合专家架构的希伯来语专用大语言模型，通过课程学习和反遗忘训练高效推理，性能超越同类模型。


<details>
  <summary>Details</summary>
Motivation: 填补希伯来语缺乏高效、开源、支持长上下文的专用大语言模型的空白，利用MoE架构在提升性能的同时降低推理成本。

Method: 采用三阶段从易到难的课程学习并持续进行反遗忘锚定，随后在200万条希伯来语-英语双语数据上监督微调；底层模型为Nemotron-3稀疏MoE架构。

Result: 课程顺序带来3个基准点的提升；Hebatron希伯来语推理平均分73.8%，优于DictaLM-3.0-24B-Thinking的68.9%，与Gemma-3-27B-IT相当；30B总参数仅激活3B，推理吞吐量提升约9倍，支持65K原生上下文。

Conclusion: 这是首次将Nemotron-3架构应用于特定语言，也是首个提供开放权重的希伯来语MoE模型并支持原生长上下文，其权重公开发布以促进希伯来语及闪语族NLP研究。
...
TL;DR: 本文评估了大语言模型从非正式灾难社交媒体帖子中提取因果关系的能力，提出专家评估框架，发现其具有潜力但存在风险。


<details>
  <summary>Details</summary>
Motivation: 灾难期间社交媒体帖子可增强态势感知，但因非正式语言和上下文依赖性，因果关系提取困难。作者研究大语言模型能否有效提取。

Method: 提出基于专家知识的评估框架，将大语言模型生成的因果图与基于灾难报告的参考图对比，并评估提取的关系是基于事后证据还是模型先验。

Result: 发现大语言模型用于灾难决策支持的因果提取既有潜力也有风险。

Conclusion: 结论：大语言模型可提取灾难帖子的因果关系，但需验证其依据以防风险。
...
TL;DR: 本文提出EvalAgent，一个通过注入领域评估技能自动化端到端智能体评估的AI助手，将首次运行即产出有效评估的比率从17.5%提升至65%，并在人工偏好对比中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 多步骤智能体评估成本高、依赖专家，而前沿编码助手仅凭简单提示无法可靠执行评估（执行成功率仅30%，且产生过度工程化的指标），需要专门的方法来将编码能力转化为可靠的评估自动化。

Method: EvalAgent将评估领域专业知识编码为评估技能（流程指令、可复用代码与模板、动态检索的API文档），组合成基于轨迹的流水线，生成包含指标、可执行代码和报告的完整评估制品。同时构建元评估框架与包含20个智能体的AgentEvalBench基准，并提出Eval@1指标衡量评估代码首次运行即成功且产生有意义结果的能力。

Result: EvalAgent将Eval@1从基线的17.5%提升至65%，并在与基线方法的人工专家偏好比较中获得79.5%的胜率。消融实验显示，移除评估技能会导致Eval@1从65%骤降至30%，证明领域技能对处理复杂评估至关重要。

Conclusion: 仅依靠通用编码能力不足以实现可靠的智能体自动化评估；注入结构化的领域评估技能（如EvalAgent中的评估技能）是使评估既聚焦又可靠的关键，这为自动化端到端评估流程提供了有效路径。
...
TL;DR: 本文提出Deep Reasoning方法，通过结构化元推理在推理时动态构建任务专属的推理支架，使LLM智能体能够灵活切换推理模式。其实例化智能体DOLORES在多个硬基准上显著优于现有支架方法，甚至8B模型超过32B基线。


<details>
  <summary>Details</summary>
Motivation: 人类能灵活切换规划、执行、修正目标等推理模式，而当前LLM智能体的推理支架是硬编码预设的，无法适应需要动态调整推理结构的复杂任务，导致性能脆弱。

Method: 提出Deep Reasoning，一种推理时方法，用形式语言将元推理表示为可执行分解（涵盖关联推理、形式计算和递归子问题求解），通过上下文示例编码分解原则引导测试时支架构建。并基于此实现了通用智能体DOLORES，将复杂任务分发到受控的推理线程中。

Result: 在三个模型尺寸、两个模型家族下，DOLORES在四项硬基准（多跳推理、长链问答、长上下文聚合、深度研究式信息搜寻）上全面超越现有支架方法，比最强基线平均提升24.8%。8B版本在过半设置中超越同家族32B所有基线。

Conclusion: 动态适配的推理支架能减少过早终止和幻觉，甚至弥合模型规模差距。未来智能体系统应将支架视为自适应推理，按需即时构建任务结构。
...
TL;DR: 提出LayerTracer框架，揭示深层为任务执行关键区且稳定性高，指导浅层训练深层冻结策略，效果优于全参数微调。


<details>
  <summary>Details</summary>
Motivation: 选择性层级更新缺乏可解释性指导，导致层级选择成为经验黑箱问题。

Method: 提出架构无关的诊断框架LayerTracer，通过定位任务执行位置和量化层敏感性，揭示层级表示演变模式与稳定性。

Result: 深层是任务执行关键区域且抗干扰性强；训练浅层冻结深层在C-Eval和CMMLU上优于全参数微调及反向分配；混合模型案例验证深层放置高质量预训练模块可有效保留知识。

Conclusion: 为资源受限团队提供低成本可解释方案，为持续预训练和混合模型构建的层级参数分配提供可操作指导。
...
TL;DR: Agent-BRACE decouples an LLM agent into a belief state model and a policy model, using structured natural language claims with verbalized certainty labels to compactly represent belief, achieving significant performance gains while keeping context size constant.


<details>
  <summary>Details</summary>
Motivation: LLMs in long-horizon partially observable tasks face two challenges: maintaining uncertainty over unobserved world attributes and unbounded history growth that dilutes task-relevant information. The principled solution of a belief state (a posterior distribution) is difficult to represent in open-ended text.

Method: We propose Agent-BRACE, which decouples the agent into a belief state model and a policy model, jointly trained via reinforcement learning. The belief model produces a structured approximate belief: a set of atomic natural language claims, each annotated with an ordinal verbalized certainty label (e.g., certain to unknown). The policy model conditions on this compact structured belief instead of the full interaction history, enabling action selection under explicit uncertainty.

Result: In long-horizon, partially observable embodied language environments, Agent-BRACE yields average absolute improvements of +14.5% (Qwen2.5-3B-Instruct) and +5.3% (Qwen3-4B-Instruct) over strong RL baselines, while maintaining a near-constant context window regardless of episode length. The learned belief becomes increasingly calibrated as evidence accumulates.

Conclusion: Agent-BRACE effectively addresses the dual challenges of partial observability and unbounded context in LLM agents by structuring belief representation and decoupling belief and policy learning, leading to improved performance, calibration, and scalability.
...
TL;DR: 本文提出一种基于受控语义扰动的评估框架，分析医学文献出版类型分类器的鲁棒性，并探索结合实体掩码和领域对抗训练的鲁棒性优化策略。结果表明，通过选择性抑制非任务关键特征、保留方法学信号，可以缓解鲁棒性与域内准确率之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 自动化出版类型与研究设计索引对于证据合成和知识发现至关重要，但现有模型在分布偏移下鲁棒性不足，可能过度依赖表面词汇或数据集特定线索。

Method: 设计基于受控语义扰动的鲁棒性评估框架，并研究两种训练策略：实体掩码（遮蔽特定实体）和领域对抗训练，以抑制虚假主题相关性，同时保留方法学线索。

Result: 在选择性抑制主题特征的情况下，模型在保持域内准确率的同时提升了鲁棒性；改善源于两个互补机制：更多依赖显式方法学提示，减少对虚假领域主题特征的依赖。

Conclusion: 特征级鲁棒性分析对出版类型分类具有重要意义；精炼掩码和对抗目标以更精准地抑制主题信息，有望进一步提升鲁棒性。
...
TL;DR: HLD provides systematic perplexity gains over logit-based KD but fails to consistently outperform on downstream tasks; the latent signal is useful but requires a breakthrough.


<details>
  <summary>Details</summary>
Motivation: Most KD research for LLMs relies on output logits, ignoring the semantic information in teacher intermediate representations; HLD's application to decoder-only pre-training at scale remains largely unexplored.

Method: Compute-controlled experiments benchmark HLD against logit-based KD and self-supervised baselines, using Gemma3 3.4B as teacher and 123M/735M student models trained on up to 168B tokens from C4.

Result: HLD does not consistently outperform standard KD on downstream evaluation tasks but achieves a systematic perplexity gain over KD across all shared-hyperparameter configurations.

Conclusion: A latent signal can be extracted via HLD, but a breakthrough is needed for it to have a more significant role in LLM pre-training.
...
TL;DR: Checkup2Action 是一个多模态临床体检报告数据集和基准，用于评估大型语言模型从报告生成结构化的患者导向行动卡片的能力，发现模型在问题覆盖、正确性、简洁性和安全性之间存在权衡。


<details>
  <summary>Details</summary>
Motivation: 临床体检报告包含多种异构信息，普通人难以解读并转化为具体行动。现有大语言模型在医疗总结和分诊方面有潜力，但它们在多模态检查报告上生成安全、优先级明确且以患者为中心的行动建议的能力尚缺乏基准测试。

Method: 作者构建了包含 2000 份去标识真实体检报告的 Checkup2Action 数据集，将任务定义为受约束的结构化生成：每张行动卡片需描述一个临床相关事项，并指定优先级、推荐科室、随访时间窗、患者解释和向医生提问的问题，同时避免诊断或治疗性主张。此外，设计了覆盖问题覆盖度、精确度、优先级一致性、科室和时间推荐准确率、行动复杂性、有用性、可读性和安全性合规性的评估协议，并使用通用及医学大语言模型进行实验。

Result: 实验表明，通用和医学大语言模型在问题覆盖度、行动正确性、简洁性和安全性对齐之间存在明显权衡，没有模型能在所有指标上同时达到最优。

Conclusion: Checkup2Action 提供了一个新的多模态基准，用于评估针对临床体检报告的患者导向推理能力，揭示了现有模型在结构化行动生成中的不足与改进方向。
...
TL;DR: This paper proposes a hyperparameter-free, covariance-weighted optimization method for GRPO that balances exploration-exploitation by down-weighting extreme token updates, improving reasoning performance and entropy stability.


<details>
  <summary>Details</summary>
Motivation: GRPO suffers from an exploration-exploitation trade-off that leads to suboptimal performance; theoretically, entropy changes depend on the covariance between token probabilities and advantages.

Method: A covariance-weighted optimization approach using a Gaussian kernel to dynamically down-weight extreme token-level updates without hyperparameters.

Result: The method outperforms standard GRPO on reasoning benchmarks and effectively stabilizes entropy throughout training.

Conclusion: The covariance-weighted method automatically mitigates exploration-exploitation instability, yielding better downstream performance and training stability.
...
TL;DR: DiffScore leverages masked diffusion language models to evaluate text without positional bias via bidirectional scoring, achieving superior performance across diverse benchmarks.


<details>
  <summary>Details</summary>
Motivation: Autoregressive language models introduce positional bias in text evaluation because their left-to-right factorization scores early tokens with only leftward context, conflating architectural asymmetry with true text quality.

Method: The paper proposes masked reconstruction with large masked diffusion language models to score every token using full bidirectional context. DiffScore measures text recoverability over continuous masking rates, generating multi-timestep quality profiles and a bidirectional PMI decomposition to separate fluency from faithfulness.

Result: DiffScore consistently outperforms autoregressive baselines in both zero-shot and fine-tuned settings across ten text evaluation benchmarks.

Conclusion: DiffScore eliminates positional bias, establishes a hierarchical evaluation from local fluency to global coherence, and provides new diagnostic tools for fine-grained text quality analysis.
...
TL;DR: 本文提出OmniThoughtVis，一个可扩展的多模态思维链蒸馏流水线，从大模型向小模型迁移推理能力，生成并筛选出180万高质量数据，使蒸馏后的4B模型在多个基准上超越原8B模型，实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 为解决多模态大模型在实际部署中因延迟和资源受限而难以直接应用的问题，同时弥补小型多模态模型缺乏大规模高质量思维链监督数据的瓶颈。

Method: 基于开放源种子池，生成结构化思维链轨迹，并联合标注推理难度、答案质量和语义任务标签；通过规则过滤、难度感知选择和标签多样性采样，构建含180万样本的可控子集；用于从2B到8B规模的Qwen3-VL模型的知识蒸馏。

Result: 蒸馏后模型在多模态推理基准上普遍提升，4B模型在MathVerse上最高提升16.8分，在MMMU-Pro上提升5.6分，且蒸馏后的4B模型在多项任务上达到或超越未蒸馏的8B基线。

Conclusion: 可扩展的推理蒸馏能有效赋予小型多模态模型强大的思维链推理能力，显著缩小与大型模型的性能差距，为面向实际部署的多模态大语言模型提供高效实用的优化路径。
...
TL;DR: Macro框架利用直接偏好优化（DPO）生成多语言反事实解释，有效平衡有效性与最小性，在多个模型和语言上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在非英语语言中生成有效的反事实解释，且面临有效性与最小性之间的持续权衡，影响解释质量。

Method: 提出Macro偏好对齐框架，采用DPO进行多语言SCE生成，通过组合评分函数构建偏好对，将权衡转化为可测量的偏好信号。

Result: 在4个LLM和7种类型多样语言上，Macro相对思维链基线有效性平均提升12.55%，且不降低最小性，并避免了基于翻译基线的严重最小性违规；相比监督微调在两个指标上均更优。

Conclusion: 偏好优化是增强多语言模型解释性的有前景方向，Macro可提高跨语言扰动对齐并减少生成错误。
...
TL;DR: 本文构建了一个基于日本全国学力测试的真实中学考试多模态数据集，保留原始排版、图表与全国学生作答分布，实现对大语言模型与人类表现的直接比较。基准测试表明模型在不同科目上差异显著，且对视觉推理高度敏感。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试多基于合成或策划数据，缺乏日本K-12阶段真实考试的多模态评估平台。本文旨在利用高有效性的真实学力测试，填补这一空缺，为多模态大语言模型提供人机可比的评估框架。

Method: 从日本全国学力·学习状况调查中收集官方公布的中学科学、数学、国语试题，构建多模态数据集，完整保留原始排版、图表和文本。结合全国约90万学生的作答分布，以精确匹配准确率和字符级F1为指标，对近期多模态大语言模型进行基准测试，并通过人工评估和LLM-as-judge分析自动评分的可靠性。

Result: 模型在不同科目上的表现差异很大，且对题目中视觉推理的需求极其敏感。人工评估与LLM-as-judge分析证实自动评分方法具有一定可靠性。

Conclusion: 本数据集建立了可复现、以人类表现为基准的多模态教育推理评估平台，为真实考试情境下的评估、反馈生成和可解释人工智能等研究提供支持。
...
TL;DR: 本文提出通过比较局部语法的一致性来优化葡萄牙语人名命名实体识别，最终F值达76.86，比现有最佳结果提高6个百分点。


<details>
  <summary>Details</summary>
Motivation: 人名命名实体识别是信息提取中重要但非平凡的任务，需要改进葡萄牙语人名提取的性能。

Method: 使用一个工具比较两个局部语法（LG）获得的一致性，并突出差异；通过分析比较结果，观察成对LG之间的包含、交叠和分离关系，从而组合出效果最佳的语法集合。

Result: 在第二届HAREM的黄金集合上进行案例研究，增强后的语法取得76.86的F值，比葡萄牙语现有最佳水平提高了6个百分点。

Conclusion: 通过比较局部语法关系并择优组合，可以显著提升葡萄牙语人名命名实体识别的性能。
...
TL;DR: Qwen-Scope is an open-source suite of sparse autoencoders (SAEs) for the Qwen model family, demonstrating that SAEs can go beyond post-hoc analysis to serve as practical interfaces for model development through inference-time steering, evaluation analysis, data-centric workflows, and post-training optimization.


<details>
  <summary>Details</summary>
Motivation: Large language models have opaque internal decision-making, limiting inspection, control, and improvement. SAEs decompose activations into sparse interpretable features, but their practical utility beyond analysis remains underexplored.

Method: Built 14 groups of SAEs across 7 Qwen3 and Qwen3.5 model variants (dense and MoE). Applied SAEs in four directions: (i) inference-time steering via feature directions; (ii) evaluation analysis using activated features as proxies for redundancy and coverage; (iii) data-centric workflows for toxicity classification and safety data synthesis; (iv) post-training optimization by incorporating SAE-derived signals into supervised fine-tuning and reinforcement learning objectives.

Result: SAEs enabled steering of language, concepts, and preferences; provided representation-level proxies for benchmark analysis; supported multilingual toxicity classification and safety-oriented data generation; and mitigated code-switching and repetition via fine-tuning. SAEs proved to be reusable interfaces for diagnosis, control, evaluation, and improvement of LLMs.

Conclusion: SAEs can serve as practical, reusable representation-level interfaces beyond post-hoc analysis. Open-sourcing Qwen-Scope aims to facilitate mechanistic research and accelerate workflows connecting model internals to downstream behavior.
...
TL;DR: 提出一种基于评分准则的判别评估框架RuDE，通过构造对比对预测基座模型的后训练潜力，与后训练性能相关性超90%，并能高效识别高潜力小模型。


<details>
  <summary>Details</summary>
Motivation: 传统基准如MMLU难以反映基座模型在复杂开放场景下的可塑性，导致模型选择效率低下，需要一种预测后训练潜力的方法。

Method: 提出RuDE框架，利用响应判别绕过基座模型的生成差距，基于4C分类法通过细粒度的评分准则违规构造可控对比对。

Result: 实验表明与后训练性能的相关性超过90%，并通过强化学习验证了RuDE能有效识别出超越大模型的高潜力小模型。

Conclusion: RuDE为基座模型开发提供了计算高效的机制，通过预测后训练潜力优化模型选择。
...
TL;DR: 本文提出ActGuide-RL，通过注入动作数据作为计划式参考指导，利用混合策略训练优化引导和非引导轨迹，实现最小干预原则的自适应回退，显著提升基于LLM的智能体强化学习探索能力，减少对监督微调数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 在基于大型语言模型的智能体强化学习中，当基础策略无法到达奖励状态时，训练信号仅出现在其能力范围内，需要额外训练或外部指导来恢复有效学习信号。现有方法依赖昂贵的迭代监督微调，而本文希望利用日常人类交互产生的大量动作数据来替代。

Method: 提出ActGuide-RL方法：将动作数据注入为计划式参考指导，使智能体策略克服到达奖励状态的障碍；通过混合策略训练联合优化有指导和无指导的轨迹，将探索增益内化回无指导策略；基于效益-风险权衡的理论与实证分析，采用最小干预原则，仅将指导作为自适应回退机制，匹配任务难度同时最小化离线策略风险。

Result: 在搜索智能体基准测试中，ActGuide-RL相比零强化学习显著提升（Qwen3-4B在GAIA上+10.7个百分点，XBench上+19个百分点），且在没有冷启动的情况下性能与监督微调+强化学习流程相当。

Conclusion: 提出了一种新的智能体强化学习范式，通过可扩展的动作指导而非大量监督微调数据，降低了对昂贵SFT数据的依赖。
...
TL;DR: 本文提出SAGE框架，利用微调的小模型以可扩展且低成本的方式自动增强知识评估基准的鲁棒性，生成的基准质量与人工注释相当，并能泛化到其他基准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在标准知识基准上表现强劲，但在同一知识的不同问题变体下表现脆弱。现有LLM辅助的生成-验证管道因变体生成率低和验证不可靠而成本高、难以扩展，因此需要一种可扩展的鲁棒性增强方法。

Method: 提出SAGE框架，包含两个微调的小模型：VariantQual是基于人工标注种子数据训练的评分准则验证器，VariantGen是初始经监督微调、随后以VariantQual为奖励模型通过强化学习优化的问题变体生成器。

Result: 在HellaSwag上，SAGE构建的大规模鲁棒性增强基准质量与人工标注的HellaSwag-Pro相当，但成本大幅降低；微调模型无需针对特定基准调整即可泛化到MMLU。

Conclusion: SAGE为知识评估基准的鲁棒性增强提供了一种可扩展且经济的自动化解决方案，通过小模型协同工作，在维持高质量的同时显著降低人工和计算成本，并具有良好的泛化能力。
...
TL;DR: SKILLGRAPH represents reusable skills as a directed graph with typed edges to capture dependencies, retrieves ordered subgraphs for multi-step tasks, and updates via reinforcement learning, achieving state-of-the-art on complex compositional tasks.


<details>
  <summary>Details</summary>
Motivation: Existing skill libraries store skills as isolated entries and retrieve by semantic similarity, failing to handle compositional tasks that require understanding how skills depend on each other, and lacking structural cues for library maintenance (merging, splitting, removal).

Method: SKILLGRAPH models skills as nodes in a directed graph with typed edges encoding prerequisite, enhancement, and co-occurrence relations. For a new task, it retrieves an ordered skill subgraph to guide multi-step decision making. The graph is continuously updated from agent trajectories and reinforcement learning feedback to jointly improve the skill library and agent policy.

Result: Experiments on ALFWorld, WebShop, and seven search-augmented QA tasks show SKILLGRAPH achieves state-of-the-art performance compared to memory-augmented RL methods, with particularly large gains on complex tasks requiring skill composition.

Conclusion: Structuring skills as a graph with dependency relations enables more effective skill reuse and library maintenance, leading to significant improvements in compositional task solving.
...
TL;DR: 研究发现，儿童导向语言的动词学习优势并非其独有优化，而是反映了口语语域的普遍特性。


<details>
  <summary>Details</summary>
Motivation: 探究儿童导向语言是否针对语言学习进行了优化，以及它促进了语言发展的哪些方面，特别是动词意义的习得。

Method: 使用神经语言模型，分别在儿童导向语言和成人导向语言（口语和书面语）上训练，通过选择性移除句法或词汇共现信息，评估这些操作对动词意义习得的影响，并追踪训练过程中的语义和句法表现。

Result: 破坏句法信息会损害所有数据集上的学习，但基于儿童导向语言和成人导向口语训练的模型比基于书面语训练的模型更具复原力。语义发展先于句法，动词意义在稳健的句法能力之前出现，这种不同步在口语领域（尤其是儿童导向语言）最为显著。

Conclusion: 先前归因于儿童导向语言的动词学习优势，可能更多地源于口语语域的普遍属性，而非儿童导向语言特有的优化。
...
TL;DR: 研究利用稀疏自编码器探查大语言模型中编码语法违规的特征，未发现统一的违规检测器。


<details>
  <summary>Details</summary>
Motivation: LLM预测机制不透明，假设其参数中编码了语言约束违规的表示，并在处理不合语法句子时选择性激活，旨在验证此假设。

Method: 采用稀疏自编码器分解多义激活为稀疏单义特征，引入敏感性分数无监督地识别违规相关特征，并提出包含三个标准的联合证伪框架进行评估。

Result: 结果为负面：(1) 证伪标准在不同语言现象上无法同时满足；(2) 无特征在所有类别间一致共享。尽管部分现象显示选择性因果结构的局部证据，但整体模式对当前LLM中存在统一语法违规检测器的假设支持有限。

Conclusion: 当前大语言模型可能不具备一组统一的语法违规检测器，处理语法违规的方式或许更分布式或依赖其他机制。
...
TL;DR: 本文系统综述了低资源手语识别与翻译研究，以阿塞拜疆手语为例，提炼八项可操作经验并提出从模型中心到数据中心、独立到自适应系统、参考评估到任务特定指标的三大范式转变，并给出基于轻量级MediaPipe架构的技术路线图。


<details>
  <summary>Details</summary>
Motivation: 由于文档稀缺、数据集稀疏和计算工具不足，全球300多种手语仍处于严重低资源状态，阻碍了聋人群体的交流和技术应用。

Method: 采用系统综述方法，分析全球手语处理项目，结合阿塞拜疆手语案例研究，提取经验教训；进而提出范式转变并设计轻量级MediaPipe架构、社区验证标注和离线优先部署的技术路线图。

Result: 提炼出包括社区协同设计、方言多样性捕捉和隐私保护姿态表征在内的八项全球经验；提出数据中心AI、签名者自适应系统和任务特定评估指标三大范式转变；为阿塞拜疆手语制定了具体技术实施路径。

Conclusion: 实现技术进步需要以聋人社区为中心的持续跨学科合作，确保文化真实性、伦理治理和实际沟通效益。
...
TL;DR: 本文提出一种无需个体标签的三代理分层贝叶斯管道，用于纠正生产环境中大语言模型反馈的选择偏差，仅依靠可观测的通道先验即可将偏差估计误差控制在4-13个百分点以内。


<details>
  <summary>Details</summary>
Motivation: 生产环境的大语言模型反馈通常来自非随机用户子集，满意度分布两头重，简单平均会偏离真实质量40-50个百分点。需要一种不依赖于个体交互真实标签的偏差校正方法。

Method: 设计三代理管道：①主题聚类代理，通过文本嵌入的UMAP+HDBSCAN将交互流划分为簇；②偏差建模代理，使用两步分层Beta-Binomial模型与NUTS推演，对各主题的反馈选择率𝑠_𝑐与质量𝑞_𝑐实施部分池化推断；③综合代理，以真实主题流行度𝜋̂_𝑐=𝑛_𝑐/𝑁加权𝑞_𝑐，输出带可信区间的偏差校正总体后验𝑄̅=∑𝜋̂_𝑐𝑞_𝑐，并提取漂移信号供在线校准。关键在于对反馈渠道引入轻度先验（典型正向率与负正比），该先验可从生产仪表盘直接获取，打破似然的多解简并。

Result: 在UltraFeedback（10,232条交互，18个簇，真实质量𝑄⋆=0.6249）上模拟主题与情感依赖的选择偏差：当正负样本比从1:1扫描至30:1，带渠道先验的Hierarchical-Informed模型偏差仅4-13个百分点，且在k_max=10%的50次随机种子重复中95%可信区间全覆盖𝑄⋆；无渠道先验的弱先验变体则偏差22-33个百分点。

Conclusion: 反馈渠道侧的轻度先验（而非潜在质量先验）是打破各簇充分统计量等价拟合族简并的关键，使分层贝叶斯模型能够在无标签条件下准确校正选择偏差，并支持生产环境的在线重校准。
...
TL;DR: 本文提出 Distilled Group Relative Policy Optimization (dGRPO)，通过在线知识蒸馏增强 GRPO，实现更稳定高效的长上下文推理训练，同时保持短上下文能力。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文训练方法存在缺陷：离策略方法（SFT、KD）存在曝光偏差且难以纠错；同策略强化学习（GRPO）虽能对齐生成状态但奖励稀疏导致不稳定和低效；在线蒸馏（OPD）提供密集指导却无法直接优化任意奖励信号。因此需要一种能结合密集监督与结果优化优点的方法。

Method: 提出 dGRPO，将 GRPO 与来自强教师的在线知识蒸馏（OPD）相结合，同时引入覆盖多跳推理、上下文基础和长文本生成的合成数据集 LongBlocks。该方法在单目标中融合基于结果的策略优化和密集蒸馏信号。

Result: 大量实验和消融对比表明，dGRPO 相比纯离策略训练和稀疏奖励 GRPO，能更稳定、有效地提升长上下文推理能力，且不损失短上下文性能。

Conclusion: 将基于结果的策略优化与知识蒸馏统一在单一目标中，为长上下文推理提供了更稳定、高效的训练路径，同时完好保留短上下文能力。
...
TL;DR: PRISM is a training-free retrieval-side framework for long-horizon language agents that jointly addresses retrieval and compression over graph-structured memory, achieving high accuracy with minimal context budget.


<details>
  <summary>Details</summary>
Motivation: Long-horizon language agents generate conversation history faster than fixed context windows can accommodate, making memory management crucial for accuracy and cost. Existing methods either blindly expand context, incur heavy token costs from upfront fact extraction, or use heuristic graph traversal with suboptimal performance.

Method: PRISM formulates retrieval as min-cost selection over typed path templates in a graph and integrates four inference-time components: Hierarchical Bundle Search over typed relation paths, Query-Sensitive Edge Costing that aligns traversal with query intent, Evidence Compression to produce a compact answer-side context, and Adaptive Intent Routing to divert most queries through zero-LLM tiers. No fine-tuning or ingestion pipeline modification is required.

Result: On the LoCoMo benchmark, PRISM significantly outperforms same-protocol baselines in LLM-judge accuracy while using an order-of-magnitude smaller context budget, establishing a new state-of-the-art on the accuracy-context-cost frontier.

Conclusion: PRISM demonstrates that joint retrieval and compression over graph-structured memory can effectively balance answer quality and retrieval efficiency, outperforming existing memory management approaches for long-horizon agents.
...
TL;DR: 本文介绍了BioCreative IX MedHopQA共享任务，构建了包含1000个需两跳推理的生物医学问答数据集，侧重罕见疾病。顶级系统使用检索增强生成（RAG）在MedCPT概念评分上达到89.30%的F1值，远超零样本基线。数据集和基准公开。


<details>
  <summary>Details</summary>
Motivation: 生物医学领域的多跳问答需要整合多个来源的信息，现有大语言模型表现不足。该任务旨在基准化大模型在疾病、基因、化学物质上的多跳推理能力，特别关注罕见病，以推动复杂问题解答的研究。

Method: 创建了一个包含1000个问答对的数据集，每个问题需要从两个不同的维基百科页面整合信息进行两跳推理。吸引了13支团队提交48个系统，采用表面字符串比较（精确匹配）和概念准确性（MedCPT评分）双轨评估，系统普遍采用检索增强生成（RAG）及相关检索策略。

Result: 最高排名提交在MedCPT指标上取得89.30%的F1分数和87.30%的精确匹配分数，而零样本基线仅分别为67.40%和60.20%。核心发现是RAG及检索策略对性能至关重要，概念级评估改善了表面形式不同时的答案评判。

Conclusion: 共享任务揭示了基线模型与增强系统间的显著差距，证实RAG在生物医学多跳问答中的关键作用。MedHopQA数据集已公开，以促进该领域的持续进展。
...
TL;DR: 本文提出类别错误敏感性指数（ISEC），通过融合语义距离、形态转换成本和经验频率来排序类别对的混淆风险，利用向量数据库将计算效率提升约195倍，并在三个数据集上验证。


<details>
  <summary>Details</summary>
Motivation: 中小企业数据录入系统中类别误分类问题严重，尤其当类别间存在语义或形态相近时，手工录入错误不可逆地扭曲关键绩效指标，误导管理决策。现有标准化工具孤立地评估语义与形态，依赖标准词典，无法处理中小企业主数据中的定制SKU、缩写及专业术语。

Method: 设计ISEC有序复合评分：通过词嵌入计算语义距离，采用自适应Damerau-Levenshtein算法计算自定义加权形态转换代价，并整合经验频率；基于向量数据库架构降低计算复杂度。

Result: 与暴力搜索相比，ISEC计算性能提升约195倍；在政府司法记录、零售库存与合成ISO金属加工目录三个异质数据集上得到验证，证明其可扩展性。

Conclusion: ISEC为中小企业提供了一种可扩展的主动式数据治理手段，能提前检测类别数据资产中的潜在结构性混淆风险，防止决策偏差。
...
TL;DR: MedHopQA是一个面向疾病的多跳推理基准，包含1000个专家构建的问答对，每个问题需融合两个维基百科条目的信息，以开放式自由文本作答，并利用本体同义词集增强评估。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学问答基准易受模式匹配、性能饱和及训练数据污染影响，多跳推理能力在临床任务中关键却未被充分代表。需构建能区分推理与匹配、可抵抗饱和与污染的评测基准。

Method: 通过人工标注、分诊、迭代验证和LLM裁判审核构建MedHopQA；问题跨越两篇维基百科文章，答案采用自由文本格式，引入MONDO、NCBI Gene等本体同义词集进行词汇与概念级评估；将1000个评分问题嵌入可公开下载的10000问题集合中，在CodaBench隐藏答案以防刷榜和污染。

Result: 提供了一个基准数据集和可复用的框架，用于构建以组合推理、抗饱和、抗污染为核心设计原则的未来生物医学问答数据集。

Conclusion: MedHopQA建立了强调多跳推理与抗饱和/污染能力的评测范式，为生物医学领域的大模型评估提供了更具区分度和实用性的资源。
...
TL;DR: We introduce LongMemEval-V2, a benchmark with 451 questions evaluating five memory abilities for web agents, and two AgentRunbook memory methods. AgentRunbook-C reaches 72.5% accuracy, far outperforming RAG baselines, but with high latency, while establishing a challenging testbed for environment experience memory.


<details>
  <summary>Details</summary>
Motivation: Existing agent memory benchmarks focus on user histories or short traces, failing to directly evaluate whether memory systems internalize environment-specific experience (e.g., interface affordances, state dynamics, workflows, failure modes) needed for specialized web tasks. LME-V2 fills this gap.

Method: LME-V2 contains 451 manually curated questions across five memory abilities (static state recall, dynamic state tracking, workflow knowledge, environment gotchas, premise awareness), paired with history trajectories of up to 500 trajectories and 115M tokens. Memory systems consume these histories and return compact evidence for downstream QA. We propose AgentRunbook-R, an efficient RAG-based memory with knowledge pools for observations, events, and strategy notes, and AgentRunbook-C, which stores trajectories as files and uses a coding agent in a sandbox to gather evidence.

Result: AgentRunbook-C achieves 72.5% average accuracy, substantially outperforming the best RAG baseline (48.5%) and the off-the-shelf coding agent baseline (69.3%). Coding agent methods have high latency costs, while AgentRunbook-C advances the accuracy-latency Pareto frontier but leaves room for improvement.

Conclusion: LME-V2 provides a challenging testbed for developing long-term memory systems that internalize environment experience. AgentRunbook-C demonstrates significant progress alongside an accuracy-latency trade-off, highlighting the need for further advances.
...
### [84] [FERA: Uncertainty-Aware Federated Reasoning for Large Language Models](https://arxiv.org/abs/2605.10082)
...
### [85] [Interpretable EEG Microstate Discovery via Variational Deep Embedding: A Systematic Architecture Search with Multi-Quadrant Evaluation](https://arxiv.org/abs/2605.10947)
...
### [86] [QuIDE: Mastering the Quantized Intelligence Trade-off via Active Optimization](https://arxiv.org/abs/2605.10959)
*Xiantao Jiang*
...
### [87] [Steering Without Breaking: Mechanistically Informed Interventions for Discrete Diffusion Language Models](https://arxiv.org/abs/2605.10971)
...
### [88] [Rotation-Preserving Supervised Fine-Tuning](https://arxiv.org/abs/2605.10973)
...
### [89] [Vertex-Softmax: Tight Transformer Verification via Exact Softmax Optimization](https://arxiv.org/abs/2605.10974)
...
### [90] [Hierarchical Multi-Scale Graph Neural Networks: Scalable Heterophilous Learning with Oversmoothing and Oversquashing Mitigation](https://arxiv.org/abs/2605.10975)
...
### [91] [LEAP: Unlocking dLLM Parallelism via Lookahead Early-Convergence Token Detection](https://arxiv.org/abs/2605.10980)
...
### [92] [$ξ$-DPO: Direct Preference Optimization via Ratio Reward Margin](https://arxiv.org/abs/2605.10981)
*Zhengyuan Fan,Zhonghua Wu,Yuxuan Du,Qun Chen*

Main category: cs.LG

TL;DR: SimPO faces hyperparameter tuning challenges due to non-interpretable margins. Analysis reveals that β controls sample filtering and γ's effect varies with reward gap structure. ξ-DPO is proposed, using a ratio reward margin that cancels β and yields an interpretable margin ξ, determinable from initial reward gaps, thus eliminating trial-and-error tuning.


<details>
  <summary>Details</summary>
Motivation: The joint tuning of hyperparameters β and γ in SimPO is difficult because the margin formulation is not easily interpretable across datasets with different reward gap structures. A method with an interpretable margin and easier tuning is needed.

Method: Reformulate the preference objective to minimize distance between reward gaps and optimal margins. Redefine the reward as a ratio of chosen to rejected, which cancels β and produces a bounded, interpretable margin ξ. ξ can be directly set from the initial reward gap distribution.

Result: ξ-DPO yields an interpretable ratio reward margin that can be determined from the initial reward gap distribution, avoiding repeated trial-and-error tuning of γ.

Conclusion: By introducing a ratio reward margin, ξ-DPO simplifies hyperparameter selection and provides an explicit, dataset-adaptive margin, leading to more stable and interpretable preference optimization.
...
### [93] [TMPO: Trajectory Matching Policy Optimization for Diverse and Efficient Diffusion Alignment](https://arxiv.org/abs/2605.10983)
...
TL;DR: 本文针对扩散模型强化学习中普遍存在的奖励破解问题，提出轨迹匹配策略优化方法TMPO，通过轨迹级奖励分布匹配替代标量奖励最大化，结合动态随机树采样，在保持生成多样性的同时提升奖励优化，在多个对齐任务上实现多样性与奖励的最优权衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的扩散模型微调方法多为模式寻求型，最大化期望奖励时缺乏对可接受轨迹概率分布的有效约束，导致奖励破解，表现为视觉模式坍塌和奖励不可靠，生成多样性和质量严重下降。

Method: 提出轨迹匹配策略优化（TMPO），核心包括：1) 引入Softmax轨迹平衡目标，将K条轨迹的策略概率与奖励诱导的玻尔兹曼分布进行匹配，利用前向KL散度的模式覆盖特性，在优化奖励的同时保留对可接受轨迹的覆盖；2) 动态随机树采样，通过共享去噪前缀并在动态调度步长处分支，减少大规模流匹配模型上多轨迹训练时间，提升训练效率。

Result: 在人类偏好、组合生成和文本渲染等对齐任务上，TMPO相比现有最优方法将生成多样性提升9.1%，在下游指标和效率方面均取得有竞争力的表现，实现了奖励与多样性的最优折衷。

Conclusion: TMPO通过轨迹级分布匹配有效抑制了奖励破解，兼顾生成多样性与奖励优化，为扩散模型的对齐训练提供了更优的权衡方案。
...
### [94] [Structural Interpretations of Protein Language Model Representations via Differentiable Graph Partitioning](https://arxiv.org/abs/2605.10985)
...
### [95] [AESOP: Adversarial Execution-path Selection to Overload Deep Learning Pipelines](https://arxiv.org/abs/2605.10987)
...
### [96] [Seeing the Needle in the Haystack: Towards Weakly-Supervised Log Instance Anomaly Localization via Counterfactual Perturbation](https://arxiv.org/abs/2605.10988)
...
### [97] [SURGE: Surrogate Gradient Adaptation in Binary Neural Networks](https://arxiv.org/abs/2605.10989)
...
### [99] [SkillGen: Verified Inference-Time Agent Skill Synthesis](https://arxiv.org/abs/2605.10999)
*Yuchen Ma,Yue Huang,Han Bao,Haomin Zhuang,Swadheen Shukla,Michel Galley,Xiangliang Zhang,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: SkillGen是一个多智能体框架，通过对比成功与失败轨迹的归纳学习，自动合成可审计的智能体技能，并以干预方式验证净效果，最终生成人类可读的高质量技能，无需重新训练即可提升智能体表现。


<details>
  <summary>Details</summary>
Motivation: 当前高层次智能体技能仍主要靠手工编写，成本高且无法规模化。需要自动化方法从基座智能体的执行轨迹中合成可复用的技能，同时保证技能的可审计性和可靠性。

Method: SkillGen利用多智能体协作，对基座智能体的成功与失败轨迹进行对比归纳，提取成功模式、复现失败模式及相邻成功中缺失的行为；生成候选技能后迭代优化，并将技能建模为干预变量，通过比较同一实例在有无技能下的结果，同时量化修复和退化效应来验证净增益。

Result: 在多种智能体和数据集上，SkillGen一致提升了留出测试集上的性能，超越了现有技能生成基线方法，且生成的技能可跨模型迁移。

Conclusion: SkillGen证明了通过对比归纳与干预验证自动生成可审计智能体技能的有效性，该框架能可靠地生产可复用的高水平技能，并推广至不同模型。
...
### [100] [Finite Volume-Informed Neural Network Framework for 2D Shallow Water Equations: Rugged Loss Landscapes and the Importance of Data Guidance](https://arxiv.org/abs/2605.11001)
*Xiaofeng Liu*
...
### [102] [RT-Transformer: The Transformer Block as a Spherical State Estimator](https://arxiv.org/abs/2605.11007)
*Peter Racioppo*
...
### [103] [When and How to Canonize: A Generalization Perspective](https://arxiv.org/abs/2605.11008)
*Yonatan Sverdlov,Benjamin Friedman,Snir Hordan,Nadav Dym*

Main category: cs.LG

TL;DR: 本文提出了一个理论框架，通过覆盖数界分析了群平均和规范化方法在实现不变性时的泛化误差，建立了严格泛化层级，并证明了希尔伯特曲线规范化在点云处理中的多项式增长覆盖数，为其实证成功提供了首个理论依据。


<details>
  <summary>Details</summary>
Motivation: 尽管群平均和规范化被用于非不变性骨干网络以实现不变性，但其理论泛化性质尚不清楚，需要建立分析框架来理解这些策略的泛化误差。

Method: 引入理论框架，通过覆盖数界来限制泛化误差，推导规范化模型、结构不变模型和群平均模型的误差界，并将框架应用于置换群和点云处理，分析不同规范化（如字典排序和希尔伯特曲线）的覆盖数增长。

Result: 建立了泛化层级：规范化模型的误差界最佳等于结构不变或群平均模型，最差等于非不变基线；存在最优规范化和差规范化，取决于规范化的正则性。对点云置换群，证明字典排序的覆盖数随维度指数增长，而希尔伯特曲线规范化保证多项式增长。

Conclusion: 理论框架揭示了规范化的泛化性质取决于其正则性，希尔伯特曲线在点云架构中的优势源于其多项式覆盖数增长，实验支持了理论论断。
...
### [104] [ACSAC: Adaptive Chunk Size Actor-Critic with Causal Transformer Q-Network](https://arxiv.org/abs/2605.11009)
*Qian Chen,Junqiao Zhao,Hongtu Zhou,Hang Yu,Yanping Zhao,Chen Ye,Guang Chen*

Main category: cs.LG

TL;DR: 本文提出自适应块大小演员-评论家（ACSAC）算法，通过因果Transformer评论家自适应选择动作块大小，平衡反应性与时间一致性，在长视距稀疏奖励操作任务中取得顶尖性能，无需任务特定调参。


<details>
  <summary>Details</summary>
Motivation: 长视距稀疏奖励任务中单步TD学习因自举误差累积而面临挑战，固定块大小的动作分块方法无法在反应性和一致性之间自适应平衡，导致任务特定调参难题。

Method: 提出ACSAC，利用因果Transformer评论家评估不同大小动作块的期望回报，在每个块边界自适应选择最大化期望回报的块大小，并证明其贝尔曼算子是压缩映射，不动点为自适应策略的动作价值函数。

Result: 在OGBench上实验表明，ACSAC在离线强化学习和离线到在线强化学习设置下，在长视距稀疏奖励操作任务上均达到最先进性能。

Conclusion: ACSAC通过动态选择块大小，消除了固定块大小的局限，支持状态依赖的灵活块大小，无需任务特定调参，理论保证和实验验证了其有效性。
...
### [105] [A Comparative Study of Federated Learning Aggregation Strategies under Homogeneous and Heterogeneous Data Distributions](https://arxiv.org/abs/2605.11010)
...
### [106] [LoopUS: Recasting Pretrained LLMs into Looped Latent Refinement Models](https://arxiv.org/abs/2605.11011)
...
### [107] [Backbone-Equated Diffusion OOD via Sparse Internal Snapshots](https://arxiv.org/abs/2605.11014)
...
### [108] [Simpson's Paradox in Behavioral Curves: How Aggregation Distorts Parametric Models of User Dynamics](https://arxiv.org/abs/2605.11017)
*Chao Zhou*
...
### [109] [Efficient LLM Reasoning via Variational Posterior Guidance with Efficiency Awareness](https://arxiv.org/abs/2605.11019)
*Zizhao Chen,Yuying Li,Siting Lin,Lianxi Wang*

Main category: cs.LG

TL;DR: 为解决大模型链式推理中的过度思考问题，本文提出变分策略梯度高效推理框架VPG-EA，通过后验分布引导和变分蒸馏将高效推理模式迁移到先验策略，实验显示在1.5B和7B模型上综合效率指标ε³分别提升8.73%和12.37%。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的推理链压缩方法因精心设计奖励函数导致高质量样本稀疏，在探索空间中形成先验策略的采样瓶颈，限制了推理效率的提升。

Method: 首先从理论上证明参考答案引导的后验分布具有更高的期望效用；然后将高效推理形式化为变分推断问题，提出效率感知的证据下界；在此基础上设计VPG-EA框架，采用参数共享的双流架构实例化后验分布和先验策略，通过跨视图评估过滤伪高效路径，并单向地利用变分蒸馏将后验的高效模式迁移到先验策略。

Result: 在DeepSeek-R1-Distill-Qwen的1.5B和7B规模上，VPG-EA相对于各模型规模的最强基线，将综合效率指标ε³分别提升8.73%和12.37%。

Conclusion: VPG-EA通过变分推断和双向蒸馏有效打破采样瓶颈，显著提高了链式推理的效率，为缓解大模型过度思考问题提供了新范式。
...
### [110] [Trust Region Inverse Reinforcement Learning: Explicit Dual Ascent using Local Policy Updates](https://arxiv.org/abs/2605.11020)
*Anish Diwan,Davide Tateo,Christopher E. Mower,Haitham Bou-Ammar,Jan Peters,Oleg Arenz*

Main category: cs.LG

TL;DR: 本文提出信任域逆强化学习（TRIRL），通过引入信任域策略优化，在无需每步完全求解RL问题的情况下实现奖励函数与策略的单调改进，弥合了经典与对抗逆强化学习方法的差距。


<details>
  <summary>Details</summary>
Motivation: 经典逆强化学习保证单调改进但每步需完整求解RL问题，计算成本高；对抗式方法避免该开销但缺乏单调性和稳定性。本文旨在结合两者优势：实现单调改进且无需昂贵RL求解。

Method: 理论洞察：对某奖励更新方向而言，信任域最优策略可成为同方向更小更新的全局最优策略。基于此，通过在当前策略附近局部搜索显式优化对偶目标，避免全RL求解。

Result: TRIRL在多个挑战性任务上的综合四分位均值表现超过现有模仿学习方法2.4倍，并能恢复对系统动态变化具有泛化性的奖励函数。

Conclusion: 该方法弥合了两类IRL方法的差距，在保持单调改进和训练稳定性的同时，能高效学习可全局优化以匹配专家示教的奖励函数。
...
### [111] [A Switching System Theory of Q-Learning with Linear Function Approximation](https://arxiv.org/abs/2605.11021)
...
### [112] [ASD-Bench: A Four-Axis Comprehensive Benchmark of AI Models for Autism Spectrum Disorder](https://arxiv.org/abs/2605.11091)
*Shubhankit Singh,Hassan Shaikh,Kuldeep Raghuwanshi,Keshav Bulia*

Main category: cs.LG

TL;DR: ASD-Bench 是一个系统化的表格数据基准，评估多种模型在儿童、青少年和成人三个年龄组上的自闭症筛查性能，揭示了不同年龄段的特征重要性和模型校准差异，并强调仅靠单一指标不足以评估临床AI。


<details>
  <summary>Details</summary>
Motivation: 现有自闭症筛查工具多局限于单一架构、单一评估维度，且主要面向成人，忽视了年龄特异的诊断模式，导致早期干预缺乏可靠依据。

Method: 构建 ASD-Bench 基准，使用 4068 条 AQ-10 记录，覆盖儿童（1-11岁）、青少年（12-16岁）和成人（17-64岁）三个队列；评估包括经典机器学习、深度学习、深度表格Transformer及 TabPFN v2 在内的 17 个模型；从预测性能、校准度、可解释性和对抗鲁棒性四个维度进行评测；提出代价敏感指标 HAP（Heuristic Aggregate Penalty），加大对假阴性的惩罚并融合交叉验证方差，强调部署稳定性。

Result: 成人分类任务多数模型取得完美 F1 和 AUC；青少年任务更困难（F1 上限 0.837，儿童为 0.915）；特征重要性随年龄转移：儿童以 A9（社会动机）为主，青少年以 A5（模式识别）为主，成人特征分布较平坦，符合社交掩蔽发展；准确率与校准度分离，如 AdaBoost 在成人上 F1=1.000 但 ECE=0.302；不同队列给出针对性部署建议。

Conclusion: 该基准证实问卷衍生标签下的概念验证结果，而非临床诊断性能；揭示了年龄特异的模型行为和评估维度分离，倡导多维度评估和年龄分层方法，为未来临床AI开发提供参考。
...
### [113] [Enabling Performant and Flexible Model-Internal Observability for LLM Inference](https://arxiv.org/abs/2605.11093)
...
### [114] [Newton's Lantern: A Reinforcement Learning Framework for Finetuning AC Power Flow Warm Start Models](https://arxiv.org/abs/2605.11102)
...
### [115] [GRAFT-ATHENA: Self-Improving Agentic Teams for Autonomous Discovery and Evolutionary Numerical Algorithms](https://arxiv.org/abs/2605.11117)
*Juan Diego Toscano,Zhaojie Chai,George Em Karniadakis*

Main category: cs.LG

TL;DR: GRAFT-ATHENA is a self-improving agentic AI framework that uses factored probabilistic trees to map decision spaces efficiently, learning from past problems to tackle complex scientific tasks across domains.


<details>
  <summary>Details</summary>
Motivation: Existing agentic AI systems treat each scientific problem in isolation, without a shared substrate to accumulate methodological experience across domains, limiting cross-domain learning and efficiency.

Method: The GRAFT-ATHENA framework projects combinatorial decision spaces into factored probabilistic trees (GRAFT), reducing parameter footprint from exponential to linear. Paths embed as fingerprints in a metric space, enabling learning from similar past problems. It uses LLM-driven planners, solvers, and evaluators for autonomous methodological expansion.

Result: On PIML benchmarks, GRAFT-ATHENA outperforms human and prior agentic baselines. It solves complex engineering problems like Mach-10 flow reconstruction over the Apollo Command Module and shear-thinning blood-cell rheology. It autonomously proposes regularization constraints and discovers new numerical methods (e.g., spectral PINN with exponential convergence).

Conclusion: The results demonstrate a foundation for autonomous laboratories that grow increasingly capable through accumulated problem-solving experience.
...
### [116] [Language Modeling with Hyperspherical Flows](https://arxiv.org/abs/2605.11125)
...
### [117] [HEPA: A Self-Supervised Horizon-Conditioned Event Predictive Architecture for Time Series](https://arxiv.org/abs/2605.11130)
...
### [118] [Steerable Neural ODEs on Homogeneous Spaces](https://arxiv.org/abs/2605.11133)
...
### [119] [Spurious Correlation Learning in Preference Optimization: Mechanisms, Consequences, and Mitigation via Tie Training](https://arxiv.org/abs/2605.11134)
*Christian Moya,Alex Semendinger,Guang Lin,Elliott Thornley*

Main category: cs.LG

TL;DR: Preference learning methods like DPO learn spurious correlations causing sycophancy and length bias. This paper theoretically dissects two spurious learning channels and proposes tie training with equal-utility pairs to selectively suppress spurious reliance without harming causal learning, validated on log-linear models and LLMs.


<details>
  <summary>Details</summary>
Motivation: Standard preference optimization induces reliance on spurious features, leading to sycophancy, length bias, and potential goal misgeneralization. Understanding and mitigating this spurious learning is critical for safe deployment.

Method: The authors provide a unified theoretical analysis for log-linear policies, identifying two spurious learning channels: mean spurious bias and causal–spurious correlation leakage. They then introduce tie training, a data augmentation that injects ties (equal-utility preference pairs) to act as a data-driven regularizer.

Result: Standard objectives create irreducible spurious feature dependence that persists with more in-distribution data, causing vulnerability to distribution shift. Tie training provably reduces spurious learning while preserving causal learning. Empirical results confirm these mechanisms and benefits extend to neural networks and large language models.

Conclusion: The paper formally characterizes spurious learning in preference learning and demonstrates that tie training is a principled mitigation strategy that works from log-linear models up to large-scale LLMs.
...
### [120] [Rank Is Not Capacity: Spectral Occupancy for Latent Graph Models](https://arxiv.org/abs/2605.11142)
...
TL;DR: Spectra treats model capacity as a learned, controllable property via the spectrum of a normalized kernel, replacing the brittle latent dimension hyperparameter.


<details>
  <summary>Details</summary>
Motivation: Latent dimension is a brittle hyperparameter that does not reflect true model behavior; embeddings are identifiable only up to rotation, so nominal rank rarely matches effective capacity. A principled way to control and analyze capacity is needed.

Method: Spectra uses the trace-normalized spectrum of a learned positive semidefinite kernel, then defines the Shannon effective rank as the summary of capacity. During training, a scalar targets a desired effective rank via bisection, and the method leverages local regularity and monotonicity of the rank profile.

Result: On collaboration, social, biological, and infrastructure networks, Spectra reveals accuracy–capacity trade-off frontiers, is competitive with strong link-prediction baselines, and provides lower-capacity views from the same model via spectral prefixes.

Conclusion: Capacity is transformed from a fixed training hyperparameter into an emergent, steerable property of the fitted model, enabling principled handling even in overparameterized regimes.
...
### [121] [CORE: Cyclic Orthotope Relation Embedding for Knowledge Graph Completion](https://arxiv.org/abs/2605.11159)
...
### [122] [Interpretability Can Be Actionable](https://arxiv.org/abs/2605.11161)
...
### [123] [COSMOS: Model-Agnostic Personalized Federated Learning with Clustered Server Models and Pseudo-Label-Only Communication](https://arxiv.org/abs/2605.11165)
...
### [124] [Unlearning with Asymmetric Sources: Improved Unlearning-Utility Trade-off with Public Data](https://arxiv.org/abs/2605.11170)
...
### [125] [Optimistic Dual Averaging Unifies Modern Optimizers](https://arxiv.org/abs/2605.11172)
...
### [126] [Oversmoothing as Representation Degeneracy in Neural Sheaf Diffusion](https://arxiv.org/abs/2605.11178)
...
### [127] [Muon is Not That Special: Random or Inverted Spectra Work Just as Well](https://arxiv.org/abs/2605.11181)
...
### [128] [CATS: Cascaded Adaptive Tree Speculation for Memory-Limited LLM Inference Acceleration](https://arxiv.org/abs/2605.11186)
...
### [129] [Deep Learning for Protein Complex Prediction and Design](https://arxiv.org/abs/2605.11189)
*Ziwei Xie*
...
### [130] [Variational Linear Attention: Stable Associative Memory for Long-Context Transformers](https://arxiv.org/abs/2605.11196)
...
### [131] [FeatMap: Understanding image manipulation in the feature space and its implications for feature space geometry](https://arxiv.org/abs/2605.11203)
...
### [132] [The Scaling Law of Evaluation Failure: Why Simple Averaging Collapses Under Data Sparsity and Item Difficulty Gaps, and How Item Response Theory Recovers Ground Truth Across Domains](https://arxiv.org/abs/2605.11205)
*Jung Min Kang*
...
### [133] [Measuring Five-Nines Reliability: Sample-Efficient LLM Evaluation in Saturated Benchmarks](https://arxiv.org/abs/2605.11209)
...
### [134] [Enforcing Constraints in Generative Sampling via Adaptive Correction Scheduling](https://arxiv.org/abs/2605.11214)
...
### [135] [Leveraging RAG for Training-Free Alignment of LLMs](https://arxiv.org/abs/2605.11217)
*John T. Halloran*

Main category: cs.LG

TL;DR: 论文提出了一种基于检索增强生成（RAG）的免训练对齐算法RAG-Pref，通过推理时利用偏好对比信息，显著提升大语言模型对智能体攻击的拒答能力，同时不增加大量计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的对齐算法需要巨大计算资源，且在应对新型智能体攻击时拒答能力不足，亟需一种能在不显著增加计算负担下强化拒答护栏的方法。

Method: 提出RAG-Pref，一种简单的基于检索增强生成的对齐算法，在推理时以偏好和不偏好样本为条件，利用对比信息进行决策；该算法是在线的（无需训练），可与现有离线对齐方法结合。

Result: 结合离线对齐后，RAG-Pref在五个主流LLM上使智能体攻击拒答率平均提升3.7倍，优于其他在线对齐方法的2.9倍和纯离线方法的1.5倍；同时，在通用人类偏好任务上也保持性能提升，且计算需求未大幅增加。

Conclusion: RAG-Pref作为一种高效的在线对齐方法，能有效提升拒答性能与通用对齐表现，且计算友好，为对齐技术提供了新的简单有效的方案。
...
### [136] [ADMM-Q: An Improved Hessian-based Weight Quantizer for Post-Training Quantization of Large Language Models](https://arxiv.org/abs/2605.11222)
...
### [137] [LiBaGS: Lightweight Boundary Gap Synthesis for Targeted Synthetic Data Selection](https://arxiv.org/abs/2605.11231)
*Abhishek Moturu,Anna Goldenberg,Babak Taati*

Main category: cs.LG

TL;DR: LiBaGS is a generator-agnostic method that selects synthetic training samples by scoring them on decision-boundary proximity, uncertainty, density, and support validity, allocates them to sparse boundary regions, stops when marginal value is low, uses soft labels near boundaries, and avoids redundancy—improving accuracy over existing selection and oversampling techniques.


<details>
  <summary>Details</summary>
Motivation: Synthetic data is beneficial only when it fills informative gaps in the real training distribution. Existing methods often add indiscriminately or select only the most uncertain samples, ignoring whether gaps are realistic and missing a principled stopping criterion.

Method: LiBaGS scores each candidate synthetic sample using a combination of decision-boundary proximity, predictive uncertainty, local real-data density, and support vector validity. A boundary-gap allocation rule focuses on sparse yet realistic decision-boundary neighborhoods. The method learns when to stop via a marginal-value rule, assigns softer labels near ambiguous boundaries, and applies a diversity objective to eliminate near-duplicate selections.

Result: LiBaGS improved classification accuracy over classical oversampling, hard data augmentation, ablations using only uncertainty or density, and targeted-generation selection criteria in experiments.

Conclusion: LiBaGS effectively guides synthetic data insertion by prioritizing informative, realistic, and non-redundant samples at decision boundary gaps, with a learned stopping point, leading to consistent accuracy gains without dependence on a specific generator architecture.
...
### [138] [A Comparative Study of Model Selection Criteria for Symbolic Regression](https://arxiv.org/abs/2605.11233)
...
### [139] [Internalizing Curriculum Judgment for LLM Reinforcement Fine-Tuning](https://arxiv.org/abs/2605.11235)
*Han Zheng,Yining Ma,Karthick Gunasekaran,Bharathan Balaji,Zheng Du,Shiv Vitaladevuni,Cathy Wu*

Main category: cs.LG

TL;DR: METIS 是一种将课程判断内化为 LLM 强化微调本机能力的框架，通过预测提示信息量并动态调整训练，实现闭环元认知，加速收敛并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有 RFT 课程学习方法依赖手工启发式或辅助模型，可能误导训练策略，而 METIS 旨在将课程判断内化，使其与策略训练动态对齐。

Method: METIS 利用提示内奖励方差衡量提示信息量，基于近期训练结果作为上下文示例预测该指标，动态分配训练资源，并联合优化标准 RFT 奖励与自我判断奖励，形成闭环元认知。

Result: 在数学推理、代码生成、智能函数调用等离散与连续 RFT 基准上，METIS 性能优越，收敛速度提升最高达 67%。

Conclusion: METIS 建立了一种简单、闭环、高效的内化课程学习范式，无需手工启发式或辅助模型，为 LLM 强化微调提供了新方向。
...
### [140] [DeconDTN-Toolkit: A Library for Evaluation and Enhancement of Robustness to Provenance Shift](https://arxiv.org/abs/2605.11237)
*Yongsen Tan,Zhecheng Sheng,Xiruo Ding,Serguei V. S. Pakhomov,Trevor Cohen*

Main category: cs.LG

TL;DR: 本文建立起源偏移、反事实不变性与不变学习之间的形式化联系，提出稳健学习目标，并推出DeconDTN工具包来模拟和评估起源偏移，揭示经验风险最小化的脆弱性，提供理论与工具以应对混淆问题。


<details>
  <summary>Details</summary>
Motivation: 尽管分布偏移研究众多，但起源偏移（部署时数据源与标签关系发生变化）仍被忽视且理解不足，亟需填补这一空白。

Method: 形式化连接起源偏移、反事实不变性与不变学习，推导稳健学习目标；设计DeconDTN-Toolkit模拟不同强度的起源偏移，保持训练协议和基准基础设施；评估现有算法，揭示经验风险最小化的漏洞，并提出稳健的分布外性能指标。

Result: 经验风险最小化在起源偏移下表现脆弱；所提出的稳健指标能有效表征性能；工具箱可模拟偏移并评估缓解方法。

Conclusion: 本文为起源混淆问题提供了理论基础和实用工具，包括表征与缓解方法，推动了对起源偏移的稳健学习。
...
### [141] [Extending Kernel Trick to Influence Functions](https://arxiv.org/abs/2605.11239)
...
### [142] [Support-Proximity Augmented Diffusion Estimation for Offline Black-Box Optimization](https://arxiv.org/abs/2605.11246)
*Yonghan Yang,Ye Yuan,Zipeng Sun,Linfeng Du,Bowei He,Haolun Wu,Can Chen,Xue Liu*

Main category: cs.LG

TL;DR: 本文提出SPADE框架，利用扩散模型结合校准估计和支持邻近正则化，解决离线黑盒优化中的分布外推问题，在多个基准上达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 离线黑盒优化需从静态数据集中发现高分设计，但面临分布外推挑战。现有逆向方法映射不适定，前向方法缺乏量化不确定性的分布表达力，因此需要更优的前向建模。

Method: 采用条件扩散模型建模前向似然 p(y|x)，并引入两个关键增强：校准扩散估计模块强制统计矩和成对排序的全局一致性；支持邻近正则化通过 kNN 密度估计隐式内化数据流形约束 p(x)。理论上证明该正则化等价于最大化具有有效设计先验的贝叶斯后验。

Result: 在 Design-Bench 任务和大型语言模型数据混合优化基准上，SPADE 取得了最先进性能。

Conclusion: SPADE 通过校准扩散与流形正则化有效缓解了离线优化的外推问题，为黑盒优化提供了一种强健的前向建模方案。
...
### [143] [A Proof-of-Concept Simulation-Driven Digital Twin Framework for Decision-Aware Diabetes Modeling](https://arxiv.org/abs/2605.11247)
*Zarrin Monirzadeh*
...
### [144] [Curriculum Learning-Guided Progressive Distillation in Large Language Models](https://arxiv.org/abs/2605.11260)
...
### [145] [Latent Chain-of-Thought Improves Structured-Data Transformers](https://arxiv.org/abs/2605.11262)
...
### [146] [Localization Boosting for Growth Markets: Mitigating Cross-Locale Behavioral Bias in Learning-to-Rank](https://arxiv.org/abs/2605.11272)
...
### [147] [Beyond Similarity: Temporal Operator Attention for Time Series Analysis](https://arxiv.org/abs/2605.11287)
...
### [148] [Quotient-Categorical Representations for Bellman-Compatible Average-Reward Distributional Reinforcement Learning](https://arxiv.org/abs/2605.11289)
*Ege C. Kaya,Aliasghar Pourghani,Vijay Gupta,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 本文针对平均奖励强化学习中偏差的平移模糊性，提出商空间分布框架，定义商范畴参数化与投影算子，证明其非扩张性与不动点存在，并给出采样算法的收敛性结论，揭示理想商分布与固定网格表示的结构差异。


<details>
  <summary>Details</summary>
Motivation: 平均奖励强化学习需估计增益和偏差，而偏差仅定义到加性常数，导致直接分布建模在实数轴上不适定，亟待消除平移自由度以建立适定的分布方法。

Method: 引入商空间表述，将状态索引的偏差律等同到共同平移，采用尊重该对称性的范畴参数化。在商范畴空间上定义投影平均奖励分布算子，证明其良定性、坐标Cramér度量下的非扩张性及不动点存在。进而研究基于该算子异步松弛的采样递归，并在增益未知时增补在线增益估计器，形成耦合方案。

Result: 在理想化中心奖励设定下，单状态时序差分更新几乎必然收敛，且在i.i.d.和马尔可夫采样下满足有限迭代残差界；耦合方案保持非扩张性且在马尔可夫采样下收敛；同步精确更新在商律层面增益无关，突显理想商分布与固定网格范畴表示的结构性差异。

Conclusion: 所提商空间分布方法有效解决了平均奖励分布强化学习因偏差平移引起的适定性问题，理论收敛性得到保证，并揭示了理想商分布与实用离散表示之间的本质差别。
...
### [149] [Optimal Representations for Generalized Contrastive Learning with Imbalanced Datasets](https://arxiv.org/abs/2605.11291)
...
### [150] [Primal Generation, Dual Judgment: Self-Training from Test-Time Scaling](https://arxiv.org/abs/2605.11299)
*Yizhu Jiao,Ruixiang Zhang,Richard Bai,Jiawei Han,Ronan Collobert,Yizhe Zhang*

Main category: cs.LG

TL;DR: 本文提出DuST（Dual Self-Training）框架，将多个候选程序的相对正确性结构作为训练信号，通过纯判别式目标同时提升代码判断与生成能力，单次生成即可匹配基模型的Best-of-4性能。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成训练仅在原始程序空间利用稀疏执行反馈（通过/失败），测试时缩放虽采样多个候选并比较，但其揭示的相对正确性信息在推理后即被丢弃。该信息构成一个双重判断空间，能提供更丰富的训练信号，但未被有效利用。

Method: DuST框架从模型自身分布采样候选程序，经沙盒执行标注后，保留同时包含成功与失败的候选组，采用GRPO训练模型按执行正确性对候选排序。训练目标为纯判别式：模型不因生成正确程序直接获得奖励，仅学习区分优劣。

Result: 在涵盖两种模型家族、三个规模（4B至30B）的五款模型上，DuST在LiveCodeBench上持续提升Best-of-4测试时缩放性能。以Qwen3-30B-Thinking在LiveCodeBench v6为例：判断质量NDCG提升6.2，单样本pass@1提升3.1，Best-of-4准确率提升4.1，且训练后单次生成可达到基模型Best-of-4的水平。

Conclusion: 同样排序数据上的SFT仅提升判断而不提升生成，证明在策略RL是将双重空间学习迁移回原始生成空间的关键机制，DuST展示了从判断信号进行自我训练的有效性。
...
### [151] [A Theory of Time-Sensitive Language Generation: Sparse Hallucination Beats Mode Collapse](https://arxiv.org/abs/2605.11302)
...
### [152] [Couple to Control: Joint Initial Noise Design in Diffusion Models](https://arxiv.org/abs/2605.11311)
*Jing Jia,Liyue Shen,Guanyang Wang*

Main category: cs.LG

TL;DR: 提出一种耦合初始噪声的方法，以替代独立高斯噪声，在保持边缘标准高斯的前提下设计多样本间的依赖关系，从而提升扩散模型生成的多样性和质量，无需增加采样成本，并在背景生成任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 扩散模型通常使用独立的高斯噪声初始化，但这种独立性假设并非唯一选择。通过设计初始噪声之间的耦合结构，可以在不改变单样本输入分布的同时，控制多样本间的依赖，从而实现更优的生成控制与质量提升。

Method: 将初始噪声设计问题转化为构建多个样本标准高斯边际的联合分布（耦合），每个噪声边缘仍为独立标准高斯，以保证预训练模型接收相同的单样本输入分布。提出通用耦合噪声框架，涵盖现有方法并引出新的耦合构造（如排斥性高斯耦合和子空间耦合），可单独用于生成或作为后续优化管道的结构化初始化。

Result: 在SD1.5、SDXL和SD3上，排斥性高斯耦合显著提升画廊多样性，同时保持提示对齐和图像质量；在相同采样成本下，与最新的测试时噪声优化基线相比，多项多样性指标相当或更优。子空间耦合实现固定对象的背景生成，产生多样且自然的背景，与专用修补基线相比前景保真度可调。

Conclusion: 耦合噪声设计为扩散模型的初始噪声控制提供了新框架，能有效改善生成多样性和质量，并在背景生成等应用中展示出优越性，且未增加额外计算成本，具有广泛的应用潜力。
...
### [153] [Error whitening: Why Gauss-Newton outperforms Newton](https://arxiv.org/abs/2605.11316)
...
### [154] [$\varepsilon$-Good Action Identification in Fixed-Budget Monte Carlo Tree Search](https://arxiv.org/abs/2605.11324)
...
### [155] [Neural Statistical Functions](https://arxiv.org/abs/2605.11327)
...
### [156] [Epistemic Uncertainty for Test-Time Discovery](https://arxiv.org/abs/2605.11328)
*Kainat Riaz,Muhammad Ahmed Mohsin,Ahsan Bilal,Muhammad Umer,Ayesha Mohsin,Aqib Riaz,Ali Subhan,John M. Cioffi*

Main category: cs.LG

TL;DR: UG-TTT利用低秩适配器集成的分歧作为探索奖励，引导强化学习在科学发现中探索新颖方案，提高最大奖励并保持多样性。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习在科学发现中惩罚高方差突变，导致策略偏向熟悉模式，最大奖励趋于平台。需要区分未探索区域和固有难题的信号。

Method: 在冻结基模型上维护低秩适配器集成，通过预测与权重假设间的互信息量化逐令牌分歧，分离认知不确定性作为探索奖励加入策略梯度，并使用核范数正则化保持适配器差异性。

Result: 在四个科学发现基准上，UG-TTT提升了三个任务的最大奖励，保持了更高的解决方案多样性，消融实验验证正则化器对维持此行为至关重要。

Conclusion: UG-TTT通过集成分歧有效引导探索，提升最大奖励和多样性，正则化器是保持探索信号的关键。
...
### [157] [VERDI: Single-Call Confidence Estimation for Verification-Based LLM Judges via Decomposed Inference](https://arxiv.org/abs/2605.11334)
...
### [158] [Physics-Informed Teacher-Student Ensemble Learning for Traffic State Estimation with a Varying Speed Limit Scenario](https://arxiv.org/abs/2605.11346)
...
### [159] [Gradient-Free Noise Optimization for Reward Alignment in Generative Models](https://arxiv.org/abs/2605.11347)
...
### [160] [gym-invmgmt: An Open Benchmarking Framework for Inventory Management Methods](https://arxiv.org/abs/2605.11355)
*Reza Barati,Qinmin Vivian Hu*

Main category: cs.LG

TL;DR: 本文推出 gym-invmgmt 基准，用于在统一的评估合约下比较优化、启发式和学习的库存控制器，揭示方法排名随场景条件变化，性能依赖于信息访问、需求漂移、拓扑结构和策略表示。


<details>
  <summary>Details</summary>
Motivation: 库存策略对比难以解读，因为评估合约的差异（如拓扑、需求模式、信息访问、约束、短缺处理、KPI定义）会改变方法排名，因此需要一个可审计的跨范式评估基准。

Method: 提出 gym-invmgmt，一个兼容 Gymnasium 并继承 OR-Gym 的基准，采用共享的核心环境（CoreEnv）定义转移、奖励、动作边界和 KPI 合约，设计 22 个核心场景与 4 个多智能体强化学习（MARL）补充场景。评估涵盖优化（含信息随机规划）、启发式和多种学习控制器（PPO-Transformer、Residual RL、PPO-GNN、模仿学习、受约束LLM基线）。

Result: 信息随机规划提供最强的非神谕参考，但在线计算成本高；PPO-Transformer 以快速推理获得最强学习策略质量；Residual RL 提供有竞争力的混合性能；PPO-GNN 在默认发散拓扑上强但在串联拓扑上鲁棒性差；模仿学习在平稳需求下良好，需求漂移时退化；受约束的大语言模型（LLM）基线更适合诊断而非自主优化。整体上，性能依赖于信息访问、需求漂移、拓扑和策略表示的组合。

Conclusion: 基准能够识别场景条件下的领先方法，并揭示库存控制性能是信息、需求漂移、拓扑和策略表示的联合函数。
...
### [161] [The tractability landscape of diffusion alignment: regularization, rewards, and computational primitives](https://arxiv.org/abs/2605.11361)
...
### [162] [Causal Fairness for Survival Analysis](https://arxiv.org/abs/2605.11362)
*Drago Plecko*
...
### [163] [LPDP: Inference-Time Reward Control for Variable-Length DNA Generation with Edit Flows](https://arxiv.org/abs/2605.11368)
...
### [164] [Test-Time Compute for Dense Retrieval: Agentic Program Generation with Frozen Embedding Models](https://arxiv.org/abs/2605.11374)
*Han Xiao*

Main category: cs.LG

TL;DR: 测试时计算不仅有助于大型推理模型，也能通过一种简单的无参数代数运算（软最大加权质心与查询插值）显著提升冻结的小型嵌入模型的检索性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型蒸馏出的嵌入模型继承了其表示空间，推测冻结的嵌入模型无需重新训练即可从额外的推理计算中获益，但此前测试时计算被认为仅对大型推理模型有效。该工作旨在验证并利用嵌入模型的这一潜力。

Method: 采用智能体程序搜索循环，在九十代中探索了冻结嵌入API上的259种候选推理程序，发现整个帕累托前沿收敛于一种统一的代数操作：对局部top-K文档的软最大加权质心与查询向量进行插值。该操作无需任何参数。

Result: 在七个参数量级跨越十倍的嵌入模型家族上，该无参数默认操作在nDCG@10指标上实现了统计显著的提升，并在留出的完整BEIR验证集上对所有测试模型均确认了提升效果。

Conclusion: 测试时推理计算可以普遍地提升冻结嵌入模型的检索质量，其中发现的简单插值方法是一种无需训练、广泛适用的性能增强手段。
...
### [165] [TRACE: Temporal Routing with Autoregressive Cross-channel Experts for EEG Representation Learning](https://arxiv.org/abs/2605.11380)
*Fan Ma,Qier An,Peng Chen,Lingfei Qian,Xiang Lan,Mingyang Jiang,Zhiling Gu,Xenophon Papademetris,Hua Xu*

Main category: cs.LG

TL;DR: TRACE是一种自回归脑电图预训练框架，利用跨通道时间路由学习可迁移表示，在多个下游任务中达到最佳性能。


<details>
  <summary>Details</summary>
Motivation: 脑电图信号多通道、非平稳，通道间存在耦合且时间动态随情境变化，现有架构（时间均匀计算或独立路由每个通道）无法良好匹配该结构，导致可迁移表示学习困难。

Method: 提出TRACE：自回归预训练，从因果上下文预测未来脑电图片段，采用时间自适应和跨通道协同计算。每个时间步基于因果跨通道历史导出专家路由决策，并联合应用于所有通道，保留瞬时跨通道相干性，同时允许不同时间区间激活不同计算。路由定义在可用通道和因果时间上下文上，兼容不同通道数、导联方式、序列长度和记录域的异构预训练。

Result: 在八个下游脑电图基准上，TRACE在多个任务上取得最佳结果，在运动想象和临床事件分类上保持竞争力；消融实验证实跨通道时间路由的重要性。评估设置包括下游领域作为未标记预训练数据可见及完全未见两种。

Conclusion: TRACE通过跨通道时间路由有效捕获脑电图的多通道相干性和非平稳动态，学习到强可迁移表示，其路由机制是性能关键。
...
### [166] [Behavioral Mode Discovery for Fine-tuning Multimodal Generative Policies](https://arxiv.org/abs/2605.11387)
*Alberta Longhini,David Emukpere,Jean-Michel Renders,Seungsu Kim*

Main category: cs.LG

TL;DR: 本文提出一种无监督模式发现框架，在强化学习微调生成式策略时保持动作分布的多模态性，避免行为坍缩，同时提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有RL微调方法虽能提升任务性能，但会导致生成式策略（如扩散策略）的多样化行为消失，坍缩为单一收益最大化模式，因此需要一种能够保留行为多模态性的微调方法。

Method: 提出无监督模式发现框架，从预训练生成式策略中挖掘潜在行为模式，并利用互信息作为内在奖励，在RL微调中正则化，以平衡任务成功和行为多样性。

Result: 在机器人操作任务上，该方法在成功率和动作分布多模态性方面均优于传统微调方法。

Conclusion: 通过模式发现和互信息正则化，能够有效解决RL微调中的行为坍缩问题，保留生成式策略的多模态特性并取得更高任务性能。
...
### [167] [MuonQ: Enhancing Low-Bit Muon Quantization via Directional Fidelity Optimization](https://arxiv.org/abs/2605.11396)
...
### [168] [More Than Meets the Eye: A Semantics-Aware Traffic Augmentation Framework for Generalizable Website Fingerprinting](https://arxiv.org/abs/2605.11402)
...
### [169] [fg-expo: Frontier-guided exploration-prioritized policy optimization via adaptive kl and gaussian curriculum](https://arxiv.org/abs/2605.11403)
*Mingxiong Lin,Zhangquan Gong,Maowen Tang,Qian Li,Chuangchuang Wang,Jian Ma,Sutian Huang,Kai Tang,Haonan Lu*

Main category: cs.LG

TL;DR: FG-ExPO improves GRPO for LLM mathematical reasoning by dynamically adjusting the KL penalty based on accuracy and prioritizing moderately difficult questions via Gaussian sampling, leading to significant performance gains.


<details>
  <summary>Details</summary>
Motivation: GRPO has two inefficiencies: a fixed KL coefficient overly restricts policy exploration when divergence from the reference is needed, and uniform question sampling fails to focus on moderately difficult problems that yield the most informative gradient signals.

Method: FG-ExPO integrates Accuracy-Conditioned KL Scaling (AKL), which adjusts KL penalty strength via a smooth nonlinear function of batch average accuracy, and Gaussian Curriculum Sampling (GCS), which assigns sampling weights to questions following a Gaussian distribution centered around accuracy 0.5.

Result: On DeepSeek-R1-Distill-Qwen-1.5B and Qwen3-8B-Base across six math benchmarks, FG-ExPO outperforms vanilla GRPO, with an absolute improvement of 13.34 on AIME 2025 pass@32 (63.33% to 76.67%) and an average pass@32 gain of 2.66 on the 8B model; larger gains on pass@32 than pass@1 confirm an enlarged effective exploration space.

Conclusion: FG-ExPO effectively addresses GRPO's limitations, enhancing policy exploration and improving mathematical reasoning performance, particularly under fixed inference budgets.
...
### [170] [20/20 Vision Language Models: A Prescription for Better VLMs through Data Curation Alone](https://arxiv.org/abs/2605.11405)
...
TL;DR: 通过仅改变训练数据（固定架构、训练方案和计算量），数据筛选使视觉语言模型（VLM）性能平均提升 +11.7pp，以极低训练计算量逼近前沿精度，并带来可靠性、分布外泛化、行为质量等多重增益。


<details>
  <summary>Details</summary>
Motivation: 数据筛选在语言模型和图文对比预训练中已改变质量-计算边界，但在 VLM 中作用远未明确；本文探究在不改变架构、训练方案和计算量的情况下，仅靠数据筛选能将 VLM 推到多远。

Method: 将数据筛选管线应用于 MAmmoTH-VL 单图像子集，通过固定架构和训练配方，仅改变训练数据，在 20 个公开 VLM 基准和 DatBench 高保真评测套件上评估。

Result: 在 20 个基准上平均提升 +11.7pp，在 DatBench 九大能力轴上平均 +11.3pp。2B 规模筛选模型以约 1/17 训练计算量超越 InternVL3.5-2B 9.9pp，并以约 1/87 计算量将差距缩至 Qwen3-VL-2B 的 1.8pp 以内。额外收获：可靠性（各能力跨种子的标准差下降约 67%，提升在 4k-16k 上下文长度下保持）；分布外泛化（9 项 OOD 平均 +7.2pp，仅单图像训练下多图像 BLINK +3.09pp，视觉对应 +11.8pp）；行为改进（在 1100 个开放查询中，筛选模型更诚实、更具体，比同等计算基线更简洁、拒绝更少）；推理开销帕累托占优（所有规模下提升精度同时降低 response FLOPs，筛选 4B 以 3.3 倍低 FLOPs 匹配前沿精度）。

Conclusion: 数据筛选是构建更好 VLM 的高杠杆工具，以最高约 150 倍较少的训练计算量达到近前沿精度，并在精度之外全面提升可靠性、泛化性和行为表现。
...
### [171] [A Boundary-Aware Non-parametric Granular-Ball Classifier Based on Minimum Description Length](https://arxiv.org/abs/2605.11406)
...
### [172] [MaskTab: Scalable Masked Tabular Pretraining with Scaling Laws and Distillation for Industrial Classification](https://arxiv.org/abs/2605.11408)
...
### [173] [Generative Diffusion Prior Distillation for Long-Context Knowledge Transfer](https://arxiv.org/abs/2605.11414)
...
### [174] [FastUMAP: Scalable Dimensionality Reduction via Bipartite Landmark Sampling](https://arxiv.org/abs/2605.11428)
*Hongmin Li*
...
### [175] [Deep Minds and Shallow Probes](https://arxiv.org/abs/2605.11448)
*Su Hyeong Lee,Risi Kondor*

Main category: cs.LG

TL;DR: 神经表征并非唯一，探针应对表征对称性保持稳定；本文提出一种浅层坐标稳定探针的层次结构，并通过商空间实现跨模型探针迁移。


<details>
  <summary>Details</summary>
Motivation: 神经表征不是唯一的对象，即使在下游计算相同的情况下，隐藏坐标也可能因重参数化而不同。因此，用于揭示表征结构的探针家族应在相关表征对称性下保持稳定，而不应依赖于特定基底。

Method: 在最终读出层的可解析精确设定下，研究等价的实现如何引起隐藏坐标的仿射变换群作用。由此对称性原理导出一个唯一的浅层坐标稳定探针层次结构，其中线性探针为1阶成员。进一步提出跨模型探针迁移的自然对象是共享的探针可见商空间（即模掉探针家族不可见方向的表征）。在合成和真实任务上进行实验验证。

Result: 实验表明，2阶探针能在线性探针之外提供额外帮助；基于商空间的迁移能够实现跨模型家族的覆盖感知监控可移植性。

Conclusion: 这些结果指向一个更广泛的神经探测几何表征理论，并以覆盖感知的监控器迁移作为具体的操作后果。
...
### [176] [Beyond Prediction: Interval Neural Networks for Uncertainty-Aware System Identification](https://arxiv.org/abs/2605.11460)
*Mehmet Ali Ferah,Tufan Kumbasar*

Main category: cs.LG

TL;DR: 本文提出区间神经网络（INN）框架进行不确定性感知的系统辨识，构建区间LSTM和NODE模型，并设计级联与联合两种训练策略，实验表明级联INN点预测更优，联合INN预测区间更准且校准更好，同时引入通道弹性分析不确定性模式。


<details>
  <summary>Details</summary>
Motivation: 传统系统辨识难以捕捉非线性动态，深度学习虽能建模但缺乏不确定性量化，容易导致不可靠预测，因此需建立将深度学习与不确定性感知相结合的系统性框架。

Method: 扩展清晰神经网络为区间版本，构建区间LSTM与NODE模型，通过区间算术无概率假设地传播不确定性并产生预测区间。提出两种训练策略：级联INN（C-INN）先训练清晰网络再转换为区间网络的两阶段法；联合INN（J-INN）同时优化点预测精度和区间精度的单阶段法。训练中采用不确定性感知损失函数和参数化技术。

Result: 在多个系统辨识数据集上的实验表明，两种策略均有效：C-INN获得更优的点预测精度，J-INN产生更准确且校准更好的预测区间。引入通道弹性概念发现，两种训练策略在模型参数上的不确定性表现存在不同模式。

Conclusion: 所提框架成功将深度学习与不确定性感知建模结合，为系统辨识提供了可靠的不确定性量化手段，验证了区间神经网络在该领域的有效性。
...
TL;DR: 本文针对自我蒸馏在数学推理中表现不一致的问题，提出反自我蒸馏（AntiSD），通过反转散度方向并引入熵门控机制，大幅提升训练效率和最终推理准确率。


<details>
  <summary>Details</summary>
Motivation: 自我蒸馏利用特权上下文（如验证解）虽有望增强推理能力，但在数学推理中因特权上下文使教师模型对结构性连接词和可验证断言过度自信，而对驱动多步搜索的审慎性词（如“Wait”、“Let”）低估，导致增益不稳定。

Method: 提出AntiSD，不再最小化学生与教师之间的散度，而是最大化该散度，颠倒逐令牌符号以自然获得有界优势。同时引入熵触发门控，当教师熵崩溃时自动禁用该项，直接替代标准自我蒸馏。

Result: 在4B到30B参数规模的五个模型上，AntiSD在数学推理基准中用少2到10倍的训练步数达到GRPO基线的准确率，并将最终准确率最高提升11.5个百分点。

Conclusion: AntiSD为语言模型利用自身训练信号实现推理能力的自我引导和可扩展自我改进提供了新路径。
...
### [178] [Drop the Act: Probe-Filtered RL for Faithful Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.11467)
*Swapnil Parekh*

Main category: cs.LG

TL;DR: ProFIL uses a probe trained on a frozen model to detect post-commitment reasoning steps, and during GRPO zeros out advantages for rollouts with high probe scores, reducing reasoning theater, increasing faithfulness, and shortening chain length without harming accuracy.


<details>
  <summary>Details</summary>
Motivation: Reasoning models produce 'reasoning theater' — deliberative-looking steps that do not contribute to correctness after the model has internally committed to an answer. This wastes inference tokens, harms interpretability, and obscures the model's actual computation.

Method: ProFIL (Probe-Filtered Reinforcement Learning) extends Group Relative Policy Optimization (GRPO). A multi-head attention probe is trained once on a frozen base model to detect post-commitment steps using verifier-derived labels and no human annotation. During GRPO, rollouts whose probe score exceeds a threshold have their advantage zeroed, thereby suppressing theater.

Result: Across four reasoning domains (GSM8K, LiveCodeBench, ToolUse, MMLU-Redux) and two architectures (Llama-8B, Qwen-7B), ProFIL reduces post-commitment theater by 11–100%, raises faithful-fraction (e.g., +24pp on LiveCodeBench under a Claude 3.7 Sonnet judge), and shortens chains by 4–19%, all while preserving or improving task accuracy. It outperforms a length-penalty baseline, showing the gain stems from semantic detection rather than mere compression.

Conclusion: A probe trained on a frozen base model with verifier-derived labels provides a stable signal that suppresses reasoning theater, increases chain-of-thought faithfulness, and resists the RL-obfuscation failure mode predicted by prior work. ProFIL is an effective drop-in extension to GRPO.
...
### [180] [Robust Multi-Agent Path Finding under Observation Attacks: A Principled Adversarial-Plus-Smoothing Training Recipe](https://arxiv.org/abs/2605.11469)
*Riad Ahmed*

Main category: cs.LG

TL;DR: 本文提出了两种训练方法（Adv-PPO 和 Adv-PPO+MACER），在保持网络与部署流程不变的前提下，提升去中心化多智能体路径规划策略对输入扰动的鲁棒性，其中 Adv-PPO+MACER 在最差攻击下成功率达 77.5%，且几乎不损失干净性能。


<details>
  <summary>Details</summary>
Motivation: 标准 PPO 训练的共享策略在干净观测下表现良好，但微小输入扰动就能改变单个智能体动作，引发连锁阻塞导致团队卡死，因此需要增强策略对观测扰动的鲁棒性。

Method: Adv-PPO 在训练中对抗性地扰动自身输入，并根据对抗扰动下的性能选择检查点；Adv-PPO+MACER 在该检查点基础上，微调引入一个基于随机平滑认证半径的 on-policy 平滑项，沿梯度方向提升动作稳定性的可认证界。

Result: 在 POGEMA 8×8 地图四个智能体任务上，无防护 PPO 干净成功率为 95.8%，最强攻击下仅 2.5%；Adv-PPO 以约 1 个百分点的干净代价将最差成功率恢复至 59.2%；Adv-PPO+MACER 在三次独立运行中以不到 1 个百分点的干净代价，将最差成功率恢复至 77.5% ± 6.0%。

Conclusion: 两种训练配方均有效提升了策略对观测扰动的鲁棒性，而结合认证平滑的 Adv-PPO+MACER 在保持高干净性能的同时，取得了显著更高的最差攻击成功率，并通过逐攻击曲线、认证动作稳定性检查和轨迹故事板揭示了失效模式与修正效果。
...
### [181] [On the Approximation Complexity of Matrix Product Operator Born Machines](https://arxiv.org/abs/2605.11471)
...
TL;DR: 本文提出熵力学理论框架与极性感知策略优化（PAPO），在RLVR中从token级精确控制策略熵，有效提升大语言模型推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有熵感知方法仅通过全局目标调节熵，忽略了采样策略更新在token级别如何重塑熵的微观机制。

Method: 构建RLVR的熵力学理论框架，推导熵变化的一阶近似，定义“熵极性”（有符号的token级量，预测单次更新的熵扩张或收缩），并揭示不对称性：强化高频高概率token引起收缩，扩张通常需低概率样本或更强的分布校正。进而提出PAPO，保留正负极性分支，通过优势重加权实现熵控制，并根据在线经验熵轨迹自适应分配优化压力。

Result: 在数学推理与智能体基准上，PAPO持续优于竞争基线，训练效率更高，奖励提升显著。

Conclusion: Token级熵分析揭示了探索与利用的精细平衡机制，PAPO据此实现了更高效的策略优化，证明了微观熵控制对LLM强化学习的实用价值。
...
### [183] [Efficient Adjoint Matching for Fine-tuning Diffusion Models](https://arxiv.org/abs/2605.11480)
...
### [185] [Adaptive Calibration in Non-Stationary Environments](https://arxiv.org/abs/2605.11490)
...
TL;DR: 针对GRPO等RLVR算法中的熵坍缩问题，本文从token级熵流角度揭示熵减token占优导致失衡，提出在线策略熵流优化(OPEFO)自适应重置更新，提升训练稳定性和数学推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR算法（如GRPO）常出现熵坍缩，导致过早确定性和不稳定优化。现有补救措施（熵正则化、比例裁剪）要么控制粗糙，要么依赖近似在线策略，难以从根本上平衡熵动态。

Method: 从token级熵流视角分析，发现熵减token数量持续压倒熵增token，造成严重失衡。基于此，提出在线策略熵流优化(OPEFO)，根据每个token对熵变化的贡献，自适应地重新缩放熵增和熵减更新，且严格保持在线策略。

Result: 在六个数学推理基准上的实验表明，OPEFO提高了训练稳定性和最终性能。

Conclusion: 熵流视角为RLVR中的熵坍缩提供了统一解释，OPEFO通过平衡熵动态有效缓解坍缩，增强了训练稳定性与模型表现。
...
TL;DR: GEAR通过自蒸馏获得的令牌级和分段级发散信号，自适应地重新加权轨迹级GRPO优势，以改进长时程LLM智能体训练中的信用分配，在困难基准上尤其优于基线。


<details>
  <summary>Details</summary>
Motivation: 强化学习用于LLM智能体通常依赖粗粒度的结果级奖励；为了有效更新策略，需要更细粒度的信用分配，但在长时程轨迹中获取可靠局部信用并分配到正确位置仍是一个挑战。

Method: 提出GEAR，一种自适应粒度信用分配框架，通过比较on-policy学生与基于真实条件的教师模型，获得参考引导的发散信号，用于识别自适应分段边界并调节局部优势权重。发散信号在语义偏离时尖峰，后续令牌可能回到低发散。GEAR将这些尖峰作为锚点：对齐区域保持令牌级分辨率，偏离区域将后续内容分组为自适应分段，并用偏离点发散值调节分段优势。

Result: 在8个数学推理和智能体工具使用基准上，使用Qwen3 4B和8B模型，GEAR一致优于标准GRPO、仅自蒸馏基线以及令牌级或轮次级信用分配方法。增益在GRPO基线准确率较低的基准上尤为明显，最高比GRPO提升约20%。

Conclusion: GEAR的自适应重新加权方案在更具挑战性的长时程场景中特别有效，表明自适应粒度信用分配能显著改善策略更新。
...
### [189] [CTFusion: A CTF-based Benchmark for LLM Agent Evaluation](https://arxiv.org/abs/2605.11504)
*Dongjun Lee,Ga-eun Bae,Insu Yun*

Main category: cs.LG

TL;DR: CTFusion 是一个基于实时 CTF 竞赛的流式评估框架，用于可靠地评估 LLM 驱动的网络安全智能体，避免了现有基准的数据污染问题。


<details>
  <summary>Details</summary>
Motivation: 现有 CTF 基准重用已有题目，存在数据污染和潜在作弊风险，作者通过集成网络搜索工具证实了这一点，因此需要更可靠的评估方法。

Method: 提出 CTFusion，在 CTFd 平台实现 MCP 服务器，以单一团队账号保留各智能体独立，仅转发每个题目的首个正确旗帜以减少竞赛影响，实现实时竞赛的流式评估。

Result: 使用三个 LLM、两个智能体和五场实时 CTF 的实验表明，CTFusion 能有效避免现有基准的不可靠性，可作为稳健的网络安全智能体评估方案。

Conclusion: 现有 CTF 基准存在严重缺陷，CTFusion 作为开源框架为网络安全智能体研究提供了更真实、可靠的评估方式。
...
### [190] [EqOD: Symmetry-Informed Stability Selection for PDE Identification](https://arxiv.org/abs/2605.11524)
...
TL;DR: 本文研究LoRA适配器中B矩阵内可训练参数的选择问题，发现随机选择在SFT下与信息性选择性能相当，但在GRPO下梯度信息引导的选择才能恢复标准LoRA精度。


<details>
  <summary>Details</summary>
Motivation: 探究参数放置问题（parameter placement problem）：在LoRA微调预算固定为k个可训练项时，参数的选择是否关键。此前假设随机选择即足够，但作者发现这取决于微调范式（SFT vs GRPO）。

Method: 对比监督微调（SFT）与GRPO下随机子集与梯度信息子集的性能差异。提出一种评分流程，利用梯度结构分析：SFT梯度低秩且方向稳定，任意子集能累积相干更新；GRPO梯度高秩且跨步近正交，仅持续符号一致的梯度元素保留学习信号。评分耗时不足10秒，开销低于训练成本0.5%。

Result: SFT下随机与信息性放置性能相当；GRPO下随机放置无改进甚至退化，而梯度引导放置恢复标准LoRA准确率。关键参数集中于残差流写入投影（V、O、Down），并在1.5B至8B模型家族和尺度上保持稳定。

Conclusion: 参数放置选择在GRPO范式下至关重要，梯度引导的选择策略既高效又有效，能以极低成本实现与全参数LoRA相当的精度，并跨模型泛化。
...
### [192] [OverNaN: NaN-Aware Oversampling for Imbalanced Learning with Meaningful Missingness](https://arxiv.org/abs/2605.11525)
*Amanda S Barnard*
...
### [194] [FERMI: Exploiting Relations for Membership Inference Against Tabular Diffusion Models](https://arxiv.org/abs/2605.11527)
*Abtin Mahyar,Masoumeh Shafieinejad,Yuhan Liu,Xi He*

Main category: cs.LG

TL;DR: FERMI leverages relational auxiliary information during attack training to enhance membership inference attacks on tabular diffusion models, significantly improving attack performance over single-table baselines.


<details>
  <summary>Details</summary>
Motivation: Existing membership inference attacks for tabular synthesis assume a single-table setting, ignoring the multi-relational structure of real sensitive data. A key challenge is how to exploit auxiliary information from related tables (e.g., parent tables) when at inference time only the target record's attributes are available.

Method: We propose FERMI (FEature-mapping for Relational Membership Inference), which enriches single-table features with relational membership signals. It uses auxiliary information from associated relations during attack model training, while inference relies solely on target table attributes.

Result: Across three diffusion architectures and three real-world relational datasets, FERMI consistently outperformed single-table baselines. TPR@0.1FPR improved by up to 53% in the white-box setting and 22% in the black-box setting.

Conclusion: FERMI demonstrates that relational information significantly heightens membership inference risks for tabular diffusion models. Privacy evaluations must account for multi-table structures to avoid underestimating leakage.
...
TL;DR: 本文提出了一种解耦且顺序感知的框架，用于改进大型语言模型的语言化置信度校准。该方法将答案生成与置信度估计分离，并采用基于排序的强化学习目标来对齐置信度与正确性可能性，从而在保持答案准确性的同时提升校准和故障预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有语言化置信度方法常将答案生成和置信度生成联合优化，导致置信度对齐目标干扰答案准确性。需要一种解耦的方法，在不扰动答案生成过程的前提下可靠地校准置信度。

Method: 提出一种解耦且顺序感知的框架：首先生成答案，然后基于固定的问答对估计置信度。为了将置信度与正确性可能性对齐，构建基于多次模型补全的采样替代，并优化基于排序的强化学习目标，奖励使估计正确性可能性更高的回答获得更高语言化置信度的行为。

Result: 在推理和知识密集型基准上的实验表明，该方法改善了校准和故障预测性能，同时基本保持了答案准确性。

Conclusion: 通过将置信度估计与答案生成解耦，并优化响应间置信度的相对排序，可以实现更可靠的语言化置信度对齐。
...
### [196] [Multi-Narrow Transformation as a Single-Model Ensemble: Boundary Conditions, Mechanisms, and Failure Modes](https://arxiv.org/abs/2605.11530)
...
TL;DR: 本文提出将语言模型的指令微调从顺序单一消息流改为多个并行计算流，使模型能在同一前向传递中同时读取、思考、行动和生成，克服当前智能体无法并行处理的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前基于语言模型的自主智能体仍依赖单一顺序消息流，导致智能体在读取时无法行动、思考时无法读写，限制了可用性、效率、安全性和可监控性。

Method: 将指令微调从顺序消息格式切换为多个并行计算流，为每个角色（用户、系统、自身、工具）分配独立流，每个前向传递同时从多个输入流读取并在多个输出流中生成标记，且所有流因果依赖于先前时间步。

Result: 这种数据驱动的改变消除了顺序流的瓶颈，通过并行化提升了模型效率，通过关注点分离增强了安全性，并改善了可监控性，展示了模型可被“解锁”以同时进行多任务处理。

Conclusion: 采用并行多流指令微调能够解决当前智能体的根本局限性，实现阅读、思考、行动和书写的并发执行，为更高效、安全且易监控的自主智能体铺平道路。
...
### [200] [Fast MoE Inference via Predictive Prefetching and Expert Replication](https://arxiv.org/abs/2605.11537)
...
TL;DR: KV-Fold 是一种无训练的长期上下文推理协议，将键值缓存作为左折叠累加器在块间传递，实现稳定且精确的长距离检索，无需修改模型。


<details>
  <summary>Details</summary>
Motivation: 当前长期上下文推理往往需要架构改动或额外训练，本工作旨在利用冻结预训练Transformer的KV缓存复用，开发一种简单、稳定且无需训练的长期推理方法。

Method: 将键值缓存视为类似于函数式编程中foldl的累加器。按块处理序列时，模型以前续累积的KV缓存作为前缀注意力，然后追加新生成的键值，传递扩大后的缓存；连续应用相同的一步更新，形成块到块的递归，无参数调整或再训练。

Result: 诱导的递归是稳定的：每步漂移短暂上升后饱和为平坦平台，对万倍数值精度变化不敏感，跨块大小和模型家族均鲁棒。在Needle-in-a-Haystack测试中，上下文16K至128K tokens、递归深度达511时，Llama-3.1-8B实现100%精确匹配检索，仅需单块40GB GPU。相比流式方法，KV-Fold在可控的前向传递中保持了长距离检索能力。

Conclusion: 冻结的预训练Transformer天然支持稳定的KV缓存递归，为无需架构变更或训练的长期上下文推理提供了实用路径。
...
### [202] [Sharpen Your Flow: Sharpness-Aware Sampling for Flow Matching](https://arxiv.org/abs/2605.11547)
...
### [203] [Routers Learn the Geometry of Their Experts: Geometric Coupling in Sparse Mixture-of-Experts](https://arxiv.org/abs/2605.12476)
*Sagi Ahrac,Noya Hochwald,Mor Geva*

Main category: cs.LG

TL;DR: 本文揭示了稀疏混合专家（SMoE）模型中路由器与专家之间的几何耦合机制，表明辅助负载均衡损失会破坏这一结构，并据此提出无参数的在线K均值路由器，仅凭余弦相似度分配即可实现低负载失衡，证明几何耦合是有效路由的核心。


<details>
  <summary>Details</summary>
Motivation: SMoE模型训练困难：路由易坍塌至少数专家，而辅助负载均衡损失会降低专家专业化程度。这促使作者从机制上研究路由决策如何形成。

Method: 从理论上推导路由器与对应专家之间的几何耦合关系，并在1B参数SMoE模型中实证验证；分析辅助负载均衡损失对该耦合的破坏作用；提出基于运行平均隐藏状态和余弦相似度的无参数在线K均值路由器。

Result: 发现路由器与专家存在梯度同向的几何耦合，使路由决策反映在专家神经元激活上；辅助负载均衡损失会使不同路由器方向相似度增加近三倍，打破耦合；所提出的K均值路由器相比辅助损失和无损失均衡方法，负载失衡最低，困惑度仅小幅上升。

Conclusion: 几何耦合是SMoE形成有效任务分工的核心，解释路由器如何构建分配几何结构；无参数K均值路由器可捕捉这一耦合，实现高效路由。
...
### [204] [VNDUQE: Information-Theoretic Novelty Detection using Deep Variational Information Bottleneck](https://arxiv.org/abs/2605.11551)
*Aryan Gondkar,Hayder Radha,Yiming Deng*

Main category: cs.LG

TL;DR: 本文提出VNDUQE方法，利用深度变分信息瓶颈（VIB）进行新颖性检测。通过KL散度检测远分布OOD（100% AUROC），预测熵检测近分布OOD（94.7% AUROC），并行组合策略达到95.3%平均AUROC和92% TPR@5%FPR，较MSP基线大幅提升，且校准误差降低38%。


<details>
  <summary>Details</summary>
Motivation: 当前OOD检测方法如最大softmax概率（MSP）缺乏理论依据且存在校准偏差，作者旨在通过VIB的信息论约束获得更可靠且校准的不确定性估计，提升检测性能。

Method: 在MNIST上训练VIB模型，保留部分类别作为OOD，使用信息论指标——KL散度和预测熵进行检测评估，提出并行融合两指标的检测策略，并利用信息瓶颈压缩（β=10^{-3}）提升校准。

Result: KL散度对远OOD（噪声、域偏移）达100% AUROC，预测熵对近OOD（新数字类）达94.7% AUROC；组合策略平均AUROC 95.3%，TPR@5%FPR 92%，较MSP基线（85.0% AUROC，60.1% TPR）提升32个百分点，预期校准误差降低38%。

Conclusion: VIB驱动的新颖性检测提供互补信号，组合后显著超越基线并改善校准，支持在昂贵计算神谕的主动学习中实现有原则的阈值选择。
...
### [205] [MEME: Multi-entity & Evolving Memory Evaluation](https://arxiv.org/abs/2605.12477)
*Seokwon Jung,Alexander Rubinstein,Arnas Uselis,Sangdoo Yun,Seong Joon Oh*

Main category: cs.LG

TL;DR: MEME基准定义多实体、进化环境下的六项记忆任务，发现现有记忆系统在依赖推理上几乎完全崩溃，仅高成本配置能部分缓解。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估单实体更新，而LLM代理需在持久环境中处理多实体信息的存储、更新与推理，缺乏全面评估。

Method: 定义六类任务覆盖多实体与进化维度，包括先前未评分的级联、缺失和删除；在100个受控剧集中评估三种记忆范式的六个系统。

Result: 默认配置下所有系统在依赖推理任务上崩溃（级联平均准确率3%，缺失1%），提示优化、更优检索和更强LLM均未显著改善，仅基于文件的代理配Claude Opus 4.7部分缩小差距，但成本高约70倍。

Conclusion: 当前记忆系统无法有效进行依赖推理，实用化闭差距方案成本过高，亟需更可扩展的架构。代码与数据已公开。
...
### [206] [A Controlled Counterexample to Strong Proxy-Based Explanations of OOD Performance: in a Fixed Pretraining-and-Probing Setup](https://arxiv.org/abs/2605.11554)
*Hongmin Li*

Main category: cs.LG

TL;DR: 任务无关的结构代理可能无法追踪任务相关结构，导致代理对预训练数据集的排名与OOD探测准确率排名不一致，本文通过理论构造和合成实验提供了反例。


<details>
  <summary>Details</summary>
Motivation: 检验基于结构代理的解释的可靠性条件：代理必须真正捕捉到对下游任务重要的结构，而非仅反映整体学习结构。

Method: 在固定预训练与探测框架下，引入计算有界结构概念（如epiplexity），首先进行可控的理论构造分离形式结构量、操作代理与任务相关结构，随后在合成序列模型实验中验证代理排名与OOD准确率排名的关系。

Result: 理论构造展示了三者可解耦；在主要的全样本评估下，三个随机种子中有两个出现OOD准确率排名与代理排名反转，辅助诊断和消融实验支持相同解释。

Conclusion: 反例并非否定结构解释，而是划定了强代理解释的边界：代表总学习结构的代理即使在受控设置下也可能无法跟踪驱动OOD性能的任务相关结构。
...
### [207] [A Composite Activation Function for Learning Stable Binary Representations](https://arxiv.org/abs/2605.11558)
...
### [208] [OUI as a Structural Observable: Towards an Activation-Centric View of Neural Network Training](https://arxiv.org/abs/2605.11570)
*Alberto Fernández-Hernández,Jose I. Mestre,Cristian Pérez-Corral,Manuel F. Dolz,Jose Duato,Enrique S. Quintana-Ortí*

Main category: cs.LG

TL;DR: Overfitting-Underfitting Indicator (OUI) 是一种早期、无标签、基于激活的信号，能在收敛前揭示训练优劣，成为以激活为中心的训练动力学理论的实证支点。


<details>
  <summary>Details</summary>
Motivation: 深层网络的表达力依赖激活函数，但训练过程通常仅通过外部指标（损失、精度等）评估，其内部结构演化几乎未被观测。论文提出需要一个可观测的内部结构指标，OUI 正是为此而设。

Method: OUI 是基于激活的早期无标签信号，在监督学习中预测权重衰减策略优劣，在强化学习（PPO）中早期区分学习率策略，在在线控制中驱动逐层权重衰减自适应。

Result: OUI 在多种场景下一致地提前揭示训练趋势（是否走向过拟合或欠拟合），并能驱动自适应调节。结合激活模式比参数更早稳定的现象，表明激活动态可能是新的理论核心。

Conclusion: OUI 为以激活为中心的训练动力学理论提供了实证立足点，有望成为观察和指导网络内部演化的实用工具。
...
### [209] [FedOUI: OUI-Guided Client Weighting for Federated Aggregation](https://arxiv.org/abs/2605.11571)
*Alberto Fernández-Hernández,Jose I. Mestre,Cristian Pérez-Corral,Manuel F. Dolz,Jose Duato,Enrique S. Quintana-Ortí*
...
### [210] [Stargazer: A Scalable Model-Fitting Benchmark Environment for AI Agents under Astrophysical Constraints](https://arxiv.org/abs/2604.15664)
*Xinge Liu,Terry Jingchen Zhang,Bernhard Schölkopf,Zhijing Jin,Kristen Menou*

Main category: cs.LG

TL;DR: 本文提出Stargazer环境，通过基于径向速度时间序列数据的动态迭代模型拟合任务，评估自主AI智能体在科研中的能力。它包含120个任务（含20个真实案例），分三个难度，覆盖从高信噪比单行星到复杂低信噪比多行星系统。对8个前沿智能体的评估显示，智能体虽能取得良好统计拟合，但常违反物理约束，无法恢复正确物理参数；增加推理算力仅带来边际收益，而过度消耗令牌常反映递归失败循环。Stargazer为当前实际研究中的模型拟合问题提供了训练、评估和扩展策略的机会，其仿真驱动环境设计可推广到其他科学领域的模型拟合问题。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI智能体的兴起，需要具备内在反馈机制的动态基准环境来评估它们在科研工作中的能力，尤其是基于科学实际任务的迭代式模型拟合。

Method: 构建Stargazer——一个可扩展的评估环境，以径向速度时间序列数据的推断为基础，设计动态、迭代的物理模型拟合任务。环境包含120个任务，横跨三个难度等级，其中20个源自真实档案案例，场景从高信噪比单行星系统到需精细分析的低信噪比多行星系统。

Result: 对八个前沿智能体的评估发现：智能体常能达到较好的统计拟合，却难以恢复正确的物理系统参数，即使配备基本技能仍有此局限；增加测试时计算量只带来微弱提升，过多的令牌消耗往往反映的是递归失败循环而非有效探索。

Conclusion: Stargazer为当前具有实际研究价值的模型拟合问题提供了训练、评估、脚手架和扩展策略的机会，其仿真驱动环境设计方法可推广至众多科学领域的模型拟合问题。
...
### [211] [Learning Weakly Communicating Average-Reward CMDPs: Strong Duality and Improved Regret](https://arxiv.org/abs/2605.11586)
...
### [212] [SoK: Unlearnability and Unlearning for Model Dememorization](https://arxiv.org/abs/2605.11592)
...
### [213] [EpiCastBench: Datasets and Benchmarks for Multivariate Epidemic Forecasting](https://arxiv.org/abs/2605.11598)
...
### [214] [Targeted Tests for LLM Reasoning: An Audit-Constrained Protocol](https://arxiv.org/abs/2605.11599)
*Hongmin Li*
...
### [215] [From Generic Correlation to Input-Specific Credit in On-Policy Self Distillation](https://arxiv.org/abs/2605.11613)
*Guobin Shen,Lei Huang,Xiang Cheng,Chenxiao Zhao,Jindong Li,Dongcheng Zhao,Xing Yu*

Main category: cs.LG

TL;DR: 本文提出CREDIT方法，将自蒸馏token奖励解释为贝叶斯滤波增量，其轨迹和为响应与反馈的点互信息；通过批次对比基线隔离输入特定成分，避免通用捷径，以可忽略的额外计算在多基准上取得最优综合性能。


<details>
  <summary>Details</summary>
Motivation: 自蒸馏token奖励本质不明，可能被输入无关的通用捷径抬高，需分离输入特定推理贡献以正确分配信用。

Method: 在反馈条件化的后验相容框架下，将token奖励建模为贝叶斯滤波增量，分解教师对数概率沿输入轴；提出CREDIT，利用批次内对比基线隔离输入特定成分，作为对比点互信息目标的教师端代理，并惩罚在无关输入下仍高概率的响应。

Result: 在两个模型家族的编程、科学推理和工具使用基准上，CREDIT以几乎无额外计算开销取得了最强的综合性能。

Conclusion: CREDIT有效分离输入特定信用，通过消除通用捷径提升了自蒸馏学习质量，为后续训练提供了更可靠的密集奖励信号。
...
### [216] [MIST: Reliable Streaming Decision Trees for Online Class-Incremental Learning via McDiarmid Bound](https://arxiv.org/abs/2605.11617)
*Phu-Hoa Pham,Chi-Nguyen Tran,Nguyen Lam Phu Quy,Dao Sy Duy Minh,Huynh Trung Kiet,Long Tran-Thanh*

Main category: cs.LG

TL;DR: MIST (McDiarmid Incremental Streaming Tree) introduces a K-independent splitting criterion, Bayesian knowledge transfer, and KLL quantile sketches to overcome failures of streaming decision trees in open-world continual learning, achieving robust performance on tabular streams.


<details>
  <summary>Details</summary>
Motivation: Streaming decision trees are natural for continual learning but fail in online class-incremental settings because their split criterion becomes unreliable as class count grows and they lack knowledge transfer at split time, both due to Information Gain scaling with log2 K.

Method: MIST integrates three components: (i) a K-independent McDiarmid confidence radius for Gini splitting as a structural regularizer; (ii) a Bayesian inheritance protocol that projects parent statistics to child nodes via truncated-Gaussian moments, ensuring variance reduction especially under conservative splits; (iii) per-leaf KLL quantile sketches supporting continuous threshold evaluation and geometry-adaptive leaf prediction.

Result: On standard and stress-test tabular streams, MIST is competitive with global parametric methods on near-Gaussian benchmarks and uniquely robust on non-Gaussian geometry where state-of-the-art methods collapse.

Conclusion: MIST fixes the inherent miscalibrations of streaming trees, enabling reliable class-incremental learning and demonstrating exceptional robustness on non-Gaussian data distributions.
...
### [217] [Evolutionary Task Discovery: Advancing Reasoning Frontiers via Skill Composition and Complexity Scaling](https://arxiv.org/abs/2605.11666)
*Liqin Ye,Yanbin Yin,Michael Galarnyk,Yuzhao Heng,Sudheer Chava,Chao Zhang*

Main category: cs.LG

TL;DR: 本文提出进化式任务发现框架EvoTD，通过结构化进化算子在有向搜索空间中合成多样化且适合模型学习的训练任务，显著提升LLM的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于无结构变异或探索的数据合成方法容易陷入同质化坍塌，无法系统性地扩展推理前沿，而强化学习等后训练方法的效果受限于训练数据的多样性和复杂性。

Method: 提出EvoTD框架，将数据合成视为在算法技能与复杂度属性双轴流形上的有向搜索；设计交叉算子合成新技能组合以提升多样性，参数变异算子缩放结构约束（如输入规模、树深度）以增强泛化；并引入动态最近发展区过滤器，使任务保持在模型可学习区域。

Result: EvoTD在多种模型架构、预训练策略和规模上均取得显著的推理性能提升，且增益具有良好的泛化一致性。

Conclusion: 结构化进化课程能够有效推动推理能力的提升，EvoTD框架为此提供了一种系统性扩展推理前沿的有效途径。
...
### [218] [Partial Model Sharing Improves Byzantine Resilience in Federated Conformal Prediction](https://arxiv.org/abs/2605.11684)
...
### [219] [Shaping Zero-Shot Coordination via State Blocking](https://arxiv.org/abs/2605.11688)
*Mingu Kang,Sunwoo Lee,Yonghyeon Jo,Seungyul Han*

Main category: cs.LG

TL;DR: 本文提出State-Blocked Coordination (SBC)框架，通过状态阻塞生成虚拟环境让智能体接触多样次优策略，从而提升零样本协调性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本协调方法主要依靠增加训练伙伴多样性，但在泛化到未见伙伴时往往不可靠，需要更有效的方法来提升泛化能力。

Method: SBC通过状态阻塞技术在不直接修改环境的前提下生成一系列虚拟环境，使智能体在训练中经历多种次优伙伴策略，从而诱导出丰富的交互场景。

Result: 在多个基准测试上，SBC展现出优越的零样本协调性能，并能强泛化到与人类伙伴的合作中。

Conclusion: SBC是一种简单而有效的框架，通过状态阻塞显著提升了零样本协调的泛化能力，适用于真实多智能体系统和人机协作。
...
### [220] [Compositional Neural Operators for Multi-Dimensional Fluid Dynamics](https://arxiv.org/abs/2605.11691)
...
### [221] [Augmented Lagrangian Method for Last-Iterate Convergence for Constrained MDPs](https://arxiv.org/abs/2605.11694)
...
### [222] [GRAFT: Graph-Tokenized LLMs for Tool Planning](https://arxiv.org/abs/2605.11706)
*Xinyi Gao,Xinyu Ren,Junliang Yu,Tong Chen,Quoc Viet Hung Nguyen,Hongzhi Yin*

Main category: cs.LG

TL;DR: GRAFT框架通过内部化工具图为特殊令牌并学习工具依赖，结合在轨蒸馏，实现依赖感知的LLM工具规划，达到最优序列匹配和依赖合法性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将工具依赖建模为外部图并用匹配范式结合LLM，常无法对齐子任务结构，导致违反图约束和错误累积。

Method: 1) 将工具图内部化，为每个工具节点分配专用特殊令牌，在表征空间中学习有向工具依赖；2) 引入在轨策略工具上下文蒸馏，利用模型自身采样轨迹训练并蒸馏逐步规划信号。

Result: 在精确序列匹配和依赖合法性上取得最先进性能，支持复杂工作流中更可靠的LLM工具规划。

Conclusion: GRAFT通过内部化工具图和策略蒸馏，有效克服外部匹配范式缺陷，提升工具规划的可依赖性与合规性。
...
### [223] [Unlocking Compositional Generalization in Continual Few-Shot Learning](https://arxiv.org/abs/2605.11710)
...
### [224] [Debiased Model-based Representations for Sample-efficient Continuous Control](https://arxiv.org/abs/2605.11711)
...
### [225] [Block-R1: Rethinking the Role of Block Size in Multi-domain Reinforcement Learning for Diffusion Large Language Models](https://arxiv.org/abs/2605.11726)
*Yan Jiang,Ruihong Qiu,Zi Huang*

Main category: cs.LG

TL;DR: 本文研究扩散大语言模型强化学习后训练中的多域块大小冲突，提出Block-R1基准和数据集，并设计样本级最佳块大小的跨域后训练方法。


<details>
  <summary>Details</summary>
Motivation: 在扩散大语言模型的强化学习后训练中，块大小决定并行解码粒度并影响GRPO等算法的rollout轨迹，但现有研究未从多域场景考虑域间块大小冲突，限制了多域RL后训练效果。

Method: 形式化多域块大小冲突问题，构建Block-R1-41K数据集（为每个样本提供最佳训练块大小，并引入块大小冲突分数），建立Block-R1基准，以及提出基于样本级最佳块大小的跨域后训练方法。

Result: 在13个数据集、7种最新RL算法和多种dLLM骨干上的实验表明，所提方法能有效缓解域间块大小冲突，提升多域强化学习后训练性能。

Conclusion: 本文从域冲突视角为扩散大语言模型的多域RL后训练提供解决方案，并开源Block-R1基准与Block-R1-41K数据集。
...
### [226] [Persona-Conditioned Adversarial Prompting: Multi-Identity Red-Teaming for Adversarial Discovery and Mitigation](https://arxiv.org/abs/2605.11730)
...
### [227] [U-STS-LLM A Unified Spatio-Temporal Steered Large Language Model for Traffic Prediction and Imputation](https://arxiv.org/abs/2605.11735)
*Yichen Zhang,Jun Li*
...
### [228] [Online Continual Learning with Dynamic Label Hierarchies](https://arxiv.org/abs/2605.11742)
...
### [229] [Learning Feature Encoder with Synthetic Anomalies for Weakly Supervised Graph Anomaly Detection](https://arxiv.org/abs/2605.11749)
...
### [230] [Federated Client Selection under Partial Visibility: A POMDP Approach with Spatio-Temporal Attention](https://arxiv.org/abs/2605.11752)
*Qijun Hou,Yuchen Shi,Pingyi Fan,Khaled B. Letaief*

Main category: cs.LG

TL;DR: 本文针对联邦学习中部分客户端可见性下的选择问题，将其建模为部分可观测马尔可夫决策过程，并提出基于时空注意力的强化学习框架，在异构部分可见场景下性能优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有客户端选择方法假设每轮全可见，但实际大规模或边缘部署中因通信、移动性等限制，服务器只能观测到部分客户端，导致数据异构下性能下降，需解决部分可见性挑战。

Method: 将部分可见性下的客户端选择建模为POMDP，提出时空注意力强化学习框架，融合历史全局模型和客户端身份嵌入，捕捉训练的时间上下文和客户端固有特征。

Result: 在多个数据集上的实验表明，该方法在异构和部分可见设置下性能优于现有基线。

Conclusion: 验证了该方法能有效应对实际联邦学习系统中不完整观测的挑战，实现了优越性能。
...
### [231] [One-Step Generative Modeling via Wasserstein Gradient Flows](https://arxiv.org/abs/2605.11755)
...
### [232] [Decomposing the Generalization Gap in PROTAC Activity Prediction: Variance Attribution and the Inter-Laboratory Ceiling](https://arxiv.org/abs/2605.11764)
...
### [233] [Is Monotonic Sampling Necessary in Diffusion Models?](https://arxiv.org/abs/2605.11773)
*Muhammad Haris Khan*
...
### [234] [Stop Marginalizing My Dreams: Model Inversion via Laplace Kernel for Continual Learning](https://arxiv.org/abs/2605.11804)
...
### [235] [Fed-BAC: Federated Bandit-Guided Additive Clustering in Hierarchical Federated Learning](https://arxiv.org/abs/2605.11815)
...
### [236] [Gradient Clipping Beyond Vector Norms: A Spectral Approach for Matrix-Valued Parameters](https://arxiv.org/abs/2605.11838)
...
### [237] [Martingale-Consistent Self-Supervised Learning](https://arxiv.org/abs/2605.11846)
...
### [238] [Improving the Performance and Learning Stability of Parallelizable RNNs Designed for Ultra-Low Power Applications](https://arxiv.org/abs/2605.11855)
...
### [239] [Beyond Parameter Aggregation: Semantic Consensus for Federated Fine-Tuning of LLMs](https://arxiv.org/abs/2605.11857)
...
### [240] [Information theoretic underpinning of self-supervised learning by clustering](https://arxiv.org/abs/2605.11870)
*Josef Kittler,Sara Atito,Muhammad Awais*

Main category: cs.LG

TL;DR: 该论文将自监督学习（尤其是深度聚类）形式化为K-L散度优化，通过约束教师分布防止模式坍塌，从理论上证明了归一化（基于逆聚类先验）可简化为批中心化，从而为蒸馏和中心化等常见启发式方法提供了理论支撑。


<details>
  <summary>Details</summary>
Motivation: 自监督学习是构建基础模型的关键，但其理论基础仍不完善。论文旨在推动自监督学习的理论发展，特别是为深度聚类方法建立严格的理论框架。

Method: 将自监督学习建模为K-L散度优化问题，通过对教师分布施加优化约束来防止模式坍塌，该约束导出基于逆聚类先验的归一化步骤；利用詹森不等式证明该归一化等价于常用的批中心化过程。

Result: 理论模型不仅解释了现有成功的自监督学习方法（如蒸馏与中心化）为何有效，还指出了未来研究方向。

Conclusion: 该研究为自监督学习中的启发式实践提供了理论根据，深化了对深度聚类原理的理解，并为后续方法设计提供了指导。
...
### [241] [LOFT: Low-Rank Orthogonal Fine-Tuning via Task-Aware Support Selection](https://arxiv.org/abs/2605.11872)
*Lanxin Zhao,Bamdev Mishra,Pratik Jawanpuria,Lequan Lin,Dai Shi,Junbin Gao,Andi Han*

Main category: cs.LG

TL;DR: LOFT框架将正交参数高效微调解耦为子空间选择与变换，提出基于梯度信号的任务感知支持选择策略，在多种任务中提升效率与性能的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有正交参数高效微调方法常混淆微调子空间和子空间内变换这两个设计选择，阻碍了对方法本质的理解与改进。

Method: 提出LOFT低秩正交微调框架，将正交适配统一为乘性子空间旋转，恢复多种现有方法；进一步将支持选择明确为设计主轴，通过一阶分析表明支持应由下游训练信号决定，设计任务感知支持选择策略。

Result: 在语言理解、视觉迁移、数学推理及多语言分布外适应等任务上，LOFT恢复主子空间正交适配，梯度信息支持选择在匹配的参数、内存和计算预算下改善了效率-性能折衷。

Conclusion: 有原则的支持选择是改进正交PEFT的重要方向。
...
### [242] [Adaptive TD-Lambda for Cooperative Multi-agent Reinforcement Learning](https://arxiv.org/abs/2605.11880)
*Yue Deng,Zirui Wang,Yin Zhang*

Main category: cs.LG

TL;DR: 提出ATD(λ)方法，通过密度比估计器与双回放缓冲区解决多智能体中策略分布计算难题，在QMIX和MAPPO上超越固定λ基线。


<details>
  <summary>Details</summary>
Motivation: 现有单智能体工作通过策略分布自适应调节TD(λ)，但多智能体环境中联合动作空间大、转移数据少，导致策略分布无法统计计算，制约了自适应λ方法的直接应用。

Method: 采用参数化似然无关密度比估计器，利用两个不同大小的回放缓冲区分别存储过去和当前策略的历史轨迹，为状态-动作对分配基于当前策略稳态分布下似然的自适应TD(λ)值（ATD(λ)）。

Result: 在SMAC与Gfootball学院场景下，将ATD(λ)集成到值基算法QMIX和AC基算法MAPPO中，相比固定λ的基线方法取得了持续竞争或更优的性能。

Conclusion: ATD(λ)成功克服了多智能体策略分布难以估计的障碍，提升了主流MARL算法的性能，拓展了自适应TD(λ)在多智能体领域的应用。
...
### [243] [Sobolev Regularized MMD Gradient Flow](https://arxiv.org/abs/2605.11884)
...
### [245] [Procedural-skill SFT across capacity tiers: A W-Shaped pre-SFT Trajectory and Regime-Asymmetric Mechanism on 0.8B-4B Qwen3.5 Models](https://arxiv.org/abs/2605.11907)
*Igor Strozzi*
...
### [246] [Delightful Gradients Accelerate Corner Escape](https://arxiv.org/abs/2605.11908)
...
### [247] [Understanding Sample Efficiency in Predictive Coding](https://arxiv.org/abs/2605.11911)
...
### [248] [STAGE: Tackling Semantic Drift in Multimodal Federated Graph Learning](https://arxiv.org/abs/2605.11919)
...
### [249] [NOFE -- Neural Operator Function Embedding](https://arxiv.org/abs/2605.11970)
...
TL;DR: 本文提出双组优势优化（DGAO），首次用强化学习减轻大语言模型的顺序偏差，通过平衡组内准确率优势与组间稳定性优势，实现准确性与顺序公平性的同步提升。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在顺序偏差，限制其在上下文学习和检索增强生成等场景的应用。现有方法或增加推理开销，或以牺牲准确性为代价（如导致幻觉），未能根本解决顺序敏感性问题，且缺乏对伪稳定性的有效评估。

Method: DGAO通过强化学习直接优化模型策略，计算并平衡组内相对准确率优势与组间相对稳定性优势，奖励生成顺序稳定且正确的输出，惩罚顺序敏感或错误的输出。同时提出一致性率与过度自信率两个新指标，揭示以往方法的伪稳定性并引导全面评估。

Result: 在RAG、数学推理和分类任务上的大量实验表明，DGAO在获得卓越顺序公平性的同时，显著提升了任务性能，优于现有方法。

Conclusion: DGAO首次利用强化学习有效缓解LLM的顺序敏感性，同步提升准确性与顺序稳定性；提出的新指标有助于克服伪稳定性问题，推动该领域更全面的评估。
...
TL;DR: 本文提出一种在随机环境中满足概率可达-避障约束的同时最小化期望累积成本的强化学习方法，通过引入可达-避障概率证书和基于收缩的Bellman公式实现，具有几乎必然收敛性，并在MuJoCo模拟器中验证了改进的成本和约束满足率。


<details>
  <summary>Details</summary>
Motivation: 现有安全和约束强化学习方法通常无法在随机环境的学习过程中同时强制满足概率可达-避障约束并优化成本。

Method: 引入可达-避障概率证书（RAPCs）来识别可满足约束的状态，基于RAPCs开发了一种基于收缩的Bellman公式作为将可达-避障考量融入强化学习的代理，实现了概率约束下的成本优化，并证明了算法几乎必然收敛到局部最优策略。

Result: 在MuJoCo模拟器中的实验表明，所提方法在成本性能上有所提升，且具有持续更高的可达-避障满足率。

Conclusion: 该方法有效地在随机环境中联合实现了概率可达-避障约束和成本优化，具有理论收敛性保证和实证优势。
...
### [256] [Efficient and Adaptive Human Activity Recognition via LLM Backbones](https://arxiv.org/abs/2605.12019)
...
### [257] [Approximation Theory of Laplacian-Based Neural Operators for Reaction-Diffusion System](https://arxiv.org/abs/2605.12025)
*Takashi Furuya,Ryo Ozawa,Jenn-Nan Wang*

Main category: cs.LG

TL;DR: 本文研究神经算子对广义Gierer-Meinhardt反应扩散系统的解映射，利用Laplacian谱表示建立显式逼近误差界，证明参数复杂度随精度至多多项式增长，避免了参数复杂性诅咒，数值实验支持理论。


<details>
  <summary>Details</summary>
Motivation: 神经算子的通用逼近理论虽已成熟，但对非线性反应扩散系统的逼近分析仍很有限，本文旨在填补这一空白。

Method: 将神经算子应用于广义Gierer-Meinhardt系统从初始条件到时间依赖解的映射，利用方程格林函数的Laplacian谱表示，推导出以网络深度、宽度和谱秩表示的显式误差界。

Result: 理论证明所需参数复杂度随目标精度至多多项式增长，表明基于Laplacian本征函数的架构可缓解通用算子学习中的参数复杂性诅咒；在Gierer-Meinhardt系统上的数值实验支持该发现。

Conclusion: 基于Laplacian本征函数的神经算子能高效逼近非线性反应扩散解算子，参数规模多项式缩放，避免了通用算子学习的参数诅咒。
...
### [258] [Resilient Vision-Tabular Multimodal Learning under Modality Missingness](https://arxiv.org/abs/2605.12031)
...
### [259] [Scaling Laws and Tradeoffs in Recurrent Networks of Expressive Neurons](https://arxiv.org/abs/2605.12049)
...
### [260] [Learning plug-in surrogate endpoints for randomized experiments](https://arxiv.org/abs/2605.12051)
*Alessandro-Umberto Margueritte,Ahmet Zahid Balcıoğlu,Jesse Krijthe,Dave Zachariah,Fredrik D. Johansson*

Main category: cs.LG

TL;DR: 本文针对随机实验中替代终点学习问题，提出最大化效应预测性的方法，实验表明其方法预测主要结局效果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有替代终点的因果准则大多不可识别，难以转化为从数据中学习替代终点的实用算法，亟需可操作的学习方法。

Method: 研究介入式复合替代终点，提出两种最大化效应预测性的学习方法，其中一种直接建模替代效应。

Result: 在合成实验和真实世界实验中，所提方法得到的介入式终点对主要效应的预测能力均优于已有方法。

Conclusion: 本文提供了学习高预测性替代终点的实用方法，为实验优化提供了新途径。
...
### [261] [Hölder Policy Optimisation](https://arxiv.org/abs/2605.12058)
...
TL;DR: 本文提出HölderPO框架，通过Hölder均值统一token级概率聚合，并引入动态退火策略调节参数p，以解决GRPO中固定聚合机制导致的训练坍塌与性能不足问题，在数学推理和具身任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: GRPO中轨迹级优势到策略更新的映射需要聚合token级概率，但固定聚合方式无法适应不同训练阶段，导致训练坍塌或性能不佳，存在梯度集中度与方差之间的根本权衡。

Method: 提出HölderPO，一种基于Hölder均值的广义策略优化框架，通过参数p连续控制梯度集中度与方差界限；理论证明p增大则梯度集中以强化稀疏信号，p减小则严格约束方差；并设计动态退火算法在训练全周期自适应调度p。

Result: 在多个数学基准上平均准确率达54.9%，比标准GRPO相对提升7.2%；在ALFWorld任务上成功率达93.8%，展现出优于基线的稳定性和收敛性。

Conclusion: HölderPO框架有效解决了GRPO中静态聚合的适应性问题，通过动态调节聚合方式实现了梯度集中与稳定性的平衡，为策略优化提供了新的通用且自适应的解决方案。
...
### [262] [Missing Old Logits in Asynchronous Agentic RL: Semantic Mismatch and Repair Methods for Off-Policy Correction](https://arxiv.org/abs/2605.12070)
*Zhong Guan,Yongjian Guo,Haoran Sun,Wen Huang,Shuai Di,Xiong Jun Wu,Likang Wu,Hongke Zhao*

Main category: cs.LG

TL;DR: 本文针对异步强化学习中缺失历史logits（旧logits）导致PPO离策略校正失效的问题，提出了精确和近似的校正策略，并采用修正的PPO-EWMA方法显著提升了训练速度和优化性能。


<details>
  <summary>Details</summary>
Motivation: 异步强化学习通过解耦样本生成与策略优化提高了大语言模型智能体的吞吐量，但异构训练系统中缺失历史训练侧logits（旧logits）会破坏重要性比率分解的语义，使分布差异修正与策略陈旧性修正纠缠，导致裁切和掩码阈值不当交互，引发PPO式离策略校正失败。

Method: 本文研究精确和近似校正路线。精确方案提出三种旧logits获取策略：基于快照的版本追踪、专用旧logit模型、以及通过部分rollout中断实现同步，并比较其系统权衡。近似校正方面，聚焦于在无法低成本恢复精确旧logits时，通过更合适的近似策略保持解耦校正优势，采用修正的PPO-EWMA方法，不增加额外系统开销。

Result: 修正的PPO-EWMA方法在训练速度和优化性能上均取得显著提升。

Conclusion: 该方法有效解决了缺失旧logits问题，实现了异步训练中解耦校正的语义保真，为大规模语言模型智能体的异步强化学习提供了高效解决方案。
...
### [263] [Elicitation-Augmented Bayesian Optimization](https://arxiv.org/abs/2605.12079)
...
### [264] [Disentangled Sparse Representations for Concept-Separated Diffusion Unlearning](https://arxiv.org/abs/2605.12122)
...
### [265] [Fused Gromov-Wasserstein Distance with Feature Selection](https://arxiv.org/abs/2605.12161)
...
### [266] [On What We Can Learn from Low-Resolution Data](https://arxiv.org/abs/2605.12168)
*Theresa Dahl Frehr,Niels Henrik Pontoppidan,Hiba Nassar,Tommy Sonne Alstrøm*

Main category: cs.LG

TL;DR: 该论文利用KL散度理论分析了混合分辨率数据集中低分辨率数据的影响，并通过实验证明在高质量数据稀缺时添加低分辨率数据可提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实领域（如医疗、公共机构、可穿戴设备）常因存储、隐私或资源限制无法共享集中式大数据集，导致收集的数据包含高、低分辨率混合样本。对于模型在高分辨率输入上评估时低分辨率数据的实际信息价值尚不明确。

Method: 论文基于Kullback-Leibler散度进行理论分析，刻画数据点影响力随分辨率变化的特性，并推导出高、低分辨率观测对降采样信息损失的相对贡献边界。实验中采用视觉Transformer和卷积神经网络，向训练集添加低分辨率数据。

Result: 实验表明，当高分辨率数据稀缺时，添加低分辨率数据能持续提升模型性能。

Conclusion: 低分辨率数据在训练中具有补益作用，即便存在降采样信息损失，其理论边界也支持这一发现。
...
### [267] [Lower bounds for one-layer transformers that compute parity](https://arxiv.org/abs/2605.12171)
*Daniel Hsu*
...
### [268] [Expected Batch Optimal Transport Plans and Consequences for Flow Matching](https://arxiv.org/abs/2605.12174)
...
### [269] [Multi-Task Representation Learning for Conservative Linear Bandits](https://arxiv.org/abs/2605.12176)
...
### [270] [DriftXpress: Faster Drifting Models via Projected RKHS Fields](https://arxiv.org/abs/2605.12183)
...
### [271] [Fair Conformal Classification via Learning Representation-Based Groups](https://arxiv.org/abs/2605.12195)
...
### [272] [ECTO: Exogenous-Conditioned Temporal Operator for Ultra-Short-Term Wind Power Forecasting](https://arxiv.org/abs/2605.12196)
...
### [273] [A Unified Graph Language Model for Multi-Domain Multi-Task Graph Alignment Instruction Tuning](https://arxiv.org/abs/2605.12197)
...
### [274] [Overtrained, Not Misaligned](https://arxiv.org/abs/2605.12199)
*Joel Schreiber,Ariel Goldstein*

Main category: cs.LG

TL;DR: 大规模研究显示，新现错位（EM）并非普遍现象，仅在少数大模型中出现，且可通过早停和学习率调整有效规避，将EM从微调风险转化为可控训练问题。


<details>
  <summary>Details</summary>
Motivation: 此前研究指出微调可能引发跨领域错位，但缺乏对开源模型、不同领域及缓解方法的全面分析。本研究旨在系统探究EM的普遍性、产生机制和实用缓解策略。

Method: 复现GPT-4o的EM实验，并扩展至12个开源模型（涵盖Llama、Qwen、DeepSeek、GPT-OSS家族，参数8B–671B），评估超百万条模型响应，采用多随机种子。通过微调过程中的检查点分析EM出现时机，并在医学微调领域进行跨领域验证。

Result: EM在GPT-4o中复现，但仅17%的开源模型表现出一致的EM，且与模型大小呈显著正相关。检查点分析表明EM出现在微调后期，晚于主任务收敛。早停可消除EM并平均保留93%的任务性能，学习率选择进一步降低风险。医学微调中大小-EM相关性达r=0.90，早停在67%案例中有效避免不真实倾向，但语义相近领域的错位更难以分离。

Conclusion: EM可通过早停和谨慎学习率等适当训练实践予以避免，从而将其重新定义为一种可规避的训练产物，而非不可预见的微调风险。研究为安全微调LLM提供了可行的指导。
...
### [275] [Investigating simple target-covariate relationships for Chronos-2 and TabPFN-TS](https://arxiv.org/abs/2605.12200)
...
### [276] [On the Importance of Multistability for Horizon Generalization in Reinforcement Learning](https://arxiv.org/abs/2605.12206)
*Asad Bakija,Florent De Geeter,Julien Brandoit,Pierre Sacré,Guillaume Drion*

Main category: cs.LG

TL;DR: 论文提出时间范围泛化的形式化框架，揭示多稳态和瞬态动力学是实现长时程部分可观测马尔可夫决策过程中策略泛化的关键，而现有并行化RNN架构因缺失这两种动态机制而失败。


<details>
  <summary>Details</summary>
Motivation: 长时程POMDP中，训练面临泛化差、样本效率低和探索成本高等问题，亟需理解何时能在短时程训练的策略在任意更长的时程上保持最优。

Method: 形式化定义了时间范围泛化性质，推导了其充要条件，并通过实验评估了非线性RNN及可并行化变体的能力。

Result: 多稳态是时间范围泛化的必要条件，在简单任务中也为充分条件；复杂任务还需要瞬态动力学。现代并行架构（状态空间模型和门控线性RNN）因构造上单稳态而无法泛化。

Conclusion: 多稳态和瞬态动力学是范围泛化的两个互补基本动态机制，当前可并行化RNN无法同时具备二者，因此设计兼具这两种机制的架构是可扩展长时程强化学习的关键方向。
...
### [277] [Intrinsic Vicarious Conditioning for Deep Reinforcement Learning](https://arxiv.org/abs/2605.12224)
*Rodney A Sanchez,Ferat Sahin,Alex Ororbia,Jamison Heard*

Main category: cs.LG

TL;DR: 本文提出替代性条件作用作为内在奖励机制，使智能体无需访问演示者策略或奖励函数即可从示例中学习，从而在无明确终止奖励的环境中延长存活时间。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习中的内在驱动力多基于直接条件作用，限制了智能体从环境和他人学习的能力；离策略方法虽可学习示例，但需要访问演示者的策略或奖励函数。本研究旨在打破这一限制，实现无需直接访问演示者内部信息的观察学习。

Method: 借鉴心理学与生物学理论，提出基于记忆的替代性条件作用内在奖励机制，包含注意、保持、再现和强化四个步骤，支持少样本学习，且不依赖演示者策略或奖励函数。

Result: 在MiniWorld Sidewalk和Box2D CarRacing环境中评估，结果表明替代性条件作用能引导智能体规避非描述性终止条件，延长回合长度，转向期望状态。

Conclusion: 该工作模拟了一种更符合认知规律的学习范式，尤其适用于单生命学习或持续学习等场景，有效克服了直接条件作用的局限性。
...
### [278] [No More, No Less: Task Alignment in Terminal Agents](https://arxiv.org/abs/2605.12233)
...
TL;DR: TAB是一个基准测试，包含89个终端任务，要求智能体有选择地遵循相关环境提示而忽略干扰；对前沿智能体的评估显示任务完成率高但对齐度低，防御提示注入也会抑制必要的提示。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法衡量智能体选择性解释环境指令的能力；智能体可能因盲目遵循所有指令而显得有能力，或因完全忽略指令而显得稳健。

Method: 引入TAB，基于Terminal-Bench 2.1的89个终端任务，每个任务刻意未完全指定，缺失信息以必要提示的形式嵌入自然环境工件中，同时提供看似合理但无关的干扰项。应用于十个前沿智能体，并评估六种提示注入防御。

Result: 发现任务能力与任务对齐之间存在系统性差距；最强的Terminal-Bench智能体在TAB上任务完成率高但任务对齐度低。抑制干扰执行也会抑制完成任务所需的提示。

Conclusion: 任务对齐的智能体需要有选择地利用环境指令，而非全盘接受或拒绝。
...
### [279] [SOAR: Scale Optimization for Accurate Reconstruction in NVFP4 Quantization](https://arxiv.org/abs/2605.12245)
*Chengzhu Bao,Xianglong Yan,Zhiteng Li,Guangshuo Qin,Guanghua Yu,Yulun Zhang*

Main category: cs.LG

TL;DR: SOAR是一种针对NVFP4量化的后训练框架，通过闭式联合尺度优化和分离尺度搜索，在不增加硬件开销的前提下显著提升LLM的4-bit量化精度。


<details>
  <summary>Details</summary>
Motivation: 现有NVFP4方法因尺度选择不灵活以及量化与反量化尺度耦合处理导致性能次优，亟需一种能联合优化尺度并解耦精度损失的量化方案。

Method: 提出SOAR框架，核心包括：1) 闭式联合尺度优化(CJSO)，通过最小化重构误差的解析解联合优化全局和块级尺度；2) 分离尺度搜索(DSS)，将高精度量化尺度与其受限的反量化尺度解耦，并通过离散搜索减少尺度量化的精度损失。

Result: 在多个LLM上的实验表明，SOAR在相同内存占用且无额外硬件开销的条件下，准确率一致优于现有NVFP4量化基线。

Conclusion: SOAR有效解决了NVFP4量化的尺度优化问题，实现了更高的量化精度，代码和模型将开源。
...
### [281] [Delay-Empowered Causal Hierarchical Reinforcement Learning](https://arxiv.org/abs/2605.12261)
*Chenran Zhao,Dianxi Shi,Haotian Wang,Mengzhu Wang,Yaowen Zhang,Chunping Qiu,Shaowu Yang*

Main category: cs.LG

TL;DR: 本文提出延迟赋能因果分层强化学习（DECHRL），通过显式建模因果结构与随机延迟分布，在时间不确定性下显著提升决策性能。


<details>
  <summary>Details</summary>
Motivation: 现有延迟感知强化学习方法需状态增强、先验知识或非延迟数据，泛化受限；分层方法仅处理固定延迟，二者均无法应对随机延迟的挑战。

Method: 提出DECHRL，显式建模状态转移的因果结构与随机延迟分布，并将其融入延迟感知的赋能目标，驱动智能体主动探索高可控状态。

Result: 在引入随机延迟的2D-Minecraft和MiniGrid环境中，DECHRL有效建模时间延迟，决策性能显著优于基线方法。

Conclusion: DECHRL通过结合因果建模与延迟感知赋能，有效克服时间不确定性，为延迟环境下的分层强化学习提供了通用且高效的解决方案。
...
### [282] [Hypernetworks for Dynamic Feature Selection](https://arxiv.org/abs/2605.12278)
...
### [283] [PriorZero: Bridging Language Priors and World Models for Decision Making](https://arxiv.org/abs/2605.12289)
*Junyu Xiong,Yuan Pu,Jia Tang,Yazhe Niu*

Main category: cs.LG

TL;DR: PriorZero通过解耦的 rollout-训练设计，将LLM先验仅注入MCTS根节点以引导探索，并利用世界模型的价值估计为LLM提供稳定微调信号，从而在长程决策任务中同时提升探索效率与渐进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在先验-动力学不匹配问题：静态LLM知识无法适应环境复杂转移，固定先验限制探索多样性，端到端微调面临优化不稳定和信用分配困难。

Method: 提出PriorZero框架，采用解耦的 rollout-训练设计。rollout阶段，通过根先验注入机制仅在MCTS根节点融入LLM先验，聚焦语义上有前景的动作，同时保留世界模型的深度前瞻能力；训练阶段，解耦世界模型学习与LLM适应：用交互数据在线优化世界模型的动力学、策略和价值预测，并利用价值估计为LLM提供细粒度信用分配信号，通过交替优化实现稳定微调。

Result: 在Jericho文字冒险游戏和BabyAI指令遵循网格世界等多样化基准上，PriorZero一致提升了探索效率和渐进性能。

Conclusion: PriorZero为LLM赋能的决策制定建立了一个有前景的统一框架。
...
### [284] [Targeted Neuron Modulation via Contrastive Pair Search](https://arxiv.org/abs/2605.12290)
...
### [285] [STRABLE: Benchmarking Tabular Machine Learning with Strings](https://arxiv.org/abs/2605.12292)
...
### [286] [Approximation of Maximally Monotone Operators : A Graph Convergence Perspective](https://arxiv.org/abs/2605.12301)
...
### [287] [KAN-CL: Per-Knot Importance Regularization for Continual Learning with Kolmogorov-Arnold Networks](https://arxiv.org/abs/2605.12306)
*Minjong Cheon*

Main category: cs.LG

TL;DR: KAN-CL uses Kolmogorov-Arnold Networks as a continual learning head with per-knot importance weighting, combined with backbone EWC, achieving up to 93% forgetting reduction while matching or exceeding accuracy on Split-CIFAR benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing continual learning regularizers apply uniform penalties without awareness of which input region each parameter serves, limiting their ability to prevent catastrophic forgetting.

Method: The proposed KAN-CL framework exploits the compact-support spline parameterization of KANs to perform importance-weighted anchoring at per-knot granularity, deployed as a classification head on a convolutional backbone with standard EWC regularization (bbEWC).

Result: On Split-CIFAR-10/5T and Split-CIFAR-100/10T, KAN-CL reduces forgetting by 88% and 93% over a head-only KAN baseline, matching or exceeding all baselines in accuracy. An NTK analysis reveals a structural rank deficit in cross-task NTK, yielding a forgetting bound in the feature-learning regime.

Conclusion: Combining an architecture with natural parameter locality (KAN head) and a complementary backbone regularizer (bbEWC) yields a compositional and principled solution to catastrophic forgetting.
...
### [288] [In-context learning to predict critical transitions in dynamical systems](https://arxiv.org/abs/2605.12308)
...
### [289] [Transferable Delay-Aware Reinforcement Learning via Implicit Causal Graph Modeling](https://arxiv.org/abs/2605.12312)
*Chenran Zhao,Dianxi Shi,Yaowen Zhang,Chunping Qiu,Shaowu Yang*

Main category: cs.LG

TL;DR: 本文提出一种基于隐式因果图建模的可迁移延迟感知强化学习方法，通过结构化表征和环境动力学建模解决随机延迟下的跨任务迁移问题。


<details>
  <summary>Details</summary>
Motivation: 随机延迟弱化动作与状态反馈的时间对应关系，使智能体难以识别动作效果的传播过程；跨任务场景下目标与奖励形式的变化进一步降低了已学知识的可复用性。

Method: 使用域节点编码器将高维观测表征为具有节点级语义的潜在状态，通过消息传递机制建模节点间的动态因果依赖，学习可迁移的结构化表征和环境动力学知识；在此基础上引入想象力驱动的行为学习与规划，在潜在空间中优化策略。

Result: 在具有随机延迟的DMC连续控制任务上，所提方法优于基线方法；跨任务迁移实验表明，学到的结构化表征和动力学知识可有效迁移至新任务，并显著加速策略适应。

Conclusion: 该方法能够学习可迁移的延迟感知表征与动力学知识，有效实现跨任务知识迁移和快速策略适应。
...
### [290] [Autoregressive Learning in Joint KL: Sharp Oracle Bounds and Lower Bounds](https://arxiv.org/abs/2605.12316)
...
### [291] [Grid Games: The Power of Multiple Grids for Quantizing Large Language Models](https://arxiv.org/abs/2605.12327)
...
### [292] [Manifold Sampling via Entropy Maximization](https://arxiv.org/abs/2605.12338)
...
### [293] [BSO: Safety Alignment Is Density Ratio Matching](https://arxiv.org/abs/2605.12339)
*Tien-Phat Nguyen,Truong Nguyen,Thin Nguyen,Duy Minh Ho Nguyen,Ngoc-Thanh Dinh,Trung Le*

Main category: cs.LG

TL;DR: 本文提出一种基于Bregman散度的单阶段安全对齐方法（BSO），通过密度比匹配将安全对齐转化为统一损失函数族，无需辅助模型，仅需一个额外超参数，可恢复最优安全策略并简化训练流程。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型的安全对齐方法要么依赖复杂的独立奖励/成本模型、在线强化学习和原始-对偶更新，要么在直接偏好优化中采用多阶段或启发式边界等临时修改，缺乏原则性推导。

Method: 证明最优安全策略的似然比具有闭式分解，将安全对齐归结为密度比匹配问题。通过最小化数据与模型比率之间的Bregman散度，得到一族单阶段损失函数（BSO），每个由凸生成器诱导，且理论上可恢复最优安全策略。BSO无需辅助模型，仅引入一个超参数，现有安全感知方法为其特例。

Result: 在安全对齐基准上的实验表明BSO能持续改善安全性与有用性的权衡。

Conclusion: BSO为安全对齐提供了通用、简单且原则性的方案，在实验中一致提升安全-有用性平衡。
...
### [294] [Neural-Schwarz Tiling for Geometry-Universal PDE Solving at Scale](https://arxiv.org/abs/2605.12343)
...
### [295] [A New Technique for AI Explainability using Feature Association Map](https://arxiv.org/abs/2605.12350)
...
### [296] [From Message-Passing to Linearized Graph Sequence Models](https://arxiv.org/abs/2605.12358)
...
### [297] [MetaColloc: Optimization-Free PDE Solving via Meta-Learned Basis Functions](https://arxiv.org/abs/2605.12368)
*Zichuan Yang*
...
### [298] [Agent-Based Post-Hoc Correction of Agricultural Yield Forecasts](https://arxiv.org/abs/2605.12375)
*Matthew Beddows,Aiden Durrant,Georgios Leontidis*

Main category: cs.LG

TL;DR: 提出一种利用结构化LLM智能体框架对作物产量预测进行事后修正的方法，通过领域知识工具改进基线模型，在草莓和玉米数据集上有效降低误差。


<details>
  <summary>Details</summary>
Motivation: 商业软水果生产的精准产量预测受限于典型农场记录中缺乏传感器网络、卫星影像和高分辨率气象输入，现有先进方法难以适用。

Method: 构建结构化LLM智能体框架，集成物候检测、偏差学习和范围校验等工具编码农业领域知识，对XGBoost、Moirai2和Random Forest等基线模型的预测进行事后修正。采用Llama 3.1 8B和LLaVA 13B作为智能体，在专有草莓产量数据集和公开USDA玉米收获数据集上评估。

Result: 智能体修正使XGBoost在草莓数据上的MAE降低20%、MASE降低56%；Moirai2的MAE降低24%、MASE降低22%；Random Forest的MAE降低28%、MASE降低66%。Llama 3.1 8B在所有配置中修正效果最强，而LLaVA 13B表现不稳定，表明修正效果对模型选择敏感。

Conclusion: 基于领域知识工具的LLM智能体框架能有效提升产量预测准确性，但需谨慎选择修正模型，Llama 3.1 8B表现最优。
...
### [299] [Discrete Flow Matching for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.12379)
*Fairoz Nower Khan,Nabuat Zaman Nahim,Peizhong Ju*

Main category: cs.LG

TL;DR: DRIFT是一种在线微调方法，通过优势加权离散流匹配损失更新预训练连续时间马尔可夫链策略，并使用路径空间惩罚和候选集近似，实现离散动作强化学习的稳定离线到在线提升。


<details>
  <summary>Details</summary>
Motivation: 现有生成式策略方法多针对连续控制，而许多RL任务具有离散动作空间；生成策略依赖离线数据，离线到在线强化学习需在不丢失有用行为的前提下通过交互改进策略，挑战较大。

Method: 提出DRIFT：利用优势加权离散流匹配损失更新预训练CTMC策略；添加路径空间惩罚正则化完整轨迹分布以保留知识；对大规模离散动作空间，从参考策略和均匀探索中采样动作候选集近似更新；理论分析表明候选集误差受缺失目标概率质量控制，生成器误差随覆盖度指数下降。

Result: 在离散动作RL任务（如Jericho）上，DRIFT在所有任务中稳定提升，使用简单GRU编码器取得最高平均分，超越基于预训练语言模型的方法；控制实验证明路径空间惩罚有界，CTMC生成器比确定性基线更快适应奖励变化；候选集稳定性分析显示生成器误差随覆盖率指数下降。

Conclusion: 该方法通过CTMC、流匹配和路径空间正则化，有效应对离散动作离线到在线RL挑战，兼具理论保障和实验优越性。
...
### [300] [Trust the Batch, On- or Off-Policy: Adaptive Policy Optimization for RL Post-Training](https://arxiv.org/abs/2605.12380)
*Rasool Fakoor,Murdock Aubry,Nicholas Stranges,Alexander J. Smola*

Main category: cs.LG

TL;DR: 本文提出一种批量自适应的强化学习目标，用策略比率的归一化有效样本量替代固定裁剪，无需新增超参数即可自动适应信任域和离策略程度，实验表明其性能匹配或超越调优的基线。


<details>
  <summary>Details</summary>
Motivation: 强化学习中策略更新会改变数据分布，导致训练脆弱，尤其在大模型训练时因实现细节差异（精度、采样等）而加剧。现有方法通过添加超参数来管理这种脆弱性，但导致算法对配置敏感，每次任务、规模或分布不匹配变化时都需重新调参。

Method: 提出一种批量自适应目标，将固定裁剪替换为基于当前批次策略比率的归一化有效样本量。该统计量同时用于截断得分函数权重和调节离策略正则化强度，使得更新在比率接近均匀时保持近策略更新，当陈旧或不匹配数据导致比率集中时自动收紧，同时保留高比率令牌上的非零学习信号。

Result: 在广泛设置下的实验表明，该方法匹配或超越经过调优的基线，没有引入新的目标超参数，并移除了若干现有超参数。

Conclusion: 该方法通过自适应机制解耦信任域和离策略关注，消除了预设超参数的需求，在不同分布不匹配场景下均能稳定有效训练，简化了强化学习调参流程。
...
### [301] [Detecting overfitting in Neural Networks during long-horizon grokking using Random Matrix Theory](https://arxiv.org/abs/2605.12394)
...
### [302] [OGLS-SD: On-Policy Self-Distillation with Outcome-Guided Logit Steering for LLM Reasoning](https://arxiv.org/abs/2605.12400)
...
### [303] [Aligning Flow Map Policies with Optimal Q-Guidance](https://arxiv.org/abs/2605.12416)
*Christos Ziakas,Alessandra Russo,Avishek Joey Bose*

Main category: cs.LG

TL;DR: 本文提出流映射策略（flow map policies），通过学习在流基策略生成动力学中跳跃任意步数（包括单步）实现快速动作生成，并引入FMQ和QGBS方法，在离线到在线强化学习任务中显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: 基于扩散和流匹配的生成式策略虽然适合多模态动作分布，但推理时需多步采样，导致高延迟，限制实时决策。因此需要一种既能保持高表达力又能实现快速动作生成的策略。

Method: 提出流映射策略，将动作生成建模为从基分布到目标分布的任意步跳跃（包括一步）；针对离线到在线强化学习，将在线适应形式化为信任域优化问题，并推导出闭式学习目标FMQ（Flow Map Q-Guidance）；进一步提出QGBS（Q-Guided Beam Search）采样器，通过重噪与束搜索实现推理时迭代优化。

Result: 在OGBench和RoboMimic的12个机器人操作与运动任务上，FMQ达到离线到在线RL的最优水平，相比先前单步策略MVP平均成功率相对提升21.3%。

Conclusion: 流映射策略成功降低了生成动作的推理延迟，FMQ与QGBS的结合在离线到在线RL中表现出色，证明了该方法在高效、高质动作生成上的优越性。
...
### [304] [Learning Minimally Rigid Graphs with High Realization Counts](https://arxiv.org/abs/2605.12427)
...
### [305] [Environment-Adaptive Preference Optimization for Wildfire Prediction](https://arxiv.org/abs/2605.12435)
*Enyi Jiang,Wu Sun*
...
TL;DR: 本文提出ScaleSearch策略，通过细粒度搜索微缩放格式中的尾数位来优化量化尺度因子，减少量化误差，并集成到注意力和后训练量化中，显著提升语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 标准块浮点（BFP）的固定尺度（基于块内最大幅度）在量化误差方面可能次优，需要更优的尺度选择方法。

Method: 使用ScaleSearch在微缩放格式中利用尾数位进行细粒度搜索，选择最小化量化误差的尺度因子；并提出ScaleSearchAttention，一种基于NVFP4的加速注意力算法。

Result: ScaleSearch使NVFP4量化误差降低27%，在MATH500（Qwen3-8B）上PTQ性能提升达15分；ScaleSearchAttention在Llama 3.1 70B上Wikitext-2 PPL改善最多0.77分，性能损失接近零。

Conclusion: 所提方法在保持基线性能的同时，有效提高量化精度，减少误差，对语言模型生成任务无显著性能损失。
...
### [308] [Beyond GRPO and On-Policy Distillation: An Empirical Sparse-to-Dense Reward Principle for Language-Model Post-Training](https://arxiv.org/abs/2605.12483)
*Yuanda Xu,Hejian Sang,Zhengze Zhou,Ran He,Zhipeng Wang,Alborz Geramifard*

Main category: cs.LG

TL;DR: 提出奖励密度原则：稀缺验证数据应用于强教师模型进行稀疏RL探索，再通过密集蒸馏桥接到学生模型，避免直接对学生GRPO的低效分配，显著提升数学推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准做法将稀缺标注数据直接用于部署学生模型（如GRPO），忽略了奖励密度原则：稀疏序列级奖励应训练探索性强的模型，密集令牌级教师奖励应用于压缩行为到小模型；直接对学生GRPO属低效分配，需重新设计数据使用策略。

Method: 采用‘稀疏-密集-稀疏’流程：（1）在最大教师模型上用GRPO式稀疏RL进行行为发现；（2）通过‘密集桥接’将教师行为迁移到小型学生：先使用教师rollout做前向KL预热，再用OPD（在线策略蒸馏）对学生rollout进行令牌级密集监督；（3）桥接后可选择在学生侧进行稀疏RL微调（如GRPO）。

Result: 固定Qwen3-1.7B学生时，RL增强的8B教师通过密集桥接蒸馏，表现优于直接对学生GRPO；桥接（KL预热+OPD）在MATH任务上预桥接最强，并给出8B/14B教师最佳pre-Stage 3 AIME结果；桥接后的学生侧GRPO将MATH从75.4%提升至78.5%，比匹配重放控制高2.8个点。

Conclusion: 操作原则：稀缺标注数据只能用于教师侧稀疏奖励发现与密集迁移压缩，学生侧稀疏奖励仅在桥接后引入，避免在准备最不充分的策略上浪费数据。
...
### [309] [Learning, Fast and Slow: Towards LLMs That Adapt Continually](https://arxiv.org/abs/2605.12484)
...
### [310] [Pion: A Spectrum-Preserving Optimizer via Orthogonal Equivalence Transformation](https://arxiv.org/abs/2605.12492)
...
### [311] [A Cascaded Generative Approach for e-Commerce Recommendations](https://arxiv.org/abs/2605.11118)
*Moein Hasani,Hamidreza Shahidi,Trace Levinson,Yuan Zhong,Guanghua Shu,Vinesh Gudla,Tejaswi Tenneti*

Main category: cs.AI

TL;DR: 本文提出一种级联式商品化框架，通过主题生成与约束关键词生成驱动产品检索，并结合教师-学生微调、AI内容评估与混合排序，在线上实验中将每页面浏览的加购量提升2.7%。


<details>
  <summary>Details</summary>
Motivation: 传统个性化店面由静态主题、检索系统和排序器独立组合而成，缺乏语义连贯性，难以适应动态商品需求。

Method: 方法包括两级生成任务：展位级主题生成和约束关键词生成，以驱动产品检索；利用教师-学生微调实现低延迟部署；构建AI驱动的内容评估与质量过滤体系；生成式输出与传统排序模型融合，形成混合架构。

Result: 线上A/B测试显示，相比强基线，每页面浏览加购量提升2.7%；消融实验表明微调模型接近闭源大模型性能。

Conclusion: 该框架有效提升个性化与页面语义一致性，支持动态内容的安全规模化部署，并通过混合基础设施带来显著商业收益。
...
### [312] [EVOCHAMBER: Test-Time Co-evolution of Multi-Agent System at Individual, Team, and Population Scales](https://arxiv.org/abs/2605.11136)
*Yaolun Zhang,Tianyi Xu,Shengyu Dai,Zhenwen Shao,Qingyun Wu,Huazheng Wang*

Main category: cs.AI

TL;DR: EVOCHAMBER 是一个无需训练的测试时多智能体演化框架，通过不对称协作反思（CODREAM）和群体层级操作，在数学、代码和推理任务上显著超越基线，自发生成专业化智能体，验证了不对称跨智能体知识迁移的关键作用。


<details>
  <summary>Details</summary>
Motivation: 多智能体测试时演化并非单智能体演化的简单并行；现有方法或限制个体学习导致失去跨智能体收益，或对称广播消除专业化，无法利用协作中的不对称知识流动来产生涌现专业化。

Method: 提出 EVOCHAMBER，在共同演化的智能体池上实现三级测试时演化：核心为 CODREAM 协议，当团队失败或分歧时触发，让智能体协作反思并将洞察不对称地从强智能体路由至弱智能体，保留专业化并填补知识缺口；团队级操作在线组装小生境团队并选择协作结构；群体级生命周期操作包括分叉、合并、修剪和播种智能体。

Result: 在三个异构任务流上，基于 Qwen3-8B 的 EVOCHAMBER 分别达到竞赛数学 63.9%、代码 75.7% 和多领域推理 87.1% 的准确率，相对最佳基线数学提升 32%；从相同初始化开始自发涌现 4–5 个稳定的小生境专家。

Conclusion: 多智能体演化产生的专业化结构是单智能体无法表达的，不对称跨智能体迁移是性能提升的主要驱动力，证明框架无需训练即可有效实现群体级演化与知识转移。
...
### [313] [RankQ: Offline-to-Online Reinforcement Learning via Self-Supervised Action Ranking](https://arxiv.org/abs/2605.11151)
*Andrew Choi,Wei Xu*

Main category: cs.AI

TL;DR: RankQ 是一种离线到在线强化学习方法，通过自监督排序损失学习动作的相对偏好，而非简单惩罚分布外动作，从而在稀疏奖励和机器人任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线到在线 RL 方法通过悲观惩罚分布外动作来防止值高估，但这本质上是一种行为克隆锚定，当离线数据动作次优时会阻碍在线策略改进。

Method: 提出 RankQ，在时序差分学习基础上增加一个自监督多术语排序损失，让 Q 函数学习结构化动作排序，使动作梯度朝向更高质量行为，而非统一惩罚未见动作。

Result: 在 D4RL 稀疏奖励基准上达到与或超越七种现有方法的性能；在基于视觉的机器人学习中，低数据下仿真成功率平均比次优方法高 42.7%，高数据下仿真性能提升 13.7%，真实世界立方体堆叠成功率从 43.1% 提升至 84.7%。

Conclusion: RankQ 通过动作排序学习有效提升了离线到在线 RL 的样本效率和最终策略质量，在标准基准和机器人操作任务中均展现了显著优势。
...
### [314] [OLIVIA: Online Learning via Inference-time Action Adaptation for Decision Making in LLM ReAct Agents](https://arxiv.org/abs/2605.11169)
*Sheldon Yu,Junda Wu,Xintong Li,Nikki Lijing Kuang,Sizhe Zhou,Tong Yu,Jiawei Han,Jingbo Shang,Julian McAuley*

Main category: cs.AI

TL;DR: OLIVIA提出一种推理时动作自适应框架，将LLM的动作选择层建模为上下文线性赌博机，利用上置信界探索和轻量在线更新，以极小计算开销提升ReAct智能体部署中的任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的推理时适应方法主要依赖提示或检索，缺乏显式的决策层来评分动作、表示不确定性或在线更新，导致部署中微小的动作选择错误逐渐累积，无法实现可追踪、细粒度且不确定性感知的适应。

Method: OLIVIA将LLM的最终动作选择层视为候选动作上的上下文线性赌博机，以冻结的隐藏状态作为决策上下文，使用上置信界探索实现样本高效的策略改进，支持轻量级在线更新并提供显式的不确定性估计。

Result: 在四个基准上，OLIVIA的任务性能一致优于静态ReAct以及基于提示的推理时基线方法。

Conclusion: 显式在线决策层为LLM智能体在部署期间的适应提供了有效替代方案，弥补了纯提示或检索方法在可追踪性、细粒度和不确定性意识方面的不足。
...
### [315] [The Many Faces of On-Policy Distillation: Pitfalls, Mechanisms, and Fixes](https://arxiv.org/abs/2605.11182)
...
### [316] [Don't Look at the Numbers: Visual Anchoring Bias and Layer-wise Representation in VLMs](https://arxiv.org/abs/2605.11218)
*M. Shalankin*
...
### [317] [Do Vision-Language-Models show human-like logical problem-solving capability in point and click puzzle games?](https://arxiv.org/abs/2605.11223)
...
### [318] [PIVOT: Bridging Planning and Execution in LLM Agents via Trajectory Refinement](https://arxiv.org/abs/2605.11225)
*Tuo Zhang,Alin-Ionut Popa,Yan Xu,Rui Song,Dimitrios Dimitriadis*

Main category: cs.AI

TL;DR: PIVOT是一个自监督框架，通过环境交互迭代优化代理轨迹，解决LLM代理的计划-执行不一致问题，在约束满足和效率上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: LLM代理生成看似连贯但执行时因不可行动作、约束违反和长期误差而失败的计划。

Method: 四阶段：PLAN生成候选轨迹；INSPECT执行并计算结构化损失及文本梯度；EVOLVE利用反馈改进轨迹；VERIFY最终校验。采用单调接受过程保证解质量不降。

Result: 在DeepPlanning和GAIA上达SOTA：有人类反馈时约束满足相对提升高达94%，自主变体仍保持显著收益，且Token使用量比竞争方法少3-5倍。

Conclusion: 基于反馈的轨迹优化是缓解自主代理系统计划-执行差距的原则性方法。
...
### [319] [Rethinking LLMOps for Fraud and AML: Building a Compliance-Grade LLM Serving Stack](https://arxiv.org/abs/2605.11232)
...
### [320] [The Semantic Training Gap: Ontology-Grounded Tool Architectures for Industrial AI Agent Systems](https://arxiv.org/abs/2605.11234)
*Grama Chethan*

Main category: cs.AI

TL;DR: 本文指出大语言模型智能体在制造业中缺乏对操作语义的深度理解，通过将制造本体嵌入AI工具层，在运行时强制语义约束，成功将领域标识符的幻觉率从43%降至0%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体虽具备统计流畅性，但缺乏对设备标识、工艺参数、故障代码等制造操作语义的关系性理解，导致在多智能体场景下产生语义漂移，输出操作上错误的结果。

Method: 提出一种将制造本体作为类型化关系配置直接嵌入AI工具层的架构，通过解析、语境化、标注三个接口契约，由AIOps编排层在运行时强制执行语义约束，而非依赖模型训练。

Result: 在6种工业配置的对照实验中（使用Qwen3-32B，72次工具调用），无约束的参数产生了43%的领域标识符幻觉，而本体约束的参数将幻觉率降至0%；通过数字孪生分析平台验证了跨域可配置性。

Conclusion: 通过领域本体配置可实现无需修改应用代码的跨域可配置性，并消除工具调用的幻觉，为制造环境中可靠的多智能体系统提供了有效方法。
...
### [321] [Unlocking LLM Creativity in Science through Analogical Reasoning](https://arxiv.org/abs/2605.11258)
...
### [322] [Template-as-Ontology: Configurable Synthetic Data Infrastructure for Cross-Domain Manufacturing AI Validation](https://arxiv.org/abs/2605.11259)
*Grama Chethan*

Main category: cs.AI

TL;DR: 本文提出Template-as-Ontology原则，用单个Python配置模块同时作为制造仿真器和AI分析工具的规范，确保架构对齐。五层流水线生成符合MES模式的数据，实验表明参数可控且本体约束消除AI工具参数虚构。


<details>
  <summary>Details</summary>
Motivation: 制造环境中基于大语言模型的AI代理需要填充且模式正确的数据进行验证，但实际MES数据具有专有性、隐私约束和供应商特异性。

Method: 引入Template-as-Ontology原则：单个Python配置模块（700-770行，45个验证导出）同时作为时间步进制造仿真器的规范和AI分析工具的运行时域模式，通过单源消费保证结构对齐。形式化域模板为类型化关系配置模式并证明对齐保证。构建五层流水线：仿真、PostgreSQL、CDC/Iceberg湖仓、星型模式、12个参数化AI工具，生成涵盖66种实体类型、四个ISA-95域的数据。用六个行业模板在相同框架代码上验证架构。

Result: 校准实验（60次运行，每个模板10个种子）确认参数可控性：所有模板观测KPI落在配置范围内。受控幻觉实验（72次工具调用，Qwen3-32B）表明本体约束参数消除工具参数虚构（约束组0% vs 非约束组43%幻觉率，Fisher精确检验p<10^-12），约束组0%为架构保证，适用于任何模型。

Conclusion: 该框架为离散制造AI验证提供了可复用的数据层。
...
### [323] [LatentRouter: Can We Choose the Right Multimodal Model Before Seeing Its Answer?](https://arxiv.org/abs/2605.11301)
*Xueqi Cheng,Yushun Dong*

Main category: cs.AI

TL;DR: LatentRouter 提出一种基于反事实多模态效用预测的 MLLM 路由方法，通过多模态路由胶囊与模型能力令牌的潜在通信估计各候选模型的质量，并在基准测试中超越固定模型、特征级和已有路由方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在 OCR、图表理解、空间推理、视觉问答等方面的能力异质，且还需权衡成本与延迟，因此路由不仅要估算查询难度，更要匹配当前图像-问题输入的多模态需求与各模型能力。

Method: LatentRouter 将 MLLM 路由建模为反事实多模态效用预测：抽取多模态路由胶囊，用模型能力令牌表示候选模型，通过潜在通信估计各模型被选中时的表现；分布结果头预测模型特定反事实质量，并用有界胶囊校正精调边界决策，支持性能导向与性能-成本路由，通过共享单模型评分和可用性掩码应对候选池变化。

Result: 在 MMR-Bench 和 VL-RouterBench 上，LatentRouter 优于固定模型、特征级路由和学习路由器基线；增益在依赖视觉、布局敏感或推理需求的多模态任务组上最为显著，潜在通信是主要改进来源。

Conclusion: LatentRouter 有效捕捉多模态查询与模型能力的匹配关系，通过潜在通信实现精确的效用预测，显著提升路由性能，尤其在复杂多模态任务中，且具备灵活的候选池管理能力。
...
### [324] [Constraint-Data-Value-Maximization: Utilizing Data Attribution for Effective Data Pruning in Low-Data Environments](https://arxiv.org/abs/2605.11312)
...
### [325] [Rethinking Evaluation for LLM Hallucination Detection: A Desiderata, A New RAG-based Benchmark, New Insights](https://arxiv.org/abs/2605.11330)
...
### [326] [CPEMH: An Agentic Framework for Prompt-Driven Behavior Evaluation and Assurance in Foundation-Model Systems for Mental Health Screening](https://arxiv.org/abs/2605.11341)
*Giuliano Lorenzoni,Ivens Portugal,Paulo Alencar,Donald Cowan*

Main category: cs.AI

TL;DR: CPEMH是一个用于在心理健康筛查转录数据集上评估基础模型提示驱动行为的代理框架，通过编排架构系统性地控制行为变异性，在抑郁症筛查案例中展示了稳定性和可审计性。


<details>
  <summary>Details</summary>
Motivation: 在心理健康等临床敏感领域应用大型语言模型时，需要行为保证方法来控制系统变异性，确保可重复性和鲁棒性。

Method: 提出了一种编排式代理架构，包含编排器、推理和评估代理，能自主设计、评估和选择提示策略；通过基于访谈转录的自动抑郁症筛查案例研究验证框架。

Result: 该框架成功稳定并审计了模型行为，获得的经验包括：模块化编排对行为保证的关键作用、稳定性优先于架构复杂性、以及将F1分数、偏差和鲁棒性作为核心验收标准。

Conclusion: 模块化编排是实现行为保证的有效手段，应优先重视稳定性而非复杂性，并将F1、偏差和鲁棒性整合为接受标准，该框架为敏感领域的基础模型行为审计提供了方法论支持。
...
### [327] [CVEvolve: Autonomous Algorithm Discovery for Unstructured Scientific Data Processing](https://arxiv.org/abs/2605.11359)
*Ming Du,Xiangyu Yin,Yanqi Luo,Dishant Beniwal,Songyuan Tang,Hemant Sharma,Mathew J. Cherukara*

Main category: cs.AI

TL;DR: CVEvolve is a zero-code autonomous agent that helps domain scientists discover image processing algorithms, improving over baseline methods and identifying generalizable solutions via holdout testing.


<details>
  <summary>Details</summary>
Motivation: Domain scientists face barriers in analyzing scientific data due to lack of computing expertise, especially for noisy, high-dynamic-range, sparsely labeled, or loosely specified data, requiring task-specific algorithms or AI models.

Method: CVEvolve, an autonomous agentic harness with zero-code interface, uses a multi-round search alternating between discovery and improvement actions, lineage-aware stochastic candidate sampling to balance exploration and exploitation, and integrates tools for code execution, evaluation, history management, holdout testing, and optional data inspection.

Result: On X-ray fluorescence microscopy registration, Bragg peak detection, and high-energy diffraction microscopy segmentation, CVEvolve discovered algorithms outperforming baselines; holdout testing identified candidates generalizing better than later over-optimized alternatives.

Conclusion: Zero-code, LLM-powered autonomous algorithm development can empower domain scientists to transform unstructured scientific image data into practical, generalizable algorithms, accelerating scientific discovery.
...
### [328] [Causal Bias Detection in Generative Artifical Intelligence](https://arxiv.org/abs/2605.11365)
*Drago Plecko*
...
### [329] [Causal Algorithmic Recourse: Foundations and Methods](https://arxiv.org/abs/2605.11373)
...
### [330] [LLM-X: A Scalable Negotiation-Oriented Exchange for Communication Among Personal LLM Agents](https://arxiv.org/abs/2605.11376)
*Giuliano Lorenzoni,Paulo Alencar,Donald Cowan*

Main category: cs.AI

TL;DR: LLM-X是一种可扩展的协商环境，通过消息总线和路由基板实现个人LLM代理间的直接结构化通信，支持策略执行和模式验证，实验表明严格策略提升稳健性与公平性但增加延迟和消息量，系统在持续负载下保持稳定。


<details>
  <summary>Details</summary>
Motivation: 现有工具中心型协议聚焦于代理与API交互，缺乏LLM间直接协调机制。本文旨在构建一个面向个人LLM代理群体的可扩展协商环境，以支持结构化通信、模式有效性和策略执行。

Method: 提出LLM-X架构，包含联邦网关、基于主题的路由和策略执行；设计类型化消息协议，支持能力协商和合同网式协调；首次对LLM多代理协商进行大规模实证评估，实验涉及5、9、12个代理，采用低、中、高三种协商策略，并在短期（分钟）和长期（2小时、12小时）负载条件下测试。

Result: 严格策略提升了协商的稳健性和公平性，但增加了延迟和消息量；长期实验证实LLM-X在持续负载下保持稳定，延迟漂移有界。

Conclusion: LLM-X为个人LLM提供了一种稳定的规模化协商框架，策略严格性与性能之间存在权衡，未来可在此基础上进一步优化。
...
### [331] [Revisiting Privacy Preservation in Brain-Computer Interfaces: Conceptual Boundaries, Risk Pathways, and a Protection-Strength Grading Framework](https://arxiv.org/abs/2605.11386)
...
### [332] [Transformer Interpretability from Perspective of Attention and Gradient](https://arxiv.org/abs/2605.11392)
...
### [333] [AcuityBench: Evaluating Clinical Acuity Identification and Uncertainty Alignment](https://arxiv.org/abs/2605.11398)
...
TL;DR: 提出AcuityBench基准，统一评估语言模型在用户医疗描述中识别紧急程度的能力，涵盖确定性病例和模糊病例，并比较问答与对话两种任务格式，发现模型在模糊病例中与医生判断分布不符，且存在系统性的过诊/漏诊权衡。


<details>
  <summary>Details</summary>
Motivation: 现有健康基准侧重医疗问答、宽泛交互或狭窄分诊，缺乏跨场景统一评估紧急程度识别的能力，需填补此空白。

Method: 整合五个公开数据集（用户对话、论坛帖子、临床案例、患者门户消息），采用四级紧急框架（居家监测至立即急诊），构建914例（697例共识病例+217例医生确认模糊病例）的基准；支持明确四分类问答和自由对话两种评估方式，并以病例级别评级基准进行评分，在12个前沿模型上测试。

Result: 明确病例的紧急程度准确率和错误方向差异显著；对话形式相比问答减少过度分诊但增加低诊，尤其在高紧急度病例中；模糊病例上没有模型逼近医生判断分布，模型预测更集中；还在最大模糊病例子集上比较了专家和模型的判定差异。

Conclusion: 紧急程度识别是独特的安全关键能力，AcuityBench能够系统比较和压力测试模型在真实健康场景中引导用户至适当护理级别的表现。
...
### [334] [Attributing Emergence in Million-Agent Systems](https://arxiv.org/abs/2605.11404)
*Ling Tang,Jilin Mei,Qian Chen,Qihan Ren,Linfeng Zhang,Quanshi Zhang,Jing Shao,Xia Hu,Dongrui Liu*

Main category: cs.AI

TL;DR: 本文将Aumann-Shapley路径积分归因方法扩展到百万级智能体的LLM多智能体系统，证明小规模研究存在结构性归因偏差，并给出一个归因缩放偏差定理，表明对任何非线性宏观指标必须进行全尺度分析。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多智能体系统的归因方法仅适用于N≲10³的规模，而待解释的涌现现象发生在N≥10⁶，这种尺度差距可能导致错误的归因结论。

Method: 将Aumann-Shapley路径积分归因适配到百万智能体规模的LLM多智能体系统，该方法满足四条公理且速度比采样Shapley快4-5个数量级；使用Bluesky平台14天内1,671,587名活跃用户数据，对比全尺度与基于可见性偏差抽取的100个用户子集的归因；证明了一个归因缩放偏差定理。

Result: 全尺度归因表明长尾和中间层共同承载了大部分宏观效应，而有偏的小样本几乎将所有效应归因于少数高粉丝账户；定理证明对于任何非线性宏观指标，无法通过事后全局重标定因子来调和大小尺度的归因分歧。

Conclusion: 全尺度归因不是方法选择，而是对任何非线性宏观指标的理论必需。
...
### [335] [What Do EEG Foundation Models Capture from Human Brain Signals?](https://arxiv.org/abs/2605.11410)
*Ling Tang,Qian Chen,Jilin Mei,Houshi Xu,Quanshi Zhang,Jing Shao,Na Zou,Xia Hu,Dongrui Liu*

Main category: cs.AI

TL;DR: 本研究通过层间岭回归探测、LEACE风格交叉协方差子空间消除和透明分类器与随机特征基线的对比，审计了三个EEG基础模型在五个临床任务上与63个手工特征的对齐情况。发现68.6%的特征单元具有表示因果性，21.1%仅被编码；50个通用候选特征在所有模型和至少两个任务中表现出因果作用，频域特征占主导但其他家族也贡献显著因果质量。已确认特征平均恢复基础模型相对于随机基线79.3%的优势，并呈现明显的任务梯度（MDD≈0.99至Stress≈0.56），近性能上限的任务几乎被完全解释，而高难度任务留有非平凡残差，为未来概念发现指明方向。


<details>
  <summary>Details</summary>
Motivation: 临床脑电图分析长期依赖手工精炼的特征库，现代EEG基础模型则绕过该库直接从原始信号自监督学习并达到可比或更优性能，但两者表征是否一致仍是一个开放问题。本文旨在系统回答“模型学到了什么、用了什么、能被解释多少”，从而弥合传统特征与深度学习表征之间的理解鸿沟。

Method: 采用分层岭回归探测评估特征在各层的编码程度，LEACE风格擦除进行交叉协方差子空间消除以检验因果影响，并构建透明分类器与随机特征基线对比。审计覆盖三个基础模型（CSBrain、CBraMod、LaBraM）、五个临床任务（MDD、Stress、ISRUC-Sleep、TUSL、Siena）和一个包含6族63个特征的词典。

Result: 在全部945个（模型、任务、特征）单元中，648个（68.6%）为表示因果性，199个（21.1%）为仅编码。50个特征在所有三个架构上均为表示因果且至少在两个任务中成立，成为通用候选。频域特征贡献最大，但其他五个家族各自拥有可观的因果质量。确认特征平均恢复了基础模型79.3%的随机基线优势，并呈现清晰的任务梯度：MDD恢复约0.99，Stress约0.56，显示接近天花板的任务几乎完全被词典恢复，而困难任务残留未解释部分。

Conclusion: 手工特征词典可以解释大部分基础模型的性能增益，但更困难的任务仍存在显著未解释残差，为后续概念发现提供了具体目标。该研究建立了系统化的模型-特征对齐审计框架，并指出了扩展特征空间以覆盖残余表征的必要性。
...
### [336] [Under the Hood of SKILL.md: Semantic Supply-chain Attacks on AI Agent Skill Registry](https://arxiv.org/abs/2605.11418)
*Shoumik Saha,Kazem Faghih,Soheil Feizi*

Main category: cs.AI

TL;DR: 本文揭示了Agent Skills中的SKILL.md文件可作为可操作文本，被攻击者利用来操纵AI代理在注册表各阶段发现、选择和信任恶意技能，攻击成功率极高。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理通过Agent Skills扩展能力，但这种自然语言元数据设计引入了语义供应链风险。本文旨在研究仅通过SKILL.md进行的攻击，探讨攻击者如何影响技能的发现、选择和治理。

Method: 使用真实的ClawHub技能和注册表机制，在Agent Skill生命周期的三个注册表面向阶段（发现、选择、治理）进行实验。在发现阶段，利用短文本触发器操纵嵌入检索；选择阶段，通过描述框架偏置代理；治理阶段，利用语义规避策略逃避拦截。

Result: 发现阶段：对抗性技能可见性实现高达86%的两两胜率和80%的前十排名。选择阶段：代理在77.6%的配对试验中选择了对抗性变体。治理阶段：恶意技能在36.5%-100%的情况下避开了拦截判定。

Conclusion: SKILL.md并非被动文档，而是影响代理发现、信任和使用第三方能力的操作文本。语义攻击对AI代理安全构成重大威胁。
...
### [337] [A Mechanistic Investigation of Supervised Fine Tuning](https://arxiv.org/abs/2605.11426)
*Ruhaan Chopra*

Main category: cs.AI

TL;DR: 尽管微调后模型激活的余弦相似度很高，但通过稀疏自编码器(SAE)投影发现底层稀疏特征发生显著分化，并定位了任务/层级特定的语义变化及安全对齐模式。


<details>
  <summary>Details</summary>
Motivation: 监督微调（SFT）后大语言模型隐藏激活的余弦相似度很高，表面上看模型表示几何变化不大，但实际机制可能更深层；作者旨在揭示SFT引起的表示分化的真正驱动因素。

Method: 提出一种分析流程：利用在基模型上预训练的稀疏自编码器（SAE）作为高分辨率诊断工具，比较微调前后激活在SAE潜在空间中的变化，并逐层解剖语义特征的分布与更新模式。

Result: 发现SFT后稀疏潜在表示显著分化；识别出随任务和层级分布的系统性语义特征变化，以及安全对齐特有的分层更新轮廓。

Conclusion: SAE可有效放大SFT导致的隐藏表示变化，揭示余弦相似度掩盖的稀疏特征重构，为理解微调的机械可解释性提供新路径。
...
### [338] [Adaptive Teacher Exposure for Self-Distillation in LLM Reasoning](https://arxiv.org/abs/2605.11458)
...
### [339] [Breaking $\textit{Winner-Takes-All}$: Cooperative Policy Optimization Improves Diverse LLM Reasoning](https://arxiv.org/abs/2605.11461)
*Haoxuan Chen,Tianming Liang,Wei-Shi Zheng,Jian-Fang Hu*

Main category: cs.AI

TL;DR: 本文提出GCPO，将LLM推理强化学习从独立 rollout 竞争转为团队合作，通过团队信用分配鼓励非冗余正确推理路径，缓解探索坍缩，提升推理准确性和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有基于群体的RLVR算法（如GRPO）在优化LLM推理时容易发生探索坍缩，模型过早收敛到少数高分模式，缺乏探索新解的能力。增加熵正则化或多样性奖励等方案没有改变“赢家通吃”本质，rollout 仍为个体优势竞争而非合作最大化全局多样性。

Method: 提出Group Cooperative Policy Optimization (GCPO)，将训练范式从rollout竞争转为团队合作。引入团队级信用分配：rollout奖励取决于其对团队有效解覆盖的贡献，而非个体准确率。覆盖度通过基于奖励加权的语义嵌入的行列式体积衡量，只有正确且非冗余的rollout贡献体积。优势估计时将团队总奖励按各rollout对团队的平均边际贡献重新分配，使优化走向非冗余的正确推理路径。

Result: 在多个推理基准测试上的实验表明，GCPO相比现有方法显著提升了推理准确性和解多样性。

Conclusion: 合作训练范式有效解决了RLVR中的探索坍缩问题，同时提高了推理精度和多样性，是一种有前景的优化方向。
...
### [340] [CAMPA: Efficient and Aligned Multimodal Graph Learning via Decoupled Propagation and Aggregation](https://arxiv.org/abs/2605.11468)
...
### [341] [TOPPO: Rethinking PPO for Multi-Task Reinforcement Learning with Critic Balancing](https://arxiv.org/abs/2605.11473)
*Yuanpeng Li,Gefei Lin,Annie Qu,Rui Miao*

Main category: cs.AI

TL;DR: 本文提出TOPPO，通过优化PPO的critic梯度条件，解决多任务强化学习中尾任务学习停滞问题，使on-policy方法性能超越off-policy基线。


<details>
  <summary>Details</summary>
Motivation: 现有多任务RL中SAC等off-policy方法占主导，而PPO因critic侧梯度病态导致尾任务更新被主导任务淹没，on-policy方法未得到充分挖掘。

Method: 提出TOPPO（Tail-Optimized PPO），通过Critic Balancing模块组重构PPO，改善梯度条件并平衡各任务的学习动态，直接针对PPO内部的优化瓶颈。

Result: 在Meta-World+基准上，TOPPO以更少参数和环境交互步数，在平均性能和尾任务性能上均优于已发表的SAC族和ARS族基线，并在训练早期即达到或超越强SAC基线。

Conclusion: 通过优化critic侧梯度条件，on-policy方法在多任务RL中可匹敌或超越off-policy方法，挑战了当前对SAC的依赖，并揭示了critic梯度条件为核心瓶颈。
...
### [342] [FibQuant: Universal Vector Quantization for Random-Access KV-Cache Compression](https://arxiv.org/abs/2605.11478)
...
### [343] [Engagement Process: Rethinking the Temporal Interface of Action and Observation](https://arxiv.org/abs/2605.11484)
*Jialian Li,Yuchen Cao,Junhong Liu,Weiran Guo,Xutao Wang,Jiaming Song,Jiahao Zhang,Jie Chen*

Main category: cs.AI

TL;DR: 本文提出Engagement Process（EP）形式化框架，将动作与观察解耦为时间上的事件流，而非固定步骤配对，从而显式建模时间，揭示隐藏的时序行为并支持策略适应时间成本。


<details>
  <summary>Details</summary>
Motivation: 数字与物理环境中的任务完成涉及复杂的时间交互，传统固定步骤的POMDP接口无法捕捉思考延迟、延迟反馈、持续动作等时序问题，需要一种显式表达时间的交互形式化。

Method: 提出Engagement Process（EP），继承POMDP的决策理论结构，但将动作与观察表示为沿时间解耦的事件流，支持单智能体时序问题及多速率协调和子系统组合。

Result: 在玩具示例、LLM智能体和学习实验中，EP揭示了固定步骤接口隐藏的时序行为，并使策略能够在明确的时间成本下自适应。

Conclusion: EP作为一种时间显式的交互形式化，为需考虑时间成本的复杂交互任务提供了更优框架，优于传统步骤模型。
...
### [344] [The Evaluation Differential: When Frontier AI Models Recognise They Are Being Tested](https://arxiv.org/abs/2605.11496)
...
### [345] [Selective Off-Policy Reference Tuning with Plan Guidance](https://arxiv.org/abs/2605.11505)
...
### [346] [Hierarchical LLM-Driven Control for HAPS-Assisted UAV Networks: Joint Optimization of Flight and Connectivity](https://arxiv.org/abs/2605.11509)
...
### [347] [AutoLLMResearch: Training Research Agents for Automating LLM Experiment Configuration -- Learning from Cheap, Optimizing Expensive](https://arxiv.org/abs/2605.11518)
*Taicheng Guo,Nitesh V. Chawla,Olaf Wiest,Xiangliang Zhang*

Main category: cs.AI

TL;DR: 本文提出AutoLLMResearch框架，利用多保真度实验环境学习配置规律，实现高成本大规模语言模型实验的自动配置，效果显著。


<details>
  <summary>Details</summary>
Motivation: 现有自动配置方法仅适用于低成本、可反复试错的场景，而大规模LLM实验配置成本极高且依赖专家直觉，缺乏针对高成本实验的自动化方案，导致资源浪费并限制模型潜力。

Method: 提出AutoLLMResearch代理框架，包含LLMConfig-Gym多保真度环境（涵盖四个关键LLM实验任务，基于超过一百万GPU小时的实验结果）和结构化训练流程，将配置研究建模为长时域马尔可夫决策过程，并通过跨保真度外推推理进行激励。

Result: 在未参与训练的保留实验上与多种强基线方法对比，验证了框架的有效性、泛化能力和可解释性。

Conclusion: 该框架有望成为可扩展的真实世界LLM实验自动化的实用通用解决方案。
...
### [348] [Controllable User Simulation](https://arxiv.org/abs/2605.11519)
*Guy Tennenholtz,Ofer Meshi,Amir Globerson,Uri Shalit,Jihwan Jeong,Craig Boutilier*

Main category: cs.AI

TL;DR: 本文将可控用户模拟器构建为因果推断问题，揭示标准监督微调存在前瞻偏差与可控性崩溃，并提出因果一致的训练方法，实现无偏、鲁棒的对话评估。


<details>
  <summary>Details</summary>
Motivation: 离线数据集无法覆盖稀有场景或支持新策略评估，现有基于大语言模型的可控模拟器在标准训练中引入了结构偏差，缺乏因果一致性，导致评估失真。

Method: 将可控模拟形式化为因果推断问题，分析事后标签导致的混淆和前瞻偏差；证明策略偏移下方差呈几何爆炸（可控性崩溃）。提出恢复因果一致性的理论条件，并设计三种训练缓解策略：先验控制、逐步动态控制和直接策略条件学习。

Result: 实证评估表明，传统全局控制扭曲对话分布并丧失行为多样性；所提出的因果模拟器消除了前瞻偏差，保留了自然方差，且对未见过的代理行为展现出鲁棒的零样本泛化能力。

Conclusion: 通过因果框架训练的用户模拟器克服了可控性崩溃，恢复了无偏评估，为基于离线数据的对话系统测评提供了可靠方法。
...
### [349] [Read, Grep, and Synthesize: Diagnosing Cross-Domain Seed Exposure for LLM Research Ideation](https://arxiv.org/abs/2605.11532)
*Yunju Choi,Min Song*

Main category: cs.AI

TL;DR: PaperGym finds that LLM ideation benefits from diverse seed exposure, but cross-domain retrieval does not outperform random diverse seeds, suggesting current systems don’t exploit semantic retrieval reasons.


<details>
  <summary>Details</summary>
Motivation: Investigate whether LLM ideation systems benefit from targeted cross-domain retrieval or merely from diverse mechanism exposure, given that ML breakthroughs often arise from technique migration across domains.

Method: A three-stage pipeline called PaperGym: (1) tool-augmented seed extraction using read, grep, and bash in an isolated paper environment; (2) cross-domain seed retrieval by paraphrasing across seven ML domains; (3) method synthesis from retrieved seeds with rubric-based judging.

Result: Tool-augmented extraction improves specificity, paraphrase-based retrieval broadens domain coverage. Cross-domain retrieval wins more pairwise novelty comparisons than no-retrieval and same-domain baselines, but shows no significant difference from a random diverse-seed control.

Conclusion: LLM ideation systems benefit from diverse seed exposure, but do not reliably exploit the semantic reasons for particular seed retrieval, indicating current systems lack the ability to leverage targeted cross-domain insights beyond random diversity.
...
### [350] [Optimal LTLf Synthesis](https://arxiv.org/abs/2605.11544)
*Yujian Cao,Sven Schewe,Qiyi Tang,Shufang Zhu*

Main category: cs.AI

TL;DR: 本文提出了最优LTLf合成，旨在从多目标规范中实现尽可能多的目标，即使它们并非全部可联合实现。提出了三种合成变体并通过实验验证了其实际可行性。


<details>
  <summary>Details</summary>
Motivation: 传统LTLf合成在面对无法完全保证的规范时返回“不可实现”，这种全有或全无的方法不适用于多目标规范，因为部分目标可能可实现。动机是合成能够达到最大目标子集的策略，而非完全放弃。

Method: 作者提出了三种最优合成方法：最大保证合成，预先承诺可保证的最大目标集；最大观察合成，在不同执行中最大化后验可实现的目标；增量最大观察合成，在执行过程中动态利用机会增强保证。

Result: 实验结果表明，三种合成变体在扩展性上大致相当，在给定超时内解决了大多数基准实例，显示了方法的实际可行性。

Conclusion: 本文得出结论，针对多目标规范的最优LTLf合成是可行且有效的，超越了全有或全无的范式，能更好地处理具有多个可能冲突目标的现实场景。
...
### [351] [Hindsight Hint Distillation: Scaffolded Reasoning for SWE Agents from CoT-free Answers](https://arxiv.org/abs/2605.11556)
...
### [352] [Dual-Temporal LSTM with Hybrid Attention for Airline Passenger Load Factor Forecasting: Integrating Intra-Flight and Inter-Flight Booking Dynamics](https://arxiv.org/abs/2605.11569)
...
### [353] [Native Explainability for Bayesian Confidence Propagation Neural Networks: A Framework for Trusted Brain-Like AI](https://arxiv.org/abs/2605.11595)
...
### [354] [GAR: Carbon-Aware Routing for LLM Inference via Constrained Optimization](https://arxiv.org/abs/2605.11603)
...
### [355] [CuSearch: Curriculum Rollout Sampling via Search Depth for Agentic RAG](https://arxiv.org/abs/2605.11611)
*Jianghan Shen,Siqi Luo,Xinyu Cheng,Jing Xiong,Yue Li,Jiyao Liu,Jiashi Lin,Yirong Chen,Junjun He*

Main category: cs.AI

TL;DR: 本文提出CuSearch，一种基于搜索深度的课程式rollout采样框架，通过SDGA算子将固定更新预算重新分配给更深层轨迹，以利用不同轨迹在检索监督密度上的异质性，在RLVR训练中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法均等对待所有rollout，忽略了搜索深度差异：深层搜索轨迹包含更多检索决策点，能为检索子策略提供更密集的直接监督；且训练中批次深度分布会向深层偏移，但均匀采样未能适应这种变化。

Method: 提出CuSearch框架，核心是批量级算子Search-Depth Greedy Allocation（SDGA），将固定更新预算重新分配至更深的轨迹。包含两种变体：SDGA-Auto始终选择批次中最深的可用轨迹，形成与训练对齐的隐式课程；SDGA-Phase在深层轨迹足够丰富时显式推进课程阈值。

Result: 在多种模型和检索框架上的实验表明，CuSearch一致提升性能，在ZeroSearch上较标准GRPO最高提高11.8个Exact Match点。

Conclusion: 每条轨迹的搜索深度是RLVR训练中一个可靠且无需标注的检索监督密度代理，为智能体RAG系统的课程学习提供了有效指导。
...
### [356] [Nice Fold or Hero Call: Learning Budget-Efficient Thinking for Adaptive Reasoning](https://arxiv.org/abs/2605.11625)
*Zhaomeng Zhou,Lan Zhang,Junyang Wang,Mu Yuan,Junda Lin*

Main category: cs.AI

TL;DR: 本文提出了预算高效思维（BET）框架，根据问题的可解性分配计算预算，在提升性能的同时平均减少约55%的推理tokens。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型常因忽视问题可解性而错误分配推理资源，要么在无法解出的问题上浪费大量算力，要么压缩了需要深度推理的可解难题。需要根据预期收益而非仅凭感知难度来分配计算预算。

Method: 提出预算高效思维（BET），一种两阶段框架：结合行为冷启动和投资成本感知奖励的GRPO，学习简短作答（short solve）、适时放弃（nice fold）和英雄抉择（hero call）三种行为。

Result: 在7个基准和3个基础模型上，BET平均减少约55%的推理tokens，同时实现整体性能提升，并零样本迁移到科学问答和逻辑推理任务，保持相近的效率收益。

Conclusion: 通过将推理决策与可解性对齐，BET有效分配测试时计算资源，显著提升预算效率且不牺牲性能。
...
### [357] [Can LLM Agents Respond to Disasters? Benchmarking Heterogeneous Geospatial Reasoning in Emergency Operations](https://arxiv.org/abs/2605.11633)
*Junjue Wang,Weihao Xuan,Heli Qi,Pengyu Dai,Kunyi Liu,Hongruixuan Chen,Zhuo Zheng,Junshi Xia,Stefano Ermon,Naoto Yokoya*

Main category: cs.AI

TL;DR: 本文提出灾难操作响应代理基准（DORA），首个面向端到端灾难响应的代理基准，包含515个专家编写的任务，覆盖45个真实灾难事件，代理需组合调用108种工具在多源地理空间数据上完成感知、推理、规划与报告生成，评估13个前沿LLMs后揭示了领域接地、工具选择与组合泛化的三大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有工作多孤立地处理遥感感知或评估通用的工具使用，忽略了应急操作中端到端工作流的整体需求，缺乏对灾难响应代理在完整操作链上的系统性评价。

Method: 构建DORA基准，包含10类灾难的45个真实事件、515项专家任务和3500步专家验证的工具调用轨迹；任务覆盖灾难感知、空间关系分析、救援与疏散规划、时间演化推理及多模态报告合成五个维度；代理通过108个工具的MCP库调用光学、SAR、多光谱影像及高程与社会矢量图层；在13个前沿LLMs上全面评估。

Result: 评估揭示了三个持续挑战：1）灾难领域接地导致独特失败模式（损伤语义接地、传感器模态错配、灾难流水线组合错误）；2）代理受双重瓶颈——工具选择和参数接地制约，即使提供黄金工具顺序提示，准确率仅提升1.08-4.40%，替代脚手架最多提升3.24%；3）组合脆弱性随轨迹长度增加而加剧，代理与黄金轨迹的差距从短流水线的7%扩大到长流水线的56%。

Conclusion: DORA为操作可靠的灾难响应代理建立了严格的测试平台，突显了当前LLM代理在端到端灾难操作中的关键缺陷。
...
### [358] [Seirênes: Adversarial Self-Play with Evolving Distractions for LLM Reasoning](https://arxiv.org/abs/2605.11636)
...
### [359] [A CAP-like Trilemma for Large Language Models: Correctness, Non-bias, and Utility under Semantic Underdetermination](https://arxiv.org/abs/2605.11672)
*Vinu Ellampallil Venugopal*

Main category: cs.AI

TL;DR: 本文提出，在语义欠确定的提示下，大语言模型无法同时保证强正确性、严格无偏性和高实用性，形成类似CAP定理的三难困境。


<details>
  <summary>Details</summary>
Motivation: 受分布式系统CAP定理启发，探索大语言模型在处理欠确定问题时是否存在类似的理论限制，以解释某些失效并非仅由模型缺陷引起。

Method: 通过形式化定义语义欠确定、正确性、无偏性与实用性的概念，构建三难猜想，并结合示例阐述其内在权衡与结构根源。

Result: 提出并论证了“正确性-无偏性-实用性”三难：语义欠确定时，模型若保持正确且无偏则必然降低实用性（如拒绝回答或请求澄清），若要实用则可能引入选择偏好而导致偏差。

Conclusion: 部分大语言模型失效可归因于欠确定请求本身的结构性约束，这是模型行为中固有的权衡，而非单纯的能力不足。
...
### [360] [OOM-Free Alpamayo via CPU-GPU Memory Swapping for Vision-Language-Action Models](https://arxiv.org/abs/2605.11678)
...
### [361] [Explaining and Breaking the Safety-Helpfulness Ceiling via Preference Dimensional Expansion](https://arxiv.org/abs/2605.11679)
...
### [362] [Persistent and Conversational Multi-Method Explainability for Trustworthy Financial AI](https://arxiv.org/abs/2605.11687)
*Georgios Makridis,Georgios Fatouros,John Soldatos,George Katsis,Dimosthenis Kyriazis*
...
### [363] [Measuring What Matters Beyond Text: Evaluating Multimodal Summaries by Quality, Alignment, and Diversity](https://arxiv.org/abs/2605.11693)
*Abid Ali,Diego Molla-Aliod,Usman Naseem*

Main category: cs.AI

TL;DR: 本文提出MM-Eval，一个统一的多模态摘要评估框架，整合文本质量、图文相关性和视觉多样性三个维度，通过基于人类偏好训练的聚合模型，揭示事实一致性是关键因素，视觉元素为补充，性能超越启发式基线，具有可解释性和参考弱依赖性。


<details>
  <summary>Details</summary>
Motivation: 目前多模态输出摘要（MSMO）的评估各自为政，使用单模态指标孤立衡量文本质量、图文对齐和视觉多样性，无法综合判断多模态是否共同支持忠实有用的摘要，因此需要一个统一框架。

Method: MM-Eval框架包含三个部分：①文本质量用OpenFActScore评估事实一致性，G-Eval评估连贯、流畅、相关性；②图文相关性采用MLLM-as-a-judge方式判断；③图像集多样性用截断CLIP熵量化。通过mLLM-EVAL新闻基准训练一个学习型聚合模型，使各成分权重与人类偏好对齐。

Result: 分析揭示文本主导的层次：事实一致性是感知整体质量的决定性因素，视觉相关性和多样性提供辅助信号。MM-Eval优于启发式聚合基线，提供可解释的、弱参考的比较评估方案。

Conclusion: MM-Eval成功统一了多模态摘要评估，证明事实一致性驱动整体质量感知，视觉信息增强细微判别，学习型聚合模型符合人类判断，为MSMO比较评估提供了稳健且可解释的框架。
...
### [364] [Toward Stable Value Alignment: Introducing Independent Modules for Consistent Value Guidance](https://arxiv.org/abs/2605.11712)
...
### [365] [SafeSteer: A Decoding-level Defense Mechanism for Multimodal Large Language Models](https://arxiv.org/abs/2605.11716)
...
### [366] [Allegory of the Cave: Measurement-Grounded Vision-Language Learning](https://arxiv.org/abs/2605.11727)
...
### [367] [OptArgus: A Multi-Agent System to Detect Hallucinations in LLM-based Optimization Modeling](https://arxiv.org/abs/2605.11738)
*Zhong Li,Zihan Guo,Xiaohan Lu,Juntao Wang,Jie Song,Chao Shen,Jiageng Wu,Mingyang Sun*

Main category: cs.AI

TL;DR: 本文定义优化建模中的幻觉检测问题，提出细粒度幻觉分类法并设计多智能体检测系统OptArgus，在多项测试中优于单智能体基线，将幻觉检测转化为可操作的经验性任务。


<details>
  <summary>Details</summary>
Motivation: 大语言模型将自然语言优化问题转化为数学公式与求解器代码时，仅靠匹配参考目标值无法保证语义正确，存在改变优化语义的幻觉。需要一种结构一致性审计方法来检测此类建模幻觉。

Method: 构建覆盖目标、变量、约束和实现四类失败的细粒度幻觉分类法；设计多智能体检测器OptArgus，包含指挥路由、专业审计员和证据整合模块；并构建包含484个干净产物、1266个人工注入错误产物和6292个自然生成产物的三部分基准套件。

Result: 与单智能体基线相比，OptArgus在干净产物上误报更少，在受控单错误案例上首位定位更准确，对自然模型输出的检测能力更强。

Conclusion: 本研究将优化建模幻觉检测确立为具体经验问题，表明基于分类法的模块化审计是提升优化建模可靠性的实用路径。
...
### [368] [When Reasoning Traces Become Performative: Step-Level Evidence that Chain-of-Thought Is an Imperfect Oversight Channel](https://arxiv.org/abs/2605.11746)
...
### [369] [Towards Visually Grounded Multimodal Summarization via Cross-Modal Transformer and Gated Attention](https://arxiv.org/abs/2605.11753)
*Abid Ali,Diego Molla-Aliod,Usman Naseem*
...
### [370] [Beyond Inefficiency: Systemic Costs of Incivility in Multi-Agent Monte Carlo Simulations](https://arxiv.org/abs/2605.11789)
*Alison Moldovan-Mauer,Benedikt Mangold*

Main category: cs.AI

TL;DR: 利用大语言模型多智能体系统模拟辩论，发现毒性行为增加25%收敛延迟，小模型延迟更大，且存在先手优势。


<details>
  <summary>Details</summary>
Motivation: 不文明沟通对效率的影响因伦理限制、可重复性低及自然场景不可预测而难以在人类受试研究中隔离，故采用大语言模型多智能体系统作为受控社会学沙箱。

Method: 采用蒙特卡洛模拟框架，在不同毒性条件下生成数千场一对一对抗性辩论，以达成结论所需回合数作为互动效率指标，并复制先前研究，在两个不同参数规模的额外LLM智能体上扩展实验。

Result: 确认了先前研究中的25%收敛延迟；发现该延迟在参数较少的模型中显著更大；并发现显著的先手优势，即无论毒性如何，发起讨论的智能体获胜概率均显著高于随机。

Conclusion: 毒性行为降低多智能体辩论的互动效率，尤其对能力较弱的模型影响更大，且辩论发起者具有稳固的先手优势，揭示了沟通行为对系统效率的稳健影响。
...
### [371] [Why Users Go There: World Knowledge-Augmented Generative Next POI Recommendation](https://arxiv.org/abs/2605.11807)
*Qiuyu Ding,Heng-Da Xu,Wei Zhang,Dongyi Lv,Changda Xia,Feng Xiong,Mu Xu*

Main category: cs.AI

TL;DR: 提出AWARE模型，利用LLM代理生成位置和时间感知的上下文叙事，将动态世界知识融入POI推荐，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的POI推荐模型知识固定，无法感知现实世界动态变化（如本地事件、文化趋势）对用户出行决策的影响，需要引入外部世界知识来增强模型的时效性和个性化。

Method: 设计AWARE框架，通过LLM代理生成包含区域文化、季节趋势和时事的位置-时间感知叙事，并将其锚定在用户行为上下文中，实现外部知识在个性化时空模式中的扎根。

Result: 在三个真实数据集上的实验表明，AWARE始终优于竞争基线，相对改进最高达12.4%。

Conclusion: AWARE有效弥合了静态LLM知识与动态现实世界的鸿沟，利用个性化上下文叙事增强POI推荐性能，验证了将外部世界知识融入生成式推荐的潜力。
...
### [372] [Beyond World-Frame Action Heads: Motion-Centric Action Frames for Vision-Language-Action Models](https://arxiv.org/abs/2605.11809)
...
### [373] [Automated Reformulation of Robust Optimization via Memory-Augmented Large Language Models](https://arxiv.org/abs/2605.11813)
...
### [374] [MedMemoryBench: Benchmarking Agent Memory in Personalized Healthcare](https://arxiv.org/abs/2605.11814)
...
TL;DR: 本文介绍 MedMemoryBench，一个面向医疗健康代理记忆机制的基准测试，利用基于临床原型的合成医疗对话轨迹和流式评估协议，揭示了当前架构在复杂医学推理和噪声鲁棒性方面的严重瓶颈。


<details>
  <summary>Details</summary>
Motivation: 个性化健康代理的大规模部署需要精准、安全、可长期追踪的记忆机制，但现有基准主要关注日常开放域对话，无法捕捉真实医学应用的高风险复杂性。本研究由一款服务数千万用户的行业领先健康管理代理的生产需求所驱动。

Method: 开发了一个人机协作流水线，基于临床基础的合成患者原型生成高度真实的长期医疗轨迹，形成了包含约2000个会话和16000轮交互的专家验证数据集。提出了“边构建边评估”的流式评估协议，精确模拟生产环境中的动态记忆累积。此外，正式定义并系统研究了记忆饱和现象，即持续信息流入会主动降低检索与推理的鲁棒性。

Result: 全面基准测试显示主流架构存在严重瓶颈，特别是在复杂医学推理和噪声鲁棒性方面。记忆饱和导致检索和推理性能下降。

Conclusion: MedMemoryBench 通过暴露这些根本性缺陷，为开发稳健、可投入生产的医疗代理奠定了重要基础。
...
### [375] [On-Policy Self-Evolution via Failure Trajectories for Agentic Safety Alignment](https://arxiv.org/abs/2605.11882)
*Bo Yin,Qi Li,Xinchao Wang*

Main category: cs.AI

TL;DR: FATE transforms trajectory-level failures into repair supervision for safe self-evolving LLM agents, using on-policy self-evolution and Pareto-Front Policy Optimization to preserve safety-utility balance.


<details>
  <summary>Details</summary>
Motivation: Existing safety alignment for LLM agents operates at the response level, leading to unsafe trajectories (e.g., unsafe tool use, injected instruction following) and a trade-off between safety and utility. Sparse, off-policy rewards limit real‑world deployment.

Method: FATE is an on-policy self-evolving framework that repairs failed trajectories: the policy proposes repair candidates, verifiers re-score them, and filtering across security, utility, over-refusal, and trajectory validity yields dense supervision. It incorporates Pareto-Front Policy Optimization (PFPO) with supervised warm-up to maintain the safety-utility trade-off.

Result: On AgentDojo, AgentHarm, and ATBench, FATE reduces attack success rate by 33.5%, harmful compliance by 82.6%, and improves trajectory-safety diagnosis by 6.5% while preserving useful behavior compared to strong baselines.

Conclusion: Failed trajectories provide structured repair supervision that enables safer self-evolving agents without sacrificing utility, demonstrating the value of dense, trajectory-level feedback for agent alignment.
...
### [376] [From Clever Hans to Scientific Discovery: Interpreting EEG Foundational Transformers with LRP](https://arxiv.org/abs/2605.11885)
*Justus Meyer zu Bexten,Nico Scherf,Bogdan Franczyk,Simon M. Hofmann*

Main category: cs.AI

TL;DR: 该研究将注意力感知的分层相关传播（LRP）应用于脑电图（EEG）Transformer基础模型，验证其决策并发现新颖的生物学假说，例如在运动想象任务中检测到模型依赖伪影（眼部信号）而非真实运动信号，并在情感预测中发现中央电极簇可能与觉醒的感知运动特征相关。


<details>
  <summary>Details</summary>
Motivation: 脑电图基础模型虽有潜力放大深度学习在诊断和脑机接口中的应用，但其黑箱本质阻碍了广泛采纳；因此需要开发可解释性方法以增强信任和发现新知识。

Method: 采用注意力感知的分层相关传播（LRP）作为事后归因方法，将其从基于卷积神经网络（CNN）的脑电模型扩展至当前基础模型所基于的Transformer架构。

Result: LRP既能验证模型决策（如在运动想象中揭露“聪明汉斯”行为，即模型优先利用与任务相关的眼部信号而非预期的运动关联），也能挖掘出新颖假设（如在自然范式情感预测中反复依赖中央电极集群，提示这可能是觉醒的感知运动签名）。然而，热图解释在复杂领域中仍存在模糊性。

Conclusion: LRP被定位为脑电图基础模型的验证与探索工具，随着底层模型成熟，其重要性和发现潜力将日益增长。
...
### [377] [Toward Modeling Player-Specific Chess Behaviors](https://arxiv.org/abs/2605.11893)
...
### [378] [Rethinking Supervision Granularity: Segment-Level Learning for LLM-Based Theorem Proving](https://arxiv.org/abs/2605.11905)
*Shuo Xu,Jiakun Zhang,Junyu Lai,Chun Cao,Jingwei Xu*

Main category: cs.AI

TL;DR: 本文针对大语言模型在Lean 4中的自动定理证明，提出段级监督策略，通过提取连贯证明段落训练策略模型，并在推理时触发短rollout，显著提升了证明成功率。


<details>
  <summary>Details</summary>
Motivation: 现有步骤级策略预测提供密集局部信号但易打断连贯证明过程，全证明生成保留全局结构但需复杂端到端生成，需要在监督粒度上寻找平衡以对齐模型学习和证明结构。

Method: 提出段级监督的训练数据构建方法，从证明轨迹中提取局部连贯的证明段落用于训练；推理时采用目标感知rollout策略，在段级触发短搜索以增强现有步骤级模型。

Result: 在miniF2F上，使用STP、LeanWorkbook和NuminaMath-LEAN训练的段级监督模型分别达到64.84%、60.90%和66.31%的成功率，一致超越步骤级和全证明基线；目标感知rollout将BFS-Prover-V2-7B和InternLM2.5-StepProver的成功率分别从68.77%提至70.74%和59.59%提至60.33%，同时降低推理成本。

Conclusion: 适当的监督粒度能更好地对齐模型学习与证明结构及搜索过程，段级监督有效提升了自动定理证明的性能和效率。
...
### [379] [Rethinking Positional Encoding for Neural Vehicle Routing](https://arxiv.org/abs/2605.11910)
...
### [380] [Domain Restriction via Multi SAE Layer Transitions](https://arxiv.org/abs/2605.11920)
...
### [381] [When Simulation Lies: A Sim-to-Real Benchmark and Domain-Randomized RL Recipe for Tool-Use Agents](https://arxiv.org/abs/2605.11928)
...
TL;DR: 本文针对工具使用语言智能体在真实部署中因输入噪声、工具注册模糊及API不可靠等引发的sim-to-real差距，提出一个包含22种扰动类型的基准RobustBench-TC，并开发领域随机化强化学习方法ToolRL-DR，显著提升了模型在扰动下的鲁棒性，缩小了与强闭源模型的差距。


<details>
  <summary>Details</summary>
Motivation: 现有工具使用智能体的评测假设干净输入、明确工具清单和可靠API，但实际部署中存在用户拼写错误、请求超时、工具名重复等噪声，导致模拟与真实环境的显著性能落差，现有模型对此类扰动的鲁棒性不足，且单纯扩大模型规模无法解决问题。

Method: 构建RobustBench-TC基准，按照POMDP的观测、动作空间、奖励相关元数据和转移动态四个组件设计22种现实扰动，每种扰动均基于真实的GitHub issue或工具调用失败案例。提出ToolRL-DR方法，通过在扰动增强的轨迹上进行领域随机化强化学习，训练覆盖三个可静态编码POMDP组件的工具使用智能体。

Result: 在1.5B至32B参数的21个模型上评测显示：观测扰动准确率下降不足5%，而奖励相关和转移动态扰动分别导致约40%和30%的准确率下降；模型规模无法弥补鲁棒性差距。基于3B骨干的ToolRL-DR-Full模型保留了约75%的干净准确率，扰动下总体准确率媲美14B开源函数调用基线，并与o4-mini的差距大幅缩小，且在没有接触转移扰动训练的情况下，弥补了约27%的转移差距。

Conclusion: 通过对抗性静态输入上的强化学习，智能体习得了更持久的重试策略，能够泛化至未见过的运行时故障，验证了领域随机化RL在缩小工具使用智能体sim-to-real差距方面的有效性，相关数据、代码和排行榜已公开。
...
### [382] [From Noise to Diversity: Random Embedding Injection in LLM Reasoning](https://arxiv.org/abs/2605.11936)
...
### [383] [Counterfactual Trace Auditing of LLM Agent Skills](https://arxiv.org/abs/2605.11946)
*Xiaolin Zhou,Jinbo Liu,Li Li,Ryan A. Rossi,Xiyang Hu*

Main category: cs.AI

TL;DR: This paper proposes Counterfactual Trace Auditing (CTA) to measure how agent skills change behavior beyond pass rate, revealing substantial behavioral reshaping even when task success is nearly unchanged.


<details>
  <summary>Details</summary>
Motivation: Current skill evaluations treat skills as black boxes and only report pass rate changes, failing to capture how skills alter agent decision-making and internal behavior.

Method: CTA pairs agent traces with and without a skill on the same task, segments traces into goal-directed phases, aligns them, and outputs structured Skill Influence Pattern (SIP) annotations that describe behavioral changes.

Result: Applied to Claude on 49 SWE tasks, pass rate changed only +0.3pp, but CTA detected 522 SIP instances across paired traces, showing skills reshape behavior substantially. Three patterns emerged: ceiling tasks show most effects but saturated pass rate; moderate-baseline tasks offer recoverable gains at higher token cost; dominant SIP types vary by baseline (surface anchoring on ceiling, edge-case prompting on mid/floor).

Conclusion: CTA bridges the gap between informal failure observations and reproducible measurements, enabling systematic auditing of skill influence on agent behavior.
...
### [384] [Assessing and Mitigating Miscalibration in LLM-Based Social Science Measurement](https://arxiv.org/abs/2605.11954)
*Jinyuan Wang,Ningyuan Deng,Yi Yang*

Main category: cs.AI

TL;DR: 本文研究大语言模型在社会科学测量中的校准问题，发现模型置信度与正确性对齐不佳，并影响回归估计；提出软标签蒸馏方法校准BERT，显著降低校准误差。


<details>
  <summary>Details</summary>
Motivation: 大语言模型被广泛用于将非结构化文本转换为社会科学变量，但测量有效性要求置信度能真实反映正确概率，而不仅仅是平均准确性。miscalibration可能导致下游回归估计偏差，因此需要审计和缓解校准问题。

Method: 先以FOMC案例展示miscalibration对回归的影响，然后对14个社会科学构念、包括GPT-5-mini和DeepSeek-V3.2等专有及开源模型进行校准审计。提出软标签蒸馏管道：将LLM分数和口述置信度转化为软目标分布，再训练基于编码器的小型判别分类器，以校准BERT。

Result: 软标签蒸馏方法在数据集上平均将期望校准误差（ECE）降低43.2%，将Brier分数降低34.0%。

Conclusion: 基于LLM的社会科学分析流程应将校准视为测量有效性的必要组成部分，而非可选的后处理环节。
...
### [385] [On the Limitations of Large Language Models for Conceptual Database Modeling](https://arxiv.org/abs/2605.11986)
*Arthur F. Siqueira,Carlos D. S. Nogueira,Eduarda Farias,Claudio E. C. Campelo,Júlia Menezes*

Main category: cs.AI

TL;DR: 评估大型语言模型从自然语言需求自动生成ER图：LLM在简单场景下表现尚可，但随着需求复杂性增加，可靠性下降，不适用于复杂概念建模。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否能够可靠地从自然语言需求中自动生成实体关系图，以支持关系数据库的概念建模。

Method: 结合三种大型语言模型与三种提示工程技巧（零样本、思维链、思维链+验证器），应用于同一组逐渐复杂的需求场景；通过将生成的ER图与文本需求进行结构性和语义性对比，进行定性分析。

Result: LLM在低复杂度场景下表现合理，但随着需求复杂度上升，不一致性、歧义和约束表示失败增加，可靠性显著下降。

Conclusion: 当前LLM尚未足够成熟，无法在复杂场景中可靠使用，验证成本可能抵消表面上的生产力提升。
...
### [386] [Random-Set Graph Neural Networks](https://arxiv.org/abs/2605.11987)
...
### [387] [BadSKP: Backdoor Attacks on Knowledge Graph-Enhanced LLMs with Soft Prompts](https://arxiv.org/abs/2605.11996)
...
### [388] [LegalCheck: Retrieval- and Context-Augmented Generation for Drafting Municipal Legal Advice Letters](https://arxiv.org/abs/2605.12012)
...
### [389] [LLMs and the ZPD](https://arxiv.org/abs/2605.12016)
*Peter Wallis*

Main category: cs.AI

TL;DR: 本文提出，大型语言模型并非通过‘分布式表征’进行思考，而是在进行一种基于实践的‘原始思维’，其输出更似‘做梦’而非‘幻觉’；同时强调互动是人类交流的核心，而非理解的附加品，并借鉴维果茨基的最近发展区理论。


<details>
  <summary>Details</summary>
Motivation: 挑战当前将LLM视为基于分布式表征进行思维的主流观点，重新界定模型的输出性质（梦而非幻觉），并凸显互动在认知中的基础地位，以矫正仅将互动视为理解之附属的看法。

Method: 采用概念分析与历史理论借鉴的方法，主要依托维果茨基关于意识、科学思维及最近发展区的理论，对大型语言模型的行为进行哲学性再阐释。

Result: 提出LLM的输出应被理解为‘做梦’，而非‘幻觉’；指出真正需要的是探索能够实现常识性行为的认知工具集，而非设置护栏；并论证互动是人类沟通的核心机制。

Conclusion: 互动应当被视为人类沟通与理解的根基，而非对‘真实’理解的附加；未来研究应聚焦于揭示支持常识性认知的互动性工具，而非简单约束模型输出。
...
### [390] [OmniRefine: Alignment-Aware Cooperative Compression for Efficient Omnimodal Large Language Models](https://arxiv.org/abs/2605.12056)
...
### [391] [SAGE: A Self-Evolving Agentic Graph-Memory Engine for Structure-Aware Associative Memory](https://arxiv.org/abs/2605.12061)
*Juntong Wang,Haoyue Zhao,guanghui Pan,Xiyuan Wang,Yanbo Wang,Qiyan Deng,Muhan Zhang*

Main category: cs.AI

TL;DR: SAGE 是一个自进化的图记忆引擎，通过记忆写入器逐步构建结构化图记忆，并由基于图基础模型的读取器进行检索与反馈，从而动态改进长期记忆，在多跳问答和长期记忆基准上取得最佳表现。


<details>
  <summary>Details</summary>
Motivation: 现有 RAG 和 GraphRAG 系统将记忆图视为静态检索中间件，限制了从部分线索恢复完整证据链、利用可复用的图结构角色以及通过下游反馈提升记忆的能力。

Method: SAGE 耦合了两个角色：记忆写入器从交互历史中增量构建结构化图记忆，记忆读取器基于图基础模型进行检索并向写入器提供反馈，辅以严格的理论分析。

Result: 在多跳问答中，经两轮自演化后获得最佳平均排名；在零样本开放域迁移中，NQ 数据集上 Recall@2/5 达到 82.5/91.6；在 LongMemEval 和 HaluMem 上，训练与读写反馈改善了多项长期记忆和幻觉诊断指标。

Conclusion: 自演化、结构感知的图记忆是构建鲁棒的长周期语言智能体的有前景的基础。
...
### [392] [Intermediate Artifacts as First-Class Citizens: A Data Model for Durable Intermediate Artifacts in Agentic Systems](https://arxiv.org/abs/2605.12087)
*Josh Rosen,Seth Rosen*

Main category: cs.AI

TL;DR: 本文提出AI系统应保留持久化、可检查的中间产物（如证据图、声明结构等），而非仅输出最终结果，以提升AI生成工作的可检查性、可修订性和可维护性。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在循环推理过程中，中间工作往往短暂存在，最终产物是对上游状态的失真投影，导致多步可修订工作的质量难以保证；需要一种机制来保存结构化中间产物，以便人类和代理后续审查、修订和优化。

Method: 提出一个系统级数据模型，定义中间产物的关键属性（类型化、结构化、可寻址、版本化、依赖感知、权威性等），区分中间产物与聊天记录、隐藏思维链等；形式化添加和替代更新语义，包含显式当前状态解析；描述产物谱系如何支持跨修订的持久中间状态。

Result: 贡献了该数据模型的详细设计，区别于其他概念（如记忆、叙事等），并论证了评估应针对维护状态的质量，而非仅仅最终输出质量。

Conclusion: 持久中间产物并不直接使模型更智能，但能显著提升AI生成工作随时间推移的可检查性、可修订性和可维护性，未来评估工作质量时应以维护状态为重要指标。
...
### [393] [Autonomy and Agency in Agentic AI: Architectural Tactics for Regulated Contexts](https://arxiv.org/abs/2605.12105)
...
### [394] [Large Language Models as Amortized Pareto-Front Generators for Constrained Bi-Objective Convex Optimization](https://arxiv.org/abs/2605.12106)
...
### [395] [Adaptive Multi-Round Allocation with Stochastic Arrivals](https://arxiv.org/abs/2605.12111)
*Yuqi Pan,Davin Choo,Haichuan Wang,Milind Tambe,Alastair van Heerden,Cheryl Johnson*

Main category: cs.AI

TL;DR: We solve a sequential resource allocation problem for adaptive network recruitment by using a population-level surrogate to derive a polynomial-time dynamic programming algorithm, with robustness guarantees.


<details>
  <summary>Details</summary>
Motivation: Sequential resource allocation in network recruitment, where successful referrals generate future opportunities and repeated allocations exhibit diminishing returns, demands efficient planning under high-dimensional uncertainty.

Method: We first prove an exact greedy single-round allocation policy based on marginal survival probabilities. For multi-round planning, we introduce a population-level surrogate value function depending only on remaining budget and frontier size, which yields an exact dynamic program via truncated probability generating functions with polynomial complexity.

Result: The planning algorithm runs in polynomial time in the total budget. We also derive a multi-round error bound under model misspecification that decomposes into a tight single-round frontier error and a population-level transition error.

Conclusion: The approach is evaluated on real-world inspired recruitment scenarios, demonstrating its practical effectiveness and robustness.
...
### [396] [To Whom Do Language Models Align? Measuring Principal Hierarchies Under High-Stakes Competing Demands](https://arxiv.org/abs/2605.12120)
...
### [397] [Rollout Cards: A Reproducibility Standard for Agent Research](https://arxiv.org/abs/2605.12131)
*Charlie Masters,Ziyuan Liu,Stefano V. Albrecht*

Main category: cs.AI

TL;DR: 智能体研究面临可复现性危机，因仅凭报告分数比较而忽略rollout记录，相同的智能体行为可能因评估截取和报告规则不同而得分迥异。本文审计50个代码库发现均未报告失败与错误运行数，并记录37项报告规则可大幅改变得分的案例。提出以rollout记录为复现单元，引入rollout cards出版包，经验证仅改报告规则即可使得分变化20.9个百分点并可能颠倒模型排名。


<details>
  <summary>Details</summary>
Motivation: 动机是解决智能体研究中因报告规则不透明导致的复现性问题。当前评估仅关注最终分数，而rollout记录的选取和报告规则（如如何截取轨迹、如何计算成本或计时）会显著影响分数，使得同一代码产出截然不同的结果。现有训练和评估库普遍缺失失败、错误或跳过运行的统计，阻碍了公平比较与复现。

Method: 对50个流行的训练与评估库进行结构化审计，识别报告规则可变场景。提出rollout cards概念：一种出版物捆绑格式，完整保留rollout记录，并声明视图、报告规则和丢弃清单。通过两种方式验证：1）在工具安全、多智能体、定理证明和搜索领域公开部分发布，补充原始报告中未披露的分析；2）对保留的基准输出按不同报告规则重新评分，量化得分与排名的变化。开发参考实现集成到开源RL gym Ergon，并公开多个基准的rollout-card导出。

Result: 审计发现无一库在报告头部分数的同时提供运行失败、错误或跳过的数量。记录37项仅改变报告规则就能改变任务成功率、成本/令牌计量或时序测量的案例，影响有时很剧烈。重评分实验显示，仅通过调整报告规则，得分变化可达20.9个绝对百分点，某些情况下会颠覆前沿模型的排名顺序。

Conclusion: rollout记录而非报告分数应作为智能体研究复现性的基本单元，rollout cards通过透明化报告流程有效提升了复现性和可比性。公开的实现和基准导出为未来工作提供了可重复的基础。
...
### [398] [BoolXLLM: LLM-Assisted Explainability for Boolean Models](https://arxiv.org/abs/2605.12139)
...
### [399] [MM-OptBench: A Solver-Grounded Benchmark for Multimodal Optimization Modeling](https://arxiv.org/abs/2605.12154)
...
### [400] [ALGOGEN: Tool-Generated Verifiable Traces for Reliable Algorithm Visualization](https://arxiv.org/abs/2605.12159)
...
### [401] [Do Enterprise Systems Need Learned World Models? The Importance of Context to Infer Dynamics](https://arxiv.org/abs/2605.12178)
...
TL;DR: 在可配置的企业环境中，世界模型因依赖历史数据训练而对业务逻辑变化脆弱；本文提出运行时读取系统配置以发现过渡动态的“企业发现代理”，在CascadeBench上验证其应对部署偏移的鲁棒性优于离线训练模型。


<details>
  <summary>Details</summary>
Motivation: 企业系统的环境动态常由租户特定的业务逻辑定义，且随部署和演化而变化，导致基于历史转移训练的模型在部署偏移下性能退化。现有世界模型文献未解决“当规则可在推理时读取，代理是否仍需学习它们”的问题，本文旨在探索在可配置且可读的过渡动态场景下，运行时发现能否补充离线训练。

Method: 提出企业发现代理，在推理时通过读取系统配置来恢复相关的过渡动态，而非仅依赖内部化表示；引入CascadeBench基准，采用World of Workflows的评估方法，构建多样化的合成环境用于企业级级联预测，并结合部署偏移评估。

Result: 离线训练的世界模型在分布内表现良好，但随着动态变化性能下降；基于发现的代理通过将预测扎根于当前系统实例，在偏移下表现出更强的鲁棒性。

Conclusion: 在可配置的企业环境中，代理不应仅依赖固定的内部化动态，而应整合运行时发现相关过渡逻辑的机制，以增强应对变化的能力。
...
### [402] [MolDeTox: Evaluating Language Model's Stepwise Fragment Editing for Molecular Detoxification](https://arxiv.org/abs/2605.12181)
*Jueon Park,Wonjune Jang,Jiwoo Lee,Yein Park,Jaewoo Kang*

Main category: cs.AI

TL;DR: 提出MolDeTox基准，用于分子去毒任务的细粒度评估，填补毒性修复评估空白，证明片段级生成提升结构有效性和分子质量。


<details>
  <summary>Details</summary>
Motivation: 现有模型和基准普遍忽视毒性相关挑战，现有毒性修复基准存在数据多样性有限、生成分子结构有效性低、过度依赖代理模型评估毒性等问题。

Method: 设计MolDeTox细粒度基准，通过逐步任务实现可靠毒性感知分子优化评估；在不同设置下评估通用LLM和VLM，并采用片段级分子理解与生成策略。

Result: 片段级生成显著提高了结构有效性和生成分子质量；MolDeTox通过详细的任务级性能分析，提供了可解释的基准，帮助深入理解去毒过程。

Conclusion: MolDeTox为毒性感知分子优化提供了可靠评估工具，片段级方法能有效改善生成质量，推动药物设计中安全优化的研究。
...
### [403] [Goal-Oriented Reasoning for RAG-based Memory in Conversational Agentic LLM Systems](https://arxiv.org/abs/2605.12213)
*Jiazhou Liang,Armin Toroghi,Yifan Simon Liu,Faeze Moradi Kalarde,Liam Gallagher,Scott Sanner*

Main category: cs.AI

TL;DR: Goal-Mem 是一种面向目标的推理框架，用于基于 RAG 的智能体记忆，通过反向链和针对性检索，改善对话 AI 中的多跳推理。


<details>
  <summary>Details</summary>
Motivation: 基于 LLM 的对话智能体因上下文有限难以维持长程连贯行为；虽然 RAG 方法通过外部记忆检索缓解此问题，但现有方法仅基于原始用户话语的语义相似性检索，缺乏对缺失中间事实的推理，常返回无关或不足的证据。

Method: Goal-Mem 将用户话语作为目标，分解为原子子目标，进行针对性记忆检索以满足每个子目标，并迭代识别中间目标无法解决时需从记忆检索的信息。该方法形式化为自然语言逻辑，结合一阶逻辑的可验证性和自然语言的表达力。

Result: 在两个数据集上，与九个强基线比较，Goal-Mem 持续提升性能，尤其在需要多跳推理和隐式推断的任务上。

Conclusion: Goal-Mem 有效增强了对话智能体对检索记忆的推理能力，弥补了现有 RAG 方法的不足。
...
### [404] [No Action Without a NOD: A Heterogeneous Multi-Agent Architecture for Reliable Service Agents](https://arxiv.org/abs/2605.12240)
*Zixu Yang,Hang Zheng,Nan Jiang,Zhiyang Tang,Situo Zhang,Xiaobao Wu,Lu Chen,Kai Yu*

Main category: cs.AI

TL;DR: 提出NOD异构多智能体架构，通过外部全局状态与选择性监督提升服务智能体长期任务的可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM服务智能体在长期任务中常出现策略违规、工具幻觉和意图偏差，严重阻碍其实际部署。

Method: 提出NOD（Navigator-Operator-Director）架构：将任务状态外化为结构化Global State，由Navigator进行显式状态追踪与一致决策；引入独立Director代理在关键动作前进行选择性外部监督以验证和干预。

Result: 在τ^2-Bench上，NOD比基线取得更高的任务成功率和关键动作精度，并显著减少策略违规、工具幻觉和意图偏差。

Conclusion: NOD有效缓解了长期任务中的错误传播和不安全行为，增强了服务智能体的可靠性。
...
### [405] [Why Conclusions Diverge from the Same Observations: Formalizing World-Model Non-Identifiability via an Inference](https://arxiv.org/abs/2605.12255)
*Toru Takahashi*
...
### [406] [Missingness-MDPs: Bridging the Theory of Missing Data and POMDPs](https://arxiv.org/abs/2605.12262)
...
### [407] [How Useful Is Cross-Domain Generalization for Training LLM Monitors?](https://arxiv.org/abs/2605.12265)
...
### [408] [NARA: Anchor-Conditioned Relation-Aware Contextualization of Heterogeneous Geoentities](https://arxiv.org/abs/2605.12276)
...
### [409] [Executable Agentic Memory for GUI Agent](https://arxiv.org/abs/2605.12294)
*Zerui Qin,Sheng Yue,Xingyuan Hua,Yongjian Fu,Ju Ren*

Main category: cs.AI

TL;DR: 提出可执行智能记忆（EAM）这一结构化知识图谱，将 GUI 规划从自由生成转变为检索-执行过程，通过状态感知 DFS 和动作组挖掘压缩多步例程，并利用值引导的图搜索实现高效规划，在长链路 GUI 任务上大幅提升性能且降低成本。


<details>
  <summary>Details</summary>
Motivation: 现有模型驱动、逐步交互的 GUI 代理在长链路任务中脆弱，因每步需重新理解界面和决策，缺乏结构化记忆与高效规划。

Method: 构建 EAM 知识图谱：使用状态感知深度优先搜索和动作组挖掘进行样本高效记忆构建，压缩多步例程；规划阶段引入轻量 Q 函数引导蒙特卡洛树搜索实现对图谱的值引导搜索，理论证明 Q 模型的偏差一致性并推导路径恢复的样本复杂度界。

Result: 在 AndroidWorld 上，EAM 性能比 UI-TARS-7B 等顶尖基线最高提升 19.6%，相比 GPT-4o 令牌成本降低 6 倍，平均延迟仅 2.8 秒。

Conclusion: EAM 通过结构化记忆和高效图搜索，实现了可靠、快速且可处理长链路任务的 GUI 自动化。
...
### [410] [LISA: Cognitive Arbitration for Signal-Free Autonomous Intersection Management](https://arxiv.org/abs/2605.12321)
*Abderrahmane Lakas,Mohamed Amine Ferrag,Merouane Debbah*

Main category: cs.AI

TL;DR: 提出LISA，一个基于LLM、无信号灯的意图驱动速度建议框架，实现自主交叉口管理，大幅降低延迟和油耗，提升意图满足率。


<details>
  <summary>Details</summary>
Motivation: 现有交叉口管理依赖信号灯基础设施，基于规则的方法难以处理复杂动态交通；LLM虽具推理能力，但因推理延迟无法用于实时控制，且多作为辅助组件，缺乏直接决策能力。

Method: 设计LISA（LLM驱动意图速度建议），利用LLM推理车辆声明的意图，结合优先级类别、队列压力和能耗偏好，生成车辆速度建议，实现信号灯无关的认知仲裁。

Result: 相比固定周期、SCATS、AIM和GLOSA等基线，LISA将平均控制延迟降低最高89.1%，在接近饱和需求下维持服务水平C，而其他方法降为F；平均等待时间减少93%，峰值队列长度减少60.6%，油耗降低最高48.8%，意图满足率86.2%，显著优于非LLM最佳方法的61.2%。

Conclusion: LLM推理可支撑实时、无信号灯的交叉口管理，解决了传统方法在混合优先级和动态约束下的不足，展示了LLM在智能交通系统中的直接决策潜力。
...
### [411] [Towards Automated Air Traffic Safety Assessment Around Non-Towered Airports Using Large Language Models](https://arxiv.org/abs/2605.12332)
...
### [412] [Reinforcing VLAs in Task-Agnostic World Models](https://arxiv.org/abs/2605.12334)
*Yucen Wang,Rui Yu,Fengming Zhang,Junjie Lu,Xinyao Qin,Tianxiang Zhang,Kaixin Wang,Li Zhao*

Main category: cs.AI

TL;DR: RAW-Dream 提出一种任务无关的世界模型和视觉语言模型（VLM）奖励生成框架，使视觉-语言-动作（VLA）模型能够在不依赖任务特定数据的情况下进行零样本适应，并通过双重噪声验证机制减少世界模型的幻觉，在仿真和真实环境中均取得显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于世界模型的 VLA 强化学习微调方法仍需依赖任务特定数据来微调世界模型和奖励模型，这限制了其对未知任务的零样本扩展能力。本文旨在利用可迁移的物理先验，实现完全任务无关的想象训练。

Method: 提出 RAW-Dream，将世界模型学习与下游任务解耦：使用在多样化无任务行为上预训练的世界模型进行未来轨迹预测，并采用现成的视觉语言模型生成奖励，两者均为任务无关。在想象空间中直接微调 VLA 策略。同时引入双重噪声验证机制，过滤世界模型生成中的不可靠轨迹。

Result: 在仿真和真实环境的大量实验中，方法持续取得性能提升，证明通用的物理先验能够有效替代昂贵的任务相关数据。

Conclusion: 该工作证明任务无关的先验组件可以成功替代特定任务依赖，为 VLA 的规模化适应提供了一条高度可扩展的技术路径。
...
TL;DR: 研究表明，当前前沿模型在分类编程代理的长对话记录时，随着上下文增长容易漏检危险行为，但可通过提醒和训练缓解，忽视长上下文退化会高估监测性能。


<details>
  <summary>Details</summary>
Motivation: 实际代理监测中对话记录常超过50万token，而现有基准数据鲜有超过10万token的样本，需要评估模型在长上下文下的监测能力。

Method: 构建一个需识别编程代理微妙危险行为的数据集，将危险动作置于80万token的良性活动之后，使用Opus 4.6、GPT 5.4和Gemini 3.1作为分类器进行测试。

Result: 与危险动作单独出现相比，这些模型在长上下文中漏检该动作的频率高出2到30倍。

Conclusion: 长上下文退化可能导致监测性能被高估，但可通过在对话记录中插入定期提醒和改善后训练来部分缓解。
...
TL;DR: ProfiliTable是一个多代理框架，通过动态分析迭代优化表格处理任务的代码生成，在复杂多步骤场景中显著优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的表格处理方法常因指令模糊、任务结构复杂和缺乏结构化反馈而产生语义错误，需可靠地将模糊用户意图转换为鲁棒的表格转换。

Method: 该框架包含三个代理：(i) Profiler 使用ReAct式数据探索建立语义理解；(ii) Generator 检索相关算子合成任务感知代码；(iii) Evaluator-Summarizer 循环注入执行分数和诊断洞见，实现闭环优化。

Result: 在涵盖18种表格任务类型的基准测试中，ProfiliTable一致优于强基线，尤其在多步骤复杂任务中表现突出。

Conclusion: 动态分析在将模糊用户意图可靠转化为鲁棒且符合治理要求的表格转换中起关键作用。
...
TL;DR: 本文提出“语义奖励坍缩”（SRC）概念，指出标量化的偏好优化会让模型将不同类型的评估不满混淆为单一优化信号，导致模型压制可见的不确定性等行为；并引入“宪法式奖励分层”（CRS）作为保护认知分化型属性归属的研究方向。


<details>
  <summary>Details</summary>
Motivation: 当前RLHF与偏好优化方法仍存在表演式确定、幻觉延续、校准漂移等结构性问题，源于标量化奖励将多种异质的不满信号压缩为通用梯度，损害了模型对自身认知边界的合理表达。

Method: 本文通过概念分析与跨领域类比（制度代理坍缩、度量博弈、软件可靠性工程、人类学习理论），定义了语义奖励坍缩现象，并提出保护不确定性揭示行为的框架；在此基础上设计CRS，一种领域感知的奖励分层方案。

Result: 提出了两个核心概念：SRC揭示了标量奖励优化中异质不满信号的纠缠与可见认知失败的系统性压制；CRS作为治理导向的解决方案，旨在保持模型中不同认知类别的独立属性归因，但尚未经过实证验证。

Conclusion: 标量化的偏好优化会迫使自适应系统压制可见的认知边界表达，应将不确定性揭示视为受保护的认知行为而非任务失败；CRS可作为需要进一步实证研究的可检验治理方向。
...
TL;DR: LLMs can synthesize combinatorial solvers, but the representation paradigm matters. Python+OR-Tools yields highest correctness; prompting for search optimization often causes regressions due to heuristic traps. A conservative approach is advised: use LLMs only to formalize models for verified solvers.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with direct combinatorial reasoning, so neuro-symbolic systems use them to generate solvers. However, the optimal representation and whether LLMs should optimize search remain open questions.

Method: Introduced CP-SynC-XL benchmark with 100 problems (4,577 instances). Evaluated three solver-construction paradigms: native Python, Python + OR-Tools API, and declarative MiniZinc + OR-Tools. Also tested search optimization prompting and conducted code-level audit.

Result: Python + OR-Tools achieved highest correctness; MiniZinc + OR-Tools had lower coverage. Native Python often produced valid-looking but incorrect solutions. Optimization prompts gave only small median speed-ups (1.03–1.12x), with many instances slowing down and correctness dropping. LLMs exhibited heuristic traps: local approximations (Python), unverified bounds (Python + OR-Tools), or redundant declarative machinery (MiniZinc + OR-Tools).

Conclusion: A conservative design principle: use LLMs to formalize variables, constraints, and objectives for verified solvers, while separately verifying any LLM-authored search optimizations.
...
TL;DR: 本文提出CAAFC框架，一种时序可操作的自动事实核查系统，能够检测并纠正事实错误与幻觉，提供基于一手资料的可操作证据，并在多个基准上超越现有最先进系统。


<details>
  <summary>Details</summary>
Motivation: 当前在线内容海量增长且AI生成内容存在幻觉，人工核查不可行，而现有自动事实核查系统与专业核查实践存在显著偏差，因此需要弥合这一差距。

Method: 提出CAAFC（Chronological Actionable Automated Fact-Checker）框架，可处理声明、对话等多模态文本，检测事实错误与幻觉，并提供基于原始信息源的可操作纠正依据，同时具备动态整合最新语境信息、更新证据与知识库的能力。

Result: 在多个基准数据集上，CAAFC在自动事实核查和幻觉检测任务中均超越了当前最先进的系统。

Conclusion: CAAFC成功弥合了自动核查与实际专业需求之间的鸿沟，通过可操作纠正与动态知识更新显著提升了事实核查的可靠性。
...
TL;DR: DR-Gym是一个开源在线Gymnasium兼容环境，从电力公司视角训练和评估需求响应，包含真实极端事件电价模型和建筑需求模拟。


<details>
  <summary>Details</summary>
Motivation: 离线历史数据无法捕捉电力公司定价信号与用户接受及适应性之间的动态反馈循环，因此需要在线仿真环境来训练强化学习代理，优化需求响应计划，以保护消费者在极端电价期间免受灾难性财务风险。

Method: 提出DR-Gym环境，包含基于实际极端事件校准的区制转换批发电价模型、基于物理的建筑需求剖面，以及可配置的多目标奖励函数；环境面向电力公司市场层面决策，提供丰富的观测空间。

Result: 通过基线策略和数据快照演示，表明该模拟器能够生成真实且可学习的环境。

Conclusion: DR-Gym为强化学习驱动的需求响应提供了真实的在线训练平台，使电力公司能够优化序贯决策，从而提升电网灵活性和用户用能可负担性。
...
TL;DR: 本文研究了基于评分标准的强化学习中的奖励黑客现象，区分了验证器失败和评分标准设计局限两种偏差来源。更强验证器可减少但无法消除利用行为，且评分标准增益未必转化为整体质量提升。


<details>
  <summary>Details</summary>
Motivation: 许多开放式场景依赖基于评分标准的奖励，但该类奖励易被黑客攻击，因此需要研究其机制与缓解方法。

Method: 构建框架分离验证器失败与评分标准局限，将策略针对训练验证器优化，并用三个前沿评估器组成的跨家族面板评估，引入基于策略对数概率的自内部化差距作为无验证器诊断工具，在医学和科学领域进行实验。

Result: 弱验证器带来大量不迁移的代理奖励增益，利用随训练加剧，集中在部分满足复合标准、将隐式内容视为显式、不精确话题匹配上；强验证器显著减少但未消除利用；自内部化差距能跟踪参考验证器质量并检测策略停止改进；强验证下，评分标准验证器偏好RL检查点，而无标准人工评估偏好基模型，增益体现在完整性和存在性标准，而事实正确性、简洁性、相关性及整体质量下降。

Conclusion: 更强验证可减少奖励黑客，但单靠它无法确保评分标准增益对应于更广泛的质量提升，还需应对评分标准未涵盖的失败模式。
...
TL;DR: MarsTSC is a VLMAgentic reasoning framework for few-shot multimodal time series classification, featuring a self-evolving knowledge bank refined by agentic roles to boost performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot multimodal time series classification methods lack robust reasoning and suffer from few-shot bias and distribution shift, limiting their effectiveness and interpretability.

Method: MarsTSC uses a three-role agentic system: Generator for reasoning-based classification, Reflector to diagnose errors and extract discriminative temporal insights, and Modifier to update a knowledge bank, with a test-time refinement strategy to mitigate bias and shift.

Result: MarsTSC achieves significant and consistent performance improvements over classical and foundation model baselines on 12 time series benchmarks, across 6 VLM backbones, while generating human-readable rationales.

Conclusion: The MarsTSC framework effectively enhances few-shot multimodal time series classification through agentic reasoning and dynamic knowledge bank updates, delivering state-of-the-art performance and interpretable decisions.
